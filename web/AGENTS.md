...
- Prefer `@aletheia/shared` in frontend-safe web code for shared types, zod
  schemas, domain contracts, table definitions, prompt/eval/model-pricing
  helpers, and other cross-runtime utilities.
- Use `@aletheia/shared/src/server` only from server-only web code such as
  `src/server/**`, `src/pages/api/**`, and server tests.
- Use `@aletheia/shared/src/db` only in backend or test code that needs direct
  Prisma access; never route it into client bundles.
- Use narrower subpaths such as `@aletheia/shared/src/env` or
  `@aletheia/shared/encryption` only when that focused surface is the clearest
  dependency.
...
1. Throw `BaseError` subclasses (eg `AletheiaNotFoundError`) from handlers and services.
