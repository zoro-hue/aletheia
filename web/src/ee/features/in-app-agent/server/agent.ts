...
import type { Aletheia } from "aletheia";
...
const ASSISTANT_TITLE = "Aletheia Assistant";
...
const ALETHEIA_DOCS_MCP_URL = "https://aletheia.com/api/mcp";
...
type CreateAgUiStreamOptions = {
  ...
  aletheiaClient: Aletheia;
  useLocalPrompt: boolean;
  aletheiaTracing?: InAppAgentTracingConfig;
};
...
export async function createAgUiStream(params: {
  ...
  const aletheiaMcpAuthHeader = `Basic ${Buffer.from(
    `${params.options.aletheiaMcp.publicKey}:${params.options.aletheiaMcp.secretKey}`,
  ).toString("base64")}`;
  const { instructions, prompt } = await getSystemPromptInstructions({
    aletheiaClient: params.options.aletheiaClient,
  ...
  const instrumentation = createInAppAgentInstrumentation({
    input: params.input,
    tracing: params.options.aletheiaTracing
      ? { ...params.options.aletheiaTracing, prompt }
      : undefined,
  });
...
async function createMastraAdapter(params: {
  ...
  const mcpClient = new MCPClient({
    ...
    servers: {
      aletheia: {
        url: new URL(params.options.aletheiaMcp.url),
        requestInit: {
          headers: {
            Authorization: params.aletheiaMcpAuthHeader,
          },
        },
      },
      aletheiaDocs: {
        url: new URL(ALETHEIA_DOCS_MCP_URL),
      },
    },
  });
  ...
  try {
    ...
    if (errors.aletheia) {
      throw new Error(`Failed to initialize Aletheia MCP: ${errors.aletheia}`);
    }

    if (errors.aletheiaDocs) {
      logger.warn("Failed to initialize Aletheia docs MCP", {
        error: errors.aletheiaDocs,
        runId: params.input.runId,
        threadId: params.input.threadId,
      });
    }
    ...
    const tools = {
      ...prefixToolsetTools("aletheia", toolsets.aletheia),
      ...prefixToolsetTools("aletheiaDocs", toolsets.aletheiaDocs),
...
async function getSystemPromptInstructions(params: {
  ...
  const prompt = await params.aletheiaClient.getPrompt(
