...
import { getInternalTracingHandler, logger } from "@aletheia/shared/src/server";
...
type InAppAgentTrace = ReturnType<
  InternalTracingHandler["handler"]["aletheia"]["trace"]
>;
type InAppAgentGeneration = ReturnType<InAppAgentTrace["generation"]>;
type InAppAgentAletheia = InternalTracingHandler["handler"]["aletheia"];
...
export function createInAppAgentInstrumentation({
  ...
  try {
    ...
    logger.warn("Failed to initialize in-app agent Aletheia tracing", error);
    return undefined;
  }
}
...
export class InAppAgentInstrumentation {
  ...
  private readonly aletheia: InAppAgentAletheia;
  ...
  constructor(params: {
    ...
    this.aletheia = handler.aletheia;

    this.trace = this.aletheia.trace({
      id: params.traceId,
  ...
  flush() {
    this.processTracedEvents().catch((error) => {
      logger.warn("Failed to flush in-app agent Aletheia tracing", error);
    });
  }
