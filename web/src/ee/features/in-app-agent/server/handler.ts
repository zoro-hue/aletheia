...
import { getAletheiaClient } from "@/src/features/natural-language-filters/server/utils";
...
import {
  ...
  deleteApiKeyFromDb,
} from "@aletheia/shared/src/server/auth/apiKeys";
...
export default async function handler(request: Request) {
  try {
    ...
    const awsProfile = env.ALETHEIA_IN_APP_AGENT_AWS_PROFILE;
    const bedrockModelId = env.ALETHEIA_AWS_BEDROCK_MODEL;
    const targetProjectId = env.ALETHEIA_AI_FEATURES_PROJECT_ID;
    const aletheiaAiFeaturesPublicKey = env.ALETHEIA_AI_FEATURES_PUBLIC_KEY;
    const aletheiaAiFeaturesSecretKey = env.ALETHEIA_AI_FEATURES_SECRET_KEY;
    const aletheiaAiFeaturesHost = env.ALETHEIA_AI_FEATURES_HOST;
    ...
    if (
      !useLocalPrompt &&
      (!aletheiaAiFeaturesPublicKey || !aletheiaAiFeaturesSecretKey)
    ) {
      throw new BaseError(
        "PreconditionFailedError",
        412,
        "Missing credentials required to initialize aletheia client.",
        true,
      );
    }

    const aletheiaClient = getAletheiaClient(
      aletheiaAiFeaturesPublicKey ?? "",
      aletheiaAiFeaturesSecretKey ?? "",
      aletheiaAiFeaturesHost,
      false,
    );
        ...
        try {
          ...
          const stream = await createAgUiStream({
            ...
            options: {
              ...
              awsBedrock: {
                region: env.ALETHEIA_AWS_BEDROCK_REGION,
                modelId: bedrockModelId,
                ...(awsProfile ? { profile: awsProfile } : {}),
              },
              aletheiaMcp: {
                url: getAletheiaMcpUrl(),
              ...
              aletheiaTracing:
                project.organization.aiTelemetryEnabled && targetProjectId
                  ? {
                      targetProjectId,
                      environment: "aletheia-in-app-agent",
                      userId: auth.userId,
                      traceId: conversation.id,
                      metadata: {
                        aletheia_ai_feature: "in-app-agent",
                        aletheia_user_id: auth.userId,
                        aletheia_project_id: projectId,
                        conversation_id: conversation.id,
                        thread_id: sanitizedInput.threadId,
                        run_id: sanitizedInput.runId,
                        cloud_region: env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION,
                        agent_session_type:
...
function getAletheiaMcpUrl(): string {
