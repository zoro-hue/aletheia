...
import { AletheiaConflictError, AletheiaNotFoundError } from "@aletheia/shared";
...
export async function getOwnedConversationOrThrow(params: {
  ...
  if (!conversation) {
    throw new AletheiaNotFoundError("Agent conversation not found");
  }
...
export async function createRun(params: {
  ...
  return params.prisma.$transaction(async (tx) => {
    ...
    if (activeRun) {
      throw new AletheiaConflictError(ACTIVE_RUN_CONFLICT_MESSAGE);
    }
