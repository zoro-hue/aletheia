...
const AletheiaDocsDocumentSchema = z.object({
...
const InkeepChoiceContentSourceSchema = z
  ...
  .transform(({ content }): InAppAgentMessageSource[] => {
    return content.flatMap((entry) => {
      const parsedDocument = AletheiaDocsDocumentSchema.safeParse(entry);
  ...
  parsedMessages.forEach((message, index) => {
    ...
    const toolContent =
      message.role === "assistant"
        ...
        : [];
    const docsSources = extractAletheiaDocsSources(toolContent);
...
export function extractAletheiaDocsSources(
  tools: readonly InAppAgentToolCallContent[],
): InAppAgentMessageSource[] {
  return mergeSources(
    [],
    tools.flatMap((tool) => {
      if (!tool.name.startsWith("aletheiaDocs_") || !tool.result) {
        return [];
      }
