...
export interface AdminAuthOptions {
  isAllowedOnAletheiaCloud?: boolean;
}
...
export class AdminApiAuthService {
  static verifyAdminAuthFromAuthString = (
    ...
    const { isAllowedOnAletheiaCloud = false } = options;

    // Block access on Aletheia Cloud unless explicitly allowed
    if (
      !isAllowedOnAletheiaCloud &&
      env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION &&
      env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION !== "DEV" // exclude dev and CI environments
    ) {
      return {
        isAuthorized: false,
        error: "Not accessible on Aletheia Cloud",
      };
    }
   ...
   * @param options Admin auth options. By default, blocks access on Aletheia Cloud (isAllowedOnAletheiaCloud: false)
