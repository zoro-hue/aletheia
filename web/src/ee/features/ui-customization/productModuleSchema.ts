...
/**
 * All available product modules in Aletheia
 */
 ...
 * - When ALETHEIA_UI_VISIBLE_PRODUCT_MODULES is set: only listed modules are visible
 * - When ALETHEIA_UI_HIDDEN_PRODUCT_MODULES is set: all modules except listed ones are visible
 *
 * If both are set, ALETHEIA_UI_VISIBLE_PRODUCT_MODULES takes precedence.
...
export function getVisibleProductModules(
  ...
  if (visibleModulesEnv && hiddenModulesEnv) {
    console.warn(
      "Both ALETHEIA_UI_VISIBLE_PRODUCT_MODULES and ALETHEIA_UI_HIDDEN_PRODUCT_MODULES are set. " +
        "Using ALETHEIA_UI_VISIBLE_PRODUCT_MODULES as the allow list.",
    );
