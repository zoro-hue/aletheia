...
export async function stripeWebhookHandler(req: NextRequest) {
  ...
  if (!env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION || !stripeClient) {
    logger.error("[Stripe Webhook] Endpoint only available in Aletheia Cloud");
    return NextResponse.json(
      { message: "Stripe webhook endpoint only available in Aletheia Cloud" },
      { status: 500 },
    );
  }
...
/**
 * Retrieves an organization by its Aletheia organization ID.
 */
...
async function ensureMetadataIsSetOnStripeSubscription(
  ...
  const currentEnvironment = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;

  if (!currentEnvironment) {
    traceException(
      "[Stripe Webhook] NEXT_PUBLIC_ALETHEIA_CLOUD_REGION is not set but webhook is running",
    );
...
export async function handleSubscriptionChanged(
  ...
  if (!currentEnvironment) {
    traceException(
      `[Stripe Webhook] NEXT_PUBLIC_ALETHEIA_CLOUD_REGION is not set but webhook received event subscription.${action}`,
    );
    throw new InternalServerError(
      `[Stripe Webhook] NEXT_PUBLIC_ALETHEIA_CLOUD_REGION is not set but webhook received event subscription.${action}`, // we throw here because this should really never happen
    );
  }
