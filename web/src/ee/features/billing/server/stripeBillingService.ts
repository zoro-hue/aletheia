...
import { parseDbOrg } from "@aletheia/shared";
import {
  logger,
    ...
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        span.setAttributes({
          ...
          "aletheia.org.id": parsedOrg.id,
          "aletheia.user.id": this.ctx.session.user.id,
        });
    ...
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        span.setAttributes({
          "stripe.subscription_id": subscriptionId,
          "aletheia.org.id": parsedOrg.id,
          "aletheia.user.id": this.ctx.session.user?.id ?? "system",
