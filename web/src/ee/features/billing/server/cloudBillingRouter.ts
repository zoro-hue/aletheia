  ...
  getSubscriptionInfo: protectedOrganizationProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      throwIfNoOrganizationAccess({
        ...
        scope: "aletheiaCloudBilling:CRUD",
        session: ctx.session,
      });
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!isCloudBillingEnabled()) {
        throw new TRPCError({
          ...
          message:
            "Cloud billing is not available in this environment. This feature requires NEXT_PUBLIC_ALETHEIA_CLOUD_REGION to be configured.",
        });
      }
