 ...
 *
 * @throws {TRPCError} If not running in a Aletheia Cloud environment
 */
export const createStripeClientReference = (orgId: string) => {
  if (!env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) {
    logger.error(
      "Returning null stripeClientReference, you cannot run the checkout page outside of Aletheia Cloud",
    );
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message:
        "Cannot create stripe client reference outside of Aletheia Cloud",
    });
  }
  return `${env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION}-${orgId}`;
};
...
export const isStripeClientReferenceFromCurrentCloudRegion = (
  ...
  clientReference.startsWith(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION);
...
export const getOrgIdFromStripeClientReference = (clientReference: string) =>
  clientReference.replace(`${env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION}-`, "");
