...
import { BaseError, parseIO } from "@aletheia/shared";
...
const withErrorHandling = t.middleware(async ({ ctx, next }) => {
  ...
  if (!res.ok) {
    if (res.error.cause instanceof ClickHouseResourceError) {
      ...
      const errorMessage = isAletheiaCloud
...
/** requireAletheiaCloud rejects calls from non-Aletheia-Cloud deployments. */
export const requireAletheiaCloud = t.middleware(({ next }) => {
  if (!isAletheiaCloud) {
