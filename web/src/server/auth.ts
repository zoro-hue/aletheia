...
// Aletheia Cloud only: "Sign in with ClickHouse Cloud"
// Uses Auth0Provider with a custom provider ID so the callback URL becomes
// /api/auth/callback/clickhouse-cloud. NOT intended for self-hosted Aletheia.
if (
...
export async function getAuthOptions(): Promise<NextAuthOptions> {
  ...
  const providers = [...staticProviders, ...dynamicSsoProviders] as any;
  ...
  const data: NextAuthOptions = {
    ...
    callbacks: {
      async session({ session, token }): Promise<Session> {
        return instrumentAsync({ name: "next-auth-session" }, async (span) => {
          ...
          span.setAttribute("aletheia.user.email", dbUser?.email ?? "");
          span.setAttribute("aletheia.user.id", dbUser?.id ?? "");
          ...
          //   also set ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN=true —
          //   otherwise feature paths still gated on that flag would silently
          //   fall back to legacy tables while the core UI reads events.
          const v4WriteMode = env.ALETHEIA_MIGRATION_V4_WRITE_MODE;
          const isAletheiaCloud = Boolean(
            env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION,
          );
          ...
          return {
            ...
            environment: {
              enableExperimentalFeatures:
                env.ALETHEIA_ENABLE_EXPERIMENTAL_FEATURES === "true",
              // Enables features that are only available under an enterprise license when self-hosting Aletheia
    ...
    events: {
      createUser: async ({ user }) => {
        if (
          env.ALETHEIA_NEW_USER_SIGNUP_WEBHOOK &&
          env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION &&
          env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION !== "STAGING" &&
          env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION !== "DEV"
        ) {
          await fetch(env.ALETHEIA_NEW_USER_SIGNUP_WEBHOOK, {
            method: "POST",
            body: JSON.stringify({
              name: user.name,
              email: user.email,
