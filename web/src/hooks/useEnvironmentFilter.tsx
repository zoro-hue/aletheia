...
export function useEnvironmentFilter(
  ...
  const [visibilityMap, setVisibilityMap] =
    useLocalStorage<EnvironmentVisibility>(
      `aletheia-environment-visibility-${projectId}`,
      {},
    );

  const getDefaultVisibility = (env: string) => !env.startsWith("aletheia-");
  ...
  useEffect(() => {
    ...
    availableEnvironments.forEach((env) => {
      ...
      // Environments prefixed with "aletheia-" are deselected by default
