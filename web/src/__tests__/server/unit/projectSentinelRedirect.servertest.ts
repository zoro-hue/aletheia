const mockEnv = vi.hoisted(() => ({
  env: {
    NEXTAUTH_URL: "https://us.cloud.aletheia.com",
    NEXTAUTH_COOKIE_DOMAIN: "aletheia.com",
...
const makeCtx = (
  ...
  return {
    req: {
      headers: {
        host: req.host ?? "us.cloud.aletheia.com",
...
describe("sentinel redirect /project/~/", () => {
  ...
  it("same-origin cookie, member project: redirects to cookie project", async () => {
    ...
    const result = await getServerSideProps(
      makeCtx({ path: ["traces"] }, "/project/~/traces", {
        host: "us.cloud.aletheia.com",
        cookie: {
          origin: "https://us.cloud.aletheia.com",
          projectId: "proj-last",
        },
      }),
    );
  ...
  it("cross-region cookie, same parent domain: bounces to cookie origin sentinel", async () => {
    ...
    const result = await getServerSideProps(
      makeCtx({ path: ["traces"] }, "/project/~/traces", {
        host: "us.cloud.aletheia.com",
        cookie: {
          origin: "https://eu.cloud.aletheia.com",
          projectId: "proj-eu",
        },
      }),
    );
    ...
    expect(result).toEqual({
      redirect: {
        destination: "https://eu.cloud.aletheia.com/project/~/traces",
        permanent: false,
      },
    });
  });
  ...
  it("cross-region bounce: preserves query string on cross-host redirect", async () => {
    ...
    expect(result).toEqual({
      redirect: {
        destination: "https://eu.cloud.aletheia.com/project/~/traces?foo=bar",
        permanent: false,
      },
    });
  });
