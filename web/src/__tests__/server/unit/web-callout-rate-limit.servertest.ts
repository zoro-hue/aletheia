...
describe("web callout rate limiting", () => {
  const originalRateLimitsEnabled = env.ALETHEIA_RATE_LIMITS_ENABLED;
  const context = {
    orgId: "org-1",
    projectId: "project-1",
  ...
  beforeEach(() => {
    (env as any).ALETHEIA_RATE_LIMITS_ENABLED = "true";
  ...
  afterEach(() => {
    (env as any).ALETHEIA_RATE_LIMITS_ENABLED = originalRateLimitsEnabled;
    WebCalloutRateLimitService.shutdown();
    resetWebCalloutInFlightLimitsForTests();
  });
  ...
  it("uses scoped 10/60 Redis buckets, shares cold connects, and fails open when Redis is unavailable", async () => {
    ...
    WebCalloutRateLimitService.shutdown();
    mocks.options.length = 0;
    (env as any).ALETHEIA_RATE_LIMITS_ENABLED = "false";
    await expect(
