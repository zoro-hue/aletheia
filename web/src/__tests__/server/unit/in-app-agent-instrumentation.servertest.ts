...
vi.mock("@aletheia/shared/src/server", () => ({
...
describe("InAppAgentInstrumentation", () => {
  ...
  it("records agent output and tool calls as tool observations", () => {
    ...
    expect(mocks.handler.aletheia.trace).toHaveBeenCalledWith(
      expect.objectContaining({ id: traceId, name: "in-app-agent" }),
    );
    expect(mocks.handler.aletheia.trace.mock.calls[0][0]).not.toHaveProperty(
      "input",
    );
    ...
    expect(mocks.handler.aletheia.enqueue).toHaveBeenCalledWith(
      "tool-create",
      expect.objectContaining({
        id: "tool-1",
        traceId,
    ...
    expect(mocks.handler.aletheia.enqueue).not.toHaveBeenCalledWith(
      "span-create",
      expect.anything(),
    );
      ...
      context: [
        {
          ...
          value:
            "https://cloud.aletheia.com/project/project-1/traces?filter=value",
        },
      ],
    });
