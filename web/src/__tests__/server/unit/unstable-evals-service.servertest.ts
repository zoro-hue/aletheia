...
vi.mock("@aletheia/shared/src/db", async () => {
...
import { EvalTemplateType, prisma } from "@aletheia/shared/src/db";
...
describe("unstable public eval services", () => {
  ...
  it("rejects user-provided mappings for code evaluation rule updates", async () => {
    ...
    await expect(
      ...
      "Code evaluator mappings are managed by Aletheia and cannot be provided in the request body.",
    );
