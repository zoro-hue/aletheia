...
import { decrypt } from "@aletheia/shared/encryption";
import {
...
vi.mock("@aletheia/shared/encryption", async () => {
  const actual = await vi.importActual("@aletheia/shared/encryption");
  return {
    ...actual,
...
describe("webCallouts router", () => {
  ...
  it("allows loopback callout targets in local development", async () => {
    ...
    await createEndpoint(caller, projectId, {
      url: "http://127.0.0.1:4047/api/aletheia-trace",
      toastMessage: "Callout sent",
    });

    expect(validateWebhookURL).toHaveBeenCalledWith(
      "http://127.0.0.1:4047/api/aletheia-trace",
