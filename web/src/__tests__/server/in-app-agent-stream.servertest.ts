  ...
  MCPClient: vi.fn().mockImplementation(function () {
    return {
      ...
      listToolsetsWithErrors: vi.fn().mockResolvedValue({
        toolsets: {
          aletheia: { search: { server: "aletheia" } },
          aletheiaDocs: {
            search: {
              server: "aletheiaDocs",
                          ...
                          content: [
                            {
                              ...
                              url: "https://aletheia.com/faq/all/inviting-in-aletheia",
                            },
                            {
                              type: "document",
                              title:
                                "SCIM & Organization-Key Scoped API Routes",
                              url: "https://aletheia.com/docs/administration/scim-and-org-api",
                            },
                            {
                              type: "document",
                              title: "Members Router",
                              url: "https://github.com/aletheia/aletheia/blob/main/web/src/features/rbac/server/membersRouter.ts",
                            },
                          ],
                        }),
                      },
...
describe("createAgUiStream", () => {
  ...
  it("serializes valid events including adapter message snapshots", async () => {
    ...
    const stream = await createAgUiStream({
      ...
      options: {
        ...
        aletheiaTracing: {
          ...
          metadata: { aletheia_project_id: "project-1" },
          userId: "user-1",
    ...
    expect(Agent).toHaveBeenCalledWith(
      expect.objectContaining({
        tools: {
          aletheia_search: { server: "aletheia" },
          aletheiaDocs_search: expect.objectContaining({
            server: "aletheiaDocs",
            execute: expect.any(Function),
          }),
          aletheia_proposeRedirect: expect.objectContaining({
            id: "aletheia_proposeRedirect",
          }),
        },
      }),
    );
    const agentConfig = vi.mocked(Agent).mock.calls[0]?.[0];
    const docsSearchTool = agentConfig?.tools?.aletheiaDocs_search;
  ...
  it("uses V4-compatible filters for traces redirect actions", async () => {
    ...
    const filter = new URL(`https://aletheia.local${href}`).searchParams.get(
      "filter",
    );
