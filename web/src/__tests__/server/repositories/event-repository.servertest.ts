...
import { type FilterCondition } from "@aletheia/shared";
...
describe("Clickhouse Events Repository Test", () => {
  ...
  maybe("getLatestSdkVersionInfoFromEvents", () => {
      ...
      await createEventsCh([
        createEvent({
          ...
          metadata_values: [
            ...
            '{"name":"aletheia-sdk","version":"3.14.6"}',
          ],
    ...
    it("should return v4 version from direct columns when available", async () => {
      ...
      await createEventsCh([
        createEvent({
          ...
          metadata_values: ["aletheia-sdk", "4.2.0"],
          scope_name: "aletheia-sdk",
          scope_version: "4.2.0",
    ...
    it("should return isOtel: true for Vercel AI SDK events (OTel without Aletheia SDK)", async () => {
      const uniqueProjectId = randomUUID();
      const now = Date.now() * 1000;

      // Vercel AI SDK: sent via OTel but without Aletheia SDK
