...
describe("Admin API Key Authentication", () => {
  ...
  describe("Admin API Key Auth (isAdminApiKeyAuthAllowed = true)", () => {
    it("should successfully authenticate with valid admin API key", async () => {
      const mockReq = {
        headers: {
          ...
          "x-aletheia-admin-api-key": ADMIN_API_KEY,
          "x-aletheia-project-id": projectId,
        },
      } as NextApiRequest;
    ...
    it("should fall back to basic auth when no x-aletheia-admin-api-key header", async () => {
      const mockReq = {
        headers: {
          authorization: auth,
        },
    ...
    it("should fail on Aletheia Cloud", async () => {
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "prod-us";
    ...
    it("should fail when ADMIN_API_KEY is not configured", async () => {
      ...
      const mockReq = {
        headers: {
          ...
          "x-aletheia-admin-api-key": "some-key",
          "x-aletheia-project-id": projectId,
        },
      } as NextApiRequest;
    ...
    it("should fail with invalid x-aletheia-admin-api-key header", async () => {
      const mockReq = {
        headers: {
          authorization: `Bearer ${ADMIN_API_KEY}`,
          "x-aletheia-admin-api-key": "wrong-key",
          "x-aletheia-project-id": projectId,
        },
      } as NextApiRequest;
    ...
    it("should fail when both admin key headers don't match ADMIN_API_KEY", async () => {
      const mockReq = {
        headers: {
          ...
          "x-aletheia-admin-api-key": "different-key-2",
          "x-aletheia-project-id": projectId,
        },
      } as NextApiRequest;
    ...
    it("should fail without x-aletheia-project-id header", async () => {
      const mockReq = {
        headers: {
          authorization: `Bearer ${ADMIN_API_KEY}`,
          "x-aletheia-admin-api-key": ADMIN_API_KEY,
    ...
    it("should fail with non-existent project", async () => {
      const mockReq = {
        headers: {
          authorization: `Bearer ${ADMIN_API_KEY}`,
          "x-aletheia-admin-api-key": ADMIN_API_KEY,
          "x-aletheia-project-id": "non-existent-project",
        },
      } as NextApiRequest;
