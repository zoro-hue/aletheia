...
import { createOrgProjectAndApiKey } from "@aletheia/shared/src/server";
import {
...
describe("evals trpc", () => {
  ...
  describe("evals.deleteEvalTemplate", () => {
    ...
    it("should reject deletion of aletheia-managed evaluators", async () => {
      const { project, caller } = await prepare();

      const managedTemplate = await createTemplateVersion(
        null,
        `aletheia-managed-template-${project.id}`,
        1,
      );
      ...
      try {
        ...
        ).rejects.toThrow("Aletheia-managed evaluators cannot be deleted");
      } finally {
