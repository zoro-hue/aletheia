...
describe("/api/public/traces API Endpoint", () => {
  ...
  describe.skip("GET /api/public/traces env var controls", () => {
    const originalRejectNoDateRange =
      env.ALETHEIA_API_TRACES_REJECT_NO_DATE_RANGE;
    const originalDefaultDateRangeDays =
      env.ALETHEIA_API_TRACES_DEFAULT_DATE_RANGE_DAYS;
    const originalDefaultFields = env.ALETHEIA_API_TRACES_DEFAULT_FIELDS;

    afterEach(() => {
      (env as any).ALETHEIA_API_TRACES_REJECT_NO_DATE_RANGE =
        originalRejectNoDateRange;
      (env as any).ALETHEIA_API_TRACES_DEFAULT_DATE_RANGE_DAYS =
        originalDefaultDateRangeDays;
      (env as any).ALETHEIA_API_TRACES_DEFAULT_FIELDS = originalDefaultFields;
    });

    it("should return 400 when REJECT_NO_DATE_RANGE=true and no fromTimestamp", async () => {
      (env as any).ALETHEIA_API_TRACES_REJECT_NO_DATE_RANGE = "true";
    ...
    it("should reject even when DEFAULT_DATE_RANGE_DAYS is also set (rejection takes precedence)", async () => {
      (env as any).ALETHEIA_API_TRACES_REJECT_NO_DATE_RANGE = "true";
      (env as any).ALETHEIA_API_TRACES_DEFAULT_DATE_RANGE_DAYS = 7;
    ...
    it("should apply DEFAULT_FIELDS when no fields query param is provided", async () => {
      (env as any).ALETHEIA_API_TRACES_DEFAULT_FIELDS = "core";
