...
import { DashboardService } from "@aletheia/shared/src/server";
import { DashboardWidgetViews } from "@aletheia/shared/src/db";
import { prisma } from "@aletheia/shared/src/db";
import { appRouter } from "@/src/server/api/root";
import { createInnerTRPCContext } from "@/src/server/api/trpc";
import type { Session } from "next-auth";
import { requiresV2 } from "@aletheia/shared/query";
import {
  ...
  describe("DashboardService", () => {
    ...
    it("should preserve minVersion when copying widget to project", async () => {
      const sourceWidget = await prisma.dashboardWidget.create({
        data: {
          name: "Aletheia Widget",
          description: "A Aletheia-managed widget",
          view: DashboardWidgetViews.OBSERVATIONS,
          dimensions: [{ field: "name" }],
          metrics: [{ measure: "count", agg: "count" }],
          filters: [],
