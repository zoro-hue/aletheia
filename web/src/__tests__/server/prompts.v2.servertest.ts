...
import { parsePromptDependencyTags } from "@aletheia/shared";
...
describe("/api/public/v2/prompts API Endpoint", () => {
  ...
  describe("when creating a prompt", () => {
    ...
    it("should fail if chat prompt has incorrect messages format", async () => {
      ...
      expect(status).toBe(404);
      // @ts-expect-error
      expect(body.error).toBe("AletheiaNotFoundError");
    });
    ...
    it("should fail if previous versions have different prompt type", async () => {
      ...
      // @ts-expect-error
      expect(getResponse2.body.error).toBe("AletheiaNotFoundError");
    });
  ...
  describe("when fetching a prompt list", () => {
    ...
    it("should fetch a prompt meta list", async () => {
      ...
      // https://github.com/aletheia/aletheia/issues/2068
  ...
  describe("Unresolved prompt fetching via public API (resolve parameter)", () => {
    it("should return resolved prompt by default (backward compatibility)", async () => {
      ...
      const parentContent = `Parent prompt with dependency: @@@aletheiaPrompt:name=${childPromptName}|label=production@@@`;
      ...
      // Should be resolved (no @@@aletheiaPrompt tags)
      expect(body.prompt).not.toContain("@@@aletheiaPrompt");
    ...
    it("should return resolved prompt when resolve=true", async () => {
      ...
      // Create parent prompt with dependency
      const parentPromptName = "parent-prompt-" + nanoid();
      const parentContent = `Parent: @@@aletheiaPrompt:name=${childPromptName}|label=production@@@`;
    ...
    it("should return unresolved prompt when resolve=false", async () => {
      ...
      expect(response.status).toBe(200);
      const body = response.body as Prompt;
      // Should be unresolved (keep @@@aletheiaPrompt tags)
      expect(body.prompt).toContain("@@@aletheiaPrompt");
      expect(body.prompt).toContain(
        `@@@aletheiaPrompt:name=${childPromptName}|label=production@@@`,
      );
    ...
    it("should return unresolved chat prompt when resolve=false", async () => {
      ...
      const chatMessages = [
        {
          role: "system",
          content: `System: @@@aletheiaPrompt:name=${childPromptName}|label=production@@@`,
        },
      ...
      expect(messages[0].content).toContain("@@@aletheiaPrompt");
    ...
    it("should return the raw prompt when resolve=false even if a dependency is missing", async () => {
      ...
      const parentPromptName = "parent-prompt-" + nanoid();
      const parentContent =
        "Parent: @@@aletheiaPrompt:name=missing-child-prompt|version=1@@@";
...
describe("PATCH api/public/v2/prompts/[promptName]/versions/[version]", () => {
  ...
  describe("prompt composability", () => {
    it("can create a prompt with dependencies linked via label", async () => {
      ...
      const parentPromptContent =
        "Parent prompt with dependency: @@@aletheiaPrompt:name=child-prompt|label=production@@@";
      const response = await makeAPICall(
        "POST",
        baseURI,
        {
    ...
    it("resolves prompt dependencies correctly when linked via label", async () => {
      ...
      await makeAPICall(
        ...
        baseURI,
        {
          name: "intermediate",
          prompt:
            "Intermediate with dependency: @@@aletheiaPrompt:name=nested-child|label=production@@@",
          type: "text",
          labels: ["production"],
        },
        newAuth,
      ...
      await makeAPICall(
        ...
        baseURI,
        {
          name: "nested-parent",
          prompt:
            "Parent with nested dependency: @@@aletheiaPrompt:name=intermediate|label=production@@@",
          type: "text",
        },
        newAuth,
      );
    ...
    it("resolves prompt dependencies correctly when linked via version", async () => {
      ...
      const parentPromptContent =
        "Parent with version dependency: @@@aletheiaPrompt:name=version-linked-child|version=1@@@";
      ...
      expect(responseBody.prompt).not.toContain("@@@aletheiaPrompt");
    });
    ...
    it("detects circular dependencies", async () => {
      ...
      await makeAPICall(
        ...
        baseURI,
        {
          name: "circular-b",
          prompt:
            "Prompt B with dependency: @@@aletheiaPrompt:name=circular-a|label=production@@@",
          type: "text",
          labels: ["production"],
        },
        newAuth,
      ...
      const circularResponse = await makeAPICall(
        ...
        baseURI,
        {
          name: "circular-a",
          prompt:
            "Prompt A with dependency: @@@aletheiaPrompt:name=circular-b|label=production@@@",
          type: "text",
          labels: ["production"],
        },
        newAuth,
    ...
    it("handles deeply nested dependencies (3+ levels)", async () => {
      ...
      await makeAPICall(
        ...
        baseURI,
        {
          name: "level-2",
          prompt:
            "Level 2 with dependencies: @@@aletheiaPrompt:name=level-3-a|label=production@@@ and @@@aletheiaPrompt:name=level-3-b|label=production@@@",
          type: "text",
          labels: ["production"],
        },
        newAuth,
      ...
      await makeAPICall(
        ...
        baseURI,
        {
          name: "level-1",
          prompt:
            "Level 1 with dependency: @@@aletheiaPrompt:name=level-2|label=production@@@",
          type: "text",
        },
        newAuth,
      );
    ...
    it("handles multiple dependencies at the same level", async () => {
      ...
      const parentPromptContent = `Parent prompt with multiple dependencies: First: @@@aletheiaPrompt:name=child-a|label=production@@@ Second: @@@aletheiaPrompt:name=child-b|label=production@@@ Third: @@@aletheiaPrompt:name=child-c|label=production@@@`;
    ...
    it("handles version-specific dependencies", async () => {
      ...
      // Create parent prompt with version-specific dependency
      const parentPromptContent =
        "Parent with version dependency: @@@aletheiaPrompt:name=versioned-child|version=1@@@";
    ...
    it("resolves prompt dependencies in chat prompts correctly", async () => {
      ...
      const chatPromptContent = [
        { role: "system", content: "You are a helpful assistant" },
        {
          role: "user",
          content:
            "Here's some context: @@@aletheiaPrompt:name=chat-child-text|label=production@@@",
        },
      ];
    ...
    it("supports nested dependencies in chat prompts", async () => {
      ...
      const intermediatePromptContent =
        "Intermediate prompt with dependency: @@@aletheiaPrompt:name=nested-child|label=production@@@";
      ...
      const chatPromptContent = [
        { role: "system", content: "You are a helpful assistant" },
        {
          role: "user",
          content:
            "Here's some context: @@@aletheiaPrompt:name=intermediate-prompt|label=production@@@",
        },
      ];
      ...
      expect(JSON.stringify(parsedPrompt)).not.toContain("@@@aletheiaPrompt");
    });
    ...
    it("should ignore invalid prompt tags", async () => {
      ...
      const invalidTagPromptContent = JSON.stringify({
        text: "This prompt has invalid tags: @@@aletheiaPrompt:invalid@@@, @@@aletheiaPrompt:name=missing-type@@@, @@@aletheiaPrompt:name=no-version-or-label@@@",
      });
      ...
      expect(parsedPrompt.text).toContain("@@@aletheiaPrompt:invalid@@@");
      expect(parsedPrompt.text).toContain(
        "@@@aletheiaPrompt:name=missing-type@@@",
      );
      expect(parsedPrompt.text).toContain(
        "@@@aletheiaPrompt:name=no-version-or-label@@@",
      );
    });
    ...
    it("should handle duplicate dependency tags for the same prompt", async () => {
      ...
      const duplicateTagsPromptContent =
        "This prompt has duplicate tags: @@@aletheiaPrompt:name=parent-prompt|version=1@@@ and again @@@aletheiaPrompt:name=parent-prompt|version=1@@@";
    ...
    it("should handle special characters in prompt names", async () => {
      ...
      @@@aletheiaPrompt:name=prompt-with-hyphens|version=1@@@
      @@@aletheiaPrompt:name=prompt_with_underscores|version=1@@@
      @@@aletheiaPrompt:name=prompt.with.dots|version=1@@@
      @@@aletheiaPrompt:name=prompt123WithNumbers|version=1@@@
      @@@aletheiaPrompt:name=prompt with spaces|version=1@@@
      @@@aletheiaPrompt:name=prompt/with/slashes|version=1@@@
    `;
    ...
    it("should disallow a prompt that references itself", async () => {
      ...
      @@@aletheiaPrompt:name=self-referencing-prompt|version=1@@@
      And that's it!`;
    ...
    it("should return an error when a dependent prompt doesn't exist", async () => {
      ...
      @@@aletheiaPrompt:name=non-existent-prompt|version=1@@@
      End of prompt.`;
    ...
    it("should return an error when a dependent prompt exists but in a different project", async () => {
      ...
      const promptWithCrossProjectDependency = `This prompt references a prompt from another project: @@@aletheiaPrompt:name=cross-project-prompt|version=1@@@ End of prompt.`;
    ...
    it("should throw an error if MAX_PROMPT_NESTING_DEPTH is exceeded", async () => {
      ...
      for (let i = MAX_PROMPT_NESTING_DEPTH; i >= 1; i--) {
        ...
        if (i === MAX_PROMPT_NESTING_DEPTH) {
          ...
          promptContent = `Level ${i} with dependency: @@@aletheiaPrompt:name=depth-${i + 1}|label=production@@@`;
        }
      ...
      const promptContent = `Level 0 with dependency: @@@aletheiaPrompt:name=depth-1|label=production@@@`;
    ...
    it("returns 400 when deleting prompt with dependencies", async () => {
      ...
      const parentPrompt = await prisma.prompt.create({
        data: {
          id: uuidv4(),
          name: parentName,
          prompt: `Parent with dependency: @@@aletheiaPrompt:name=${childName}|version=1@@@`,
          labels: ["production"],
          version: 1,
          projectId,
          createdBy: "user",
    ...
    it("handles mixed version and label dependencies correctly", async () => {
      ...
      const parent1 = await prisma.prompt.create({
        data: {
          id: uuidv4(),
          name: parent1Name,
          prompt: `Depends on @@@aletheiaPrompt:name=${childName}|version=1@@@`,
          labels: ["production"],
          version: 1,
          projectId,
          createdBy: "user",
      ...
      const parent2 = await prisma.prompt.create({
        data: {
          id: uuidv4(),
          name: parent2Name,
          prompt: `Depends on @@@aletheiaPrompt:name=${childName}|label=production@@@`,
          labels: ["production"],
          version: 1,
          projectId,
          createdBy: "user",
      ...
      const parent3 = await prisma.prompt.create({
        data: {
          id: uuidv4(),
          name: parent3Name,
          prompt: `Depends on @@@aletheiaPrompt:name=${childName}|label=latest@@@`,
          labels: ["production"],
          version: 1,
          projectId,
          createdBy: "user",
  ...
  describe("Parsing prompt dependency tags", () => {
    it("should extract prompt dependency tags with version", () => {
      const content =
        "This is a test with @@@aletheiaPrompt:name=test|version=1@@@ dependency";
    ...
    it("should handle prompt names with special characters", () => {
      const content = `
        @@@aletheiaPrompt:name=test-with-hyphens|version=1@@@
        @@@aletheiaPrompt:name=test with spaces|label=production@@@
        @@@aletheiaPrompt:name=test_with_underscores|version=2@@@
        @@@aletheiaPrompt:name=test.with.dots|label=staging@@@
        @@@aletheiaPrompt:name=test123WithNumbers|version=3@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should extract prompt dependency tags with label", () => {
      const content =
        "This is a test with @@@aletheiaPrompt:name=test|label=production@@@ dependency";
      const result = parsePromptDependencyTags(content);
    ...
    it("should extract multiple prompt dependency tags", () => {
      const content = `
        First dependency: @@@aletheiaPrompt:name=first|version=1@@@
        Second dependency: @@@aletheiaPrompt:name=second|label=staging@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should ignore invalid prompt dependency tags", () => {
      const content = `
        Valid: @@@aletheiaPrompt:name=valid|version=1@@@
        Invalid: @@@aletheiaPrompt:version=1@@@
        Also invalid: @@@aletheiaPrompt:name=invalid|something=else@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should handle tags with special characters in name", () => {
      const content =
        "Tag with special chars @@@aletheiaPrompt:name=test-prompt_123|version=2@@@";
      const result = parsePromptDependencyTags(content);
    ...
    it("should handle tags with special characters in label", () => {
      const content =
        "Tag with special chars @@@aletheiaPrompt:name=test|label=prod-v1.0_beta@@@";
      const result = parsePromptDependencyTags(content);
    ...
    it("should correctly coerce version to number", () => {
      const content =
        "Version as string @@@aletheiaPrompt:name=test|version=123@@@";
      const result = parsePromptDependencyTags(content);
    ...
    it("should handle tags with spaces in the content", () => {
      const content =
        "Tag with spaces @@@aletheiaPrompt:name=my prompt|label=production label@@@";
      const result = parsePromptDependencyTags(content);
    ...
    it("should handle multiple tags with the same name but different versions/labels", () => {
      const content = `
        @@@aletheiaPrompt:name=same|version=1@@@
        @@@aletheiaPrompt:name=same|version=2@@@
        @@@aletheiaPrompt:name=same|label=production@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should handle tags with the PRODUCTION_LABEL constant value", () => {
      const content = "@@@aletheiaPrompt:name=test|label=production@@@";
      const result = parsePromptDependencyTags(content);
    ...
    it("should ignore malformed tags that don't match the regex pattern", () => {
      const content = `
        Valid: @@@aletheiaPrompt:name=valid|version=1@@@
        Malformed: @@aletheiaPrompt:name=test|version=1@@
        Also malformed: @@@aletheiaPrompt:name=test|version=1
        And: aletheiaPrompt:name=test|version=1@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should not parse aletheiaMedia tags as prompt dependency tags", () => {
      const content = `
        @@@aletheiaPrompt:name=valid|version=1@@@
        @@@aletheiaMedia:type=image/jpeg|id=cc48838a-3da8-4ca4-a007-2cf8df930e69|source=base64@@@
        @@@aletheiaPrompt:name=another|label=production@@@
      `;
      const result = parsePromptDependencyTags(content);
    ...
    it("should reject tags where name is not the first parameter", () => {
      const content = `
        Valid: @@@aletheiaPrompt:name=valid|version=1@@@
        Invalid: @@@aletheiaPrompt:version=1|name=test@@@
      `;
      const result = parsePromptDependencyTags(content);
