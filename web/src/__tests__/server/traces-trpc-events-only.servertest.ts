 ...
 * when ALETHEIA_MIGRATION_V4_WRITE_MODE=events_only; in legacy/dual the legacy
...
const eventsTableAvailable = vi.hoisted(() => {
  ...
  process.env.ALETHEIA_MIGRATION_V4_WRITE_MODE = "events_only";
...
maybe("traces trpc (events_only write mode)", () => {
  ...
  it("forces events_only write mode", () => {
    expect(env.ALETHEIA_MIGRATION_V4_WRITE_MODE).toBe("events_only");
  });
