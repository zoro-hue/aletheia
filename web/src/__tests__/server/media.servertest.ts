...
describe("Media Upload API", () => {
  ...
  const isAzureBlobMode =
    process.env.ALETHEIA_USE_AZURE_BLOB === "true" ||
    env.ALETHEIA_S3_MEDIA_UPLOAD_ACCESS_KEY_ID === "devstoreaccount1" ||
    env.ALETHEIA_S3_MEDIA_UPLOAD_ENDPOINT?.includes(":10000/") === true;
  ...
  const imagePathPNG = path.join(staticFixtureDir, "aletheia-logo.png");
  ...
  describe("Request Validation", () => {
    ...
    it("should reject content length exceeding maximum allowed size", async () => {
      ...
      const result = await runMediaUploadEndToEndTest({
        ...
        traceId,
        field,
        contentLength: env.ALETHEIA_S3_MEDIA_MAX_CONTENT_LENGTH + 1,
      });
