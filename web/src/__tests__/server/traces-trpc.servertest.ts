...
import { BatchExportFileFormat, BatchTableNames } from "@aletheia/shared";

describe("traces trpc", () => {
  const projectId = "7a88fb47-b4e2-43b8-a06c-a5ce950dc53a";
  const mutableEnv = env as unknown as {
    ALETHEIA_DISABLE_LEGACY_TRACING_IO_SEARCH: "true" | "false";
  };
  const originalLegacyIoSearchDisabled =
    mutableEnv.ALETHEIA_DISABLE_LEGACY_TRACING_IO_SEARCH;
  ...
  describe("traces.all", () => {
    it("ignores legacy full-text-only search when legacy IO search is disabled", async () => {
      mutableEnv.ALETHEIA_DISABLE_LEGACY_TRACING_IO_SEARCH = "true";
