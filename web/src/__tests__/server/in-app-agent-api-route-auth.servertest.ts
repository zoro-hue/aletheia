...
const aletheiaClientMocks = vi.hoisted(() => ({
  getAletheiaClient: vi.fn(() => ({})),
}));
...
vi.mock("@/src/features/natural-language-filters/server/utils", () => ({
  getAletheiaClient: aletheiaClientMocks.getAletheiaClient,
}));
...
describe("in-app agent public API route auth", () => {
  beforeEach(() => {
    ...
    aletheiaClientMocks.getAletheiaClient.mockReturnValue({});
  ...
  it("returns 429 when an in-app agent run exceeds the rate limit", async () => {
    ...
    const originalBedrockModel = env.ALETHEIA_AWS_BEDROCK_MODEL;
    const originalAiFeaturesPublicKey = env.ALETHEIA_AI_FEATURES_PUBLIC_KEY;
    const originalAiFeaturesSecretKey = env.ALETHEIA_AI_FEATURES_SECRET_KEY;

    (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "DEV";
    (env as any).ALETHEIA_AWS_BEDROCK_MODEL = "test-model";
    (env as any).ALETHEIA_AI_FEATURES_PUBLIC_KEY = "pk-lf-test";
    (env as any).ALETHEIA_AI_FEATURES_SECRET_KEY = "sk-lf-test";
    ...
    try {
      ...
      (env as any).ALETHEIA_AWS_BEDROCK_MODEL = originalBedrockModel;
      (env as any).ALETHEIA_AI_FEATURES_PUBLIC_KEY =
        originalAiFeaturesPublicKey;
      (env as any).ALETHEIA_AI_FEATURES_SECRET_KEY =
