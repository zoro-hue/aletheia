 ...
 * Aletheia Python SDK v3 uses, and what the bug reporter used), its `input` / `output`
...
import { type TracingSearchType } from "@aletheia/shared";
 ...
 * (Python's `json.dumps` default, used by the Aletheia Python SDK's EventSerializer) emits.
 */
...
describe("multilingual full-text search (issue #11538)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("reproduces issue #11538 through the real ingestion pipeline (POST /api/public/ingestion → worker → ClickHouse)", async () => {
      ...
      // The Aletheia Python SDK serialises I/O to a JSON *string* (with ensure_ascii=True) before
