...
vi.mock("@aletheia/shared/src/server", async () => {
  const originalModule = await vi.importActual("@aletheia/shared/src/server");
  return {
    ...originalModule,
    getTraceById: vi.fn(),
  };
...
import { getTraceById } from "@aletheia/shared/src/server";
...
describe("admin access webhook in tRPC authorization middleware", () => {
  ...
  const originalWebhook = env.ALETHEIA_ADMIN_ACCESS_WEBHOOK;
  const originalRegion = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;

  beforeAll(() => {
    (env as any).ALETHEIA_ADMIN_ACCESS_WEBHOOK = "https://example.com/hook";
  ...
  afterAll(() => {
    (env as any).ALETHEIA_ADMIN_ACCESS_WEBHOOK = originalWebhook;
