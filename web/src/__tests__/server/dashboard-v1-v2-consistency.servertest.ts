...
import { type QueryType } from "@aletheia/shared/query";
import {
...
import { getGenerationLikeTypes, type FilterCondition } from "@aletheia/shared";
...
describe("dashboard v1 vs v2 consistency", () => {
  ...
  beforeAll(async () => {
    if (env.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN !== "true") return;
  ...
  // Seeder data uses "default" and "aletheia-prompt-experiment" for scores
  // (see data-generators.ts lines 240, 268, 600 and clickhouse-builder.ts).
  const SCORE_ENVIRONMENTS =
    DATA_MODE === "seeder"
      ? ["default", "aletheia-prompt-experiment"]
      : ["default", "staging"];
