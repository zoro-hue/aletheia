...
import { Role } from "@aletheia/shared";
...
describe("verifiedDomainRouter.create", () => {
  it("creates a pending row and returns DNS record details", async () => {
    ...
    expect(result.recordHost).toBe("_aletheia-verification");
    expect(result.recordValue).toMatch(/^aletheia-verify=/);
...
describe("verifiedDomainRouter.list", () => {
  it("returns rows scoped to the caller's org", async () => {
    ...
    rows.forEach((r) => {
      expect(r.recordHost).toBe("_aletheia-verification");
      expect(r.recordValue).toMatch(/^aletheia-verify=/);
    });
  });
});
...
describe("verifiedDomainRouter.verify", () => {
  ...
  it("returns PRECONDITION_FAILED when no TXT record matches", async () => {
    ...
    resolveTxtMock.mockResolvedValueOnce([["aletheia-verify=wrong"]]);
