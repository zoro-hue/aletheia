...
import { ScoreConfigDataType } from "@aletheia/shared";
import {
  createMcpTestSetup,
  createPromptInDb,
...
const maybeEventsTable =
  env.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN === "true"
    ? describe
    : describe.skip;
const maybeEventsTableIt =
  env.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN === "true" ? it : it.skip;
  ...
  describe("getPromptUnresolved tool", () => {
    ...
    it("should fetch latest prompt without resolving dependencies by default", async () => {
      ...
      const rawPromptContent =
        "You are a helpful assistant. @@@aletheiaPrompt:name=base-instructions|label=production@@@";
      ...
      expect(result.prompt).toContain(
        "@@@aletheiaPrompt:name=base-instructions|label=production@@@",
      );
    });
    ...
    it("should fetch prompt by name and specific label without resolution", async () => {
      ...
      await createPromptInDb({
        name: promptName,
        prompt: "Version 1 @@@aletheiaPrompt:name=helper|label=staging@@@",
        projectId,
        labels: ["staging"],
        version: 1,
      });
      ...
      await createPromptInDb({
        name: promptName,
        prompt: "Version 2 @@@aletheiaPrompt:name=helper|label=production@@@",
        projectId,
        labels: ["production"],
        version: 2,
      });
      ...
      expect(result.prompt).toBe(
        "Version 1 @@@aletheiaPrompt:name=helper|label=staging@@@",
      );
    });
    ...
    it("should fetch prompt by name and specific version without resolution", async () => {
      ...
      await createPromptInDb({
        name: promptName,
        prompt: "V1 content @@@aletheiaPrompt:name=dep|label=v1@@@",
        projectId,
        labels: ["staging"],
        version: 1,
      });
      ...
      await createPromptInDb({
        name: promptName,
        prompt: "V2 content @@@aletheiaPrompt:name=dep|label=v2@@@",
        projectId,
        labels: ["production"],
        version: 2,
      });
      ...
      expect(result.version).toBe(1);
      expect(result.prompt).toBe(
        "V1 content @@@aletheiaPrompt:name=dep|label=v1@@@",
      );
    });
    ...
    it("should return raw chat prompt without resolving dependencies", async () => {
      ...
      const chatMessages = [
        {
          role: "system",
          content:
            "You are helpful @@@aletheiaPrompt:name=system-base|label=production@@@",
        },
        {
          role: "user",
          content: "@@@aletheiaPrompt:name=user-template|label=production@@@",
        },
      ];
      ...
      expect(result.prompt[0].content).toContain(
        "@@@aletheiaPrompt:name=system-base|label=production@@@",
      );
    });
  });
});
