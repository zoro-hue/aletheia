...
describe("OTel Resource Span Mapping", () => {
  describe("Aletheia OTEL SDK spans", () => {
    ...
    it("should convert LF-OTEL spans to LF-events", async () => {
      const aletheiaOtelSpans = [
        {
          resource: {
            attributes: [
              ...
              {
                key: "aletheia.environment",
                value: { stringValue: "production" },
              },
              { key: "aletheia.release", value: { stringValue: "0.0.1" } },
              {
                key: "service.name",
              ...
              spans: [
                {
                  ...
                  attributes: [
                    {
                      key: "aletheia.observation.type",
                      value: { stringValue: "generation" },
                    },
                    {
                      key: "aletheia.observation.level",
                      value: { stringValue: "WARNING" },
                    },
                    {
                      key: "aletheia.observation.status_message",
                      value: { stringValue: "nothing to report" },
                    },
                    {
                      key: "aletheia.observation.input",
                      value: {
                        stringValue: '[{"role": "user", "content": "hello"}]',
                      },
                    },
                    {
                      key: "aletheia.observation.output",
                      value: {
                        stringValue:
                    ...
                    {
                      key: "aletheia.observation.model.name",
                      value: { stringValue: "gpt-4o" },
                    },
                    {
                      key: "aletheia.observation.prompt.name",
                      value: { stringValue: "my-prompt" },
                    },
                    {
                      key: "aletheia.observation.prompt.version",
                      value: { intValue: { low: 1, high: 0, unsigned: false } },
                    },
                    {
                      key: "aletheia.observation.usage_details",
                      value: {
                        stringValue:
                    ...
                    {
                      key: "aletheia.observation.cost_details",
                      value: {
                        stringValue:
                    ...
                    {
                      key: "aletheia.trace.tags",
                      value: {
                        arrayValue: { values: [{ stringValue: "tag2" }] },
                      },
                    },
                    {
                      key: "aletheia.trace.public",
                      value: { boolValue: true },
                    },
                    {
                      key: "aletheia.trace.metadata.trace-key1",
                      value: { stringValue: '"value1"' },
                    },
                    {
                      key: "aletheia.trace.metadata.trace-key2",
                      value: { stringValue: '"value2"' },
                    },
                  ],
                  status: {},
                },
                {
                  ...
                  attributes: [
                    ...
                    {
                      key: "aletheia.trace.metadata.trace-key0",
                      value: { stringValue: '"value1"' },
                    },
                  ],
                  status: {},
      ...
      const defaultMetadata = {
        resourceAttributes: {
          "aletheia.environment": "production",
          "aletheia.release": "0.0.1",
  ...
  describe("Vendor Spans", () => {
    it("should convert an OpenLit OTel Span to Aletheia Events", async () => {
      // Setup
      const resourceSpan = {
        resource: {
          attributes: [
      ...
      // When
      const aletheiaEvents = await convertOtelSpanToIngestionEvent(
        resourceSpan,
        new Set(),
      );
      ...
      const parsedEvents = aletheiaEvents.map((event) => schema.parse(event));
      expect(parsedEvents).toHaveLength(2);
    });

    it("should convert a TraceLoop OTel Span to Aletheia Events", async () => {
      // Setup
      const resourceSpan = {
        resource: {
          attributes: [
    ...
    it("LFE-5171: should convert a Semantic Kernel 1.55+ OTel Span with new event-based semantic conventions to Aletheia Events", async () => {
    ...
    it("should convert a Vercel AI SDK embedding span to Aletheia embedding-create event", async () => {
      const resourceSpan = {
        scopeSpans: [
          {
            scope: { name: "ai" },
    ...
    it("should convert a Genkit OTel span to Aletheia generation-create event", async () => {
  ...
  describe("Property Mapping", () => {
    ...
    it.each([
      ...
      (
        ...
        const attributes = otelAttributeValue
          ? [
              {
                key: "aletheia.internal.is_app_root",
    ...
    it("should interpret an empty buffer as an unset parentSpanId", async () => {
      ...
      expect(aletheiaEvents).toHaveLength(2);
    });
    ...
    it.each([
      ...
      async (spanKind, expectedEventType) => {
        ...
        // Then
        expect(aletheiaEvents).toHaveLength(2); // Should create trace + observation
        expect(
          aletheiaEvents.some((event) => event.type === expectedEventType),
        ).toBe(true);

        // Verify the observation has the correct type
        const observationEvent = aletheiaEvents.find(
          (event) =>
    ...
    it("should normalize current Pydantic AI cache usage fields into Aletheia usage details", async () => {
    ...
    it("should extract OpenInference llm.token_count.prompt_details.cache_read/cache_write into input_cached_tokens / input_cache_creation", async () => {
      ...
      // llm.token_count.prompt_details.cache_write. Aletheia must recognize these
    ...
    it("should prioritize OpenInference over OTel GenAI and model detection", async () => {
      ...
      expect(aletheiaEvents.some((event) => event.type === "tool-create")).toBe(
        true,
      );
      expect(
        aletheiaEvents.some((event) => event.type === "generation-create"),
      ).toBe(false);
    });
    ...
    it("should prioritize OTel GenAI over model-based detection", async () => {
      ...
      expect(
        aletheiaEvents.some((event) => event.type === "embedding-create"),
      ).toBe(true);
      expect(
        aletheiaEvents.some((event) => event.type === "generation-create"),
      ).toBe(false);
    ...
    it("should trust Aletheia type over OpenInference or model detection", async () => {
      const resourceSpan = {
        scopeSpans: [
          {
            spans: [
              {
                ...defaultSpanProps,
                attributes: [
                  // Explicit Aletheia type (should always win)
                  {
                    key: "aletheia.observation.type",
                    value: { stringValue: "span" },
                  },
      ...
      // Then
      expect(aletheiaEvents).toHaveLength(2);
      // Explicit Aletheia type should always win over inferred types
      expect(aletheiaEvents.some((event) => event.type === "span-create")).toBe(
        true,
      );
      expect(
        aletheiaEvents.some((event) => event.type === "agent-create"),
      ).toBe(false);
      expect(
        aletheiaEvents.some((event) => event.type === "generation-create"),
      ).toBe(false);
    ...
    it("should trust OpenInference over model detection but keep model attributes", async () => {
      ...
      expect(
        aletheiaEvents.some((event) => event.type === "retriever-create"),
      ).toBe(true);
      expect(
        aletheiaEvents.some((event) => event.type === "generation-create"),
      ).toBe(false);
      ...
      const retrieverEvent = aletheiaEvents.find(
    ...
    it("should use logfire.msg as span name", async () => {
      ...
      // Then
      expect(aletheiaEvents[0].body.name).toBe("right name");
      expect(aletheiaEvents[1].body.name).toBe("right name");
    });
    ...
    it.each([
      [
        ...
        {
          entity: "observation",
          otelAttributeKey: "aletheia.observation.completion_start_time",
          otelAttributeValue: {
      ...
      [
        "should extract environment on trace for aletheia.environment",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.environment",
      ...
      [
        "should extract promptName on observation from aletheia.prompt.name",
        {
          entity: "observation",
          otelAttributeKey: "aletheia.prompt.name",
          otelAttributeValue: { stringValue: "test" },
      ...
      [
        "should extract public on trace from aletheia.public",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.public",
      ...
      [
        "should extract userId on trace from aletheia.user.id",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.user.id",
          otelAttributeValue: { stringValue: "user-1" },
          entityAttributeKey: "userId",
          entityAttributeValue: "user-1",
        },
      ...
      [
        "should extract sessionId on trace from aletheia.session.id",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.session.id",
          otelAttributeValue: { stringValue: "session-1" },
          entityAttributeKey: "sessionId",
          entityAttributeValue: "session-1",
        },
      ...
      [
        "should map aletheia.metadata string to top-level metadata for trace",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.metadata",
          otelAttributeValue: {
      ...
      [
        "should map aletheia.metadata string to top-level metadata for observation",
        {
          entity: "observation",
          otelAttributeKey: "aletheia.metadata",
          otelAttributeValue: {
      ...
      [
        "should extract metadata from aletheia.metadata.* keys for trace",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.metadata.user_type",
          otelAttributeValue: {
            stringValue: "premium",
          },
      ...
      [
        "should extract metadata from aletheia.metadata.* keys for observation",
        {
          entity: "observation",
          otelAttributeKey: "aletheia.metadata.user_type",
          otelAttributeValue: {
      ...
      [
        "should extract tags from single string from aletheia.tags to trace",
        {
          entity: "trace",
          otelAttributeKey: "aletheia.tags",
          otelAttributeValue: {
            stringValue: "2",
          },
      ...
      async (
        ...
        const entity: { body: Record<string, any> } =
          spec.entity === "trace" ? aletheiaEvents[0] : aletheiaEvents[1];
        expect(
    ...
    it("should convert OTLP/JSON string-encoded int resource attributes without producing NaN", async () => {
      ...
      const resourceSpan = {
        resource: {
          attributes: [
            { key: "service.name", value: { stringValue: "aletheia-demo" } },
      ...
      const resourceAttributes =
        aletheiaEvents[1].body.metadata.resourceAttributes;
      expect(resourceAttributes["process.pid"]).toBe(7);
      expect(resourceAttributes["service.name"]).toBe("aletheia-demo");
    ...
    it.each([
      ...
      [
        ...
        {
          entity: "trace",
          otelResourceAttributeKey: "aletheia.environment",
      ...
      [
        ...
        {
          entity: "observation",
          otelResourceAttributeKey: "aletheia.metadata",
      ...
      [
        "should extract metadata from aletheia.metadata.* resource attributes",
        {
          entity: "observation",
          otelResourceAttributeKey: "aletheia.metadata.server_name",
          otelResourceAttributeValue: {
            ...
            spans: [
              {
                ...
                attributes: [
                  ...
                  {
                    key: "aletheia.prompt.name",
                    value: { stringValue: "my-prompt" },
                  },
                  {
                    key: "aletheia.prompt.version",
                    value: { intValue: { low: 2, high: 0, unsigned: false } },
                  },
                  {
                    key: "aletheia.observation.level",
  ...
  describe("Trace seen logic", () => {
              ...
              spans: [
                {
                  ...
                  attributes: [
                    {
                      key: "aletheia.trace.metadata.aletheia_user_id",
                      value: { stringValue: "user-123" },
                    },
                    {
                      key: "aletheia.trace.metadata.aletheia_session_id",
                      value: { stringValue: "session-456" },
                    },
                    {
                      key: "aletheia.trace.metadata.aletheia_tags",
              ...
              spans: [
                {
                  ...
                  attributes: [
                    {
                      key: "aletheia.observation.metadata.aletheia_user_id",
                      value: { stringValue: "user-789" },
                    },
                    {
                      key: "aletheia.observation.metadata.aletheia_session_id",
                      value: { stringValue: "session-abc" },
                    },
                    {
                      key: "aletheia.observation.metadata.aletheia_tags",
    ...
    it("should prioritize aletheia.session.id over gen_ai.conversation.id when both are present", async () => {
      const otelSpans = [
        {
          resource: {
            attributes: [
              ...
              spans: [
                {
                  ...
                  attributes: [
                    {
                      key: "aletheia.session.id",
                      value: { stringValue: "aletheia-session-123" },
                    },
                    {
      ...
      const traceEvent = events.find((e) => e.type === "trace-create");
      expect(traceEvent).toBeDefined();
      expect(traceEvent.body.sessionId).toBe("aletheia-session-123");
    });
    ...
    it("should override the observation type if it is declared as 'span' but holds generation-like attributes for python-sdk <= 3.3.0", async () => {
      // Issue: https://github.com/aletheia/aletheia/issues/8682
      const otelSpans = [
        {
          resource: {
            attributes: [
  ...
  describe("GenAI usage normalization", () => {
    it("should normalize official gen_ai cache usage into Aletheia canonical keys", async () => {
      const traceId = "abcdef1234567890abcdef1234567892";
  ...
  describe("Input/Output attribute filtering from metadata", () => {
    it("should filter Aletheia SDK trace input/output attributes from trace metadata", async () => {
      const traceId = "abcdef1234567890abcdef1234567890";
      const rootSpanId = "1234567890abcdef";
      ...
      expect(
        traceEvent?.body.metadata?.attributes?.["aletheia.trace.input"],
      ).toBeUndefined();
      expect(
        traceEvent?.body.metadata?.attributes?.["aletheia.trace.output"],
      ).toBeUndefined();
