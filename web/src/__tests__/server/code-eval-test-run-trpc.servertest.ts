...
import type * as SharedEnvModule from "@aletheia/shared/src/env";

vi.hoisted(() => {
  process.env.ALETHEIA_CODE_EVAL_DISPATCHER = "insecure-local";
});

vi.mock("@aletheia/shared/src/env", async (importOriginal) => {
  ...
  return {
    ...
    env: {
      ...actual.env,
      ALETHEIA_CODE_EVAL_DISPATCHER: "insecure-local",
...
import { EvalTargetObject } from "@aletheia/shared";
...
maybe("evals.testRunCodeEval", () => {
  ...
  it("runs against legacy observations when events table evals are disabled", async () => {
    ...
    const mutableEnv = env as unknown as {
      ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN: "true" | "false";
    };
    const originalEventsTableFlagsFlag =
      mutableEnv.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN;

    try {
      mutableEnv.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN = "false";
  ...
  it("persists an internal trace for the test run", async () => {
    ...
    expect(trace?.environment).toBe("aletheia-code-eval");
