...
import { env as sharedEnv } from "@aletheia/shared/src/env";
...
vi.mock("@aletheia/shared/src/server", async () => {
  const actual = await vi.importActual("@aletheia/shared/src/server");
  return {
    ...actual,
    BlobStorageIntegrationProcessingQueue: {
      getInstance: vi.fn(),
...
describe("Blob Storage Integration tRPC Router", () => {
  ...
  describe("endpoint validation", () => {
    const originalAllowedIps =
      sharedEnv.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS;
    const originalSharedCloudRegion =
      sharedEnv.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;

    beforeEach(() => {
      sharedEnv.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = undefined;
    });

    afterEach(() => {
      sharedEnv.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS =
        originalAllowedIps;
      sharedEnv.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalSharedCloudRegion;
    });
    ...
    it("allows endpoints when their IP is whitelisted", async () => {
      ...
      const originalValidationCloudRegion =
        validationEnv.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
      const originalValidationAllowedIps =
        validationEnv.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS;

      try {
        validationEnv.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = undefined;
        validationEnv.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS = [
          "127.0.0.1",
        ];
  ...
  describe("legacy blob export source cutoff gate", () => {
    ...
    it("Cloud + pre-cutoff project + pre-cutoff legacy row + legacy source → allow", async () => {
      const originalRegion = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "us";
      try {
        const { caller, project } = await prepare();
        await prisma.project.update({
          where: { id: project.id },
        ...
        (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalRegion;
      }
    });
    ...
    it("self-hosted + post-cutoff project + legacy source → allow (bypass)", async () => {
      const originalRegion = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = undefined;
      try {
        const { caller, project } = await prepare();
        await prisma.project.update({
          where: { id: project.id },
  ...
  describe("get: isEnrichedExportAvailable flag", () => {
    const originalRegion = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
    const originalV4Preview = env.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN;

    afterEach(() => {
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalRegion;
      (env as any).ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN =
        originalV4Preview;
    });

    it("returns true for Cloud deployments regardless of V4 flag", async () => {
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "us";
      (env as any).ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN = "false";
      const { caller, project } = await prepare();
      const result = await caller.blobStorageIntegration.get({
        projectId: project.id,
      });
    ...
    it("returns true for self-hosted with V4 preview opt-in enabled", async () => {
      (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = undefined;
      (env as any).ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN = "true";
      const { caller, project } = await prepare();
      const result = await caller.blobStorageIntegration.get({
        projectId: project.id,
      });
