...
describe("/api/public/v2/observations API Endpoint", () => {
  ...
  maybe("subquery rewrite", () => {
    const originalFlag = sharedEnv.ALETHEIA_OBSERVATIONS_V2_SUBQUERY_REWRITE;

    const setRewrite = (value: "true" | "false") => {
      (
        sharedEnv as { ALETHEIA_OBSERVATIONS_V2_SUBQUERY_REWRITE: string }
      ).ALETHEIA_OBSERVATIONS_V2_SUBQUERY_REWRITE = value;
    };
