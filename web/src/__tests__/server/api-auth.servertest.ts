...
import { Prisma, type PrismaClient, prisma } from "@aletheia/shared/src/db";
...
describe("Authenticate API calls", () => {
  ...
  describe("validates with redis", () => {
    ...
    it("ttl should be increased when reading from redis", async () => {
      ...
      expect(ttl).toBeGreaterThan(env.ALETHEIA_CACHE_API_KEY_TTL_SECONDS - 2);

      await redis.expire(redisKey, 10);
      const shortenedTtl = await getRedisTtl(redis, redisKey);
      expect(shortenedTtl).toBeGreaterThan(0);
      expect(shortenedTtl).toBeLessThan(env.ALETHEIA_CACHE_API_KEY_TTL_SECONDS);
      ...
      expect(ttl2).toBeGreaterThan(env.ALETHEIA_CACHE_API_KEY_TTL_SECONDS - 2);
    });
