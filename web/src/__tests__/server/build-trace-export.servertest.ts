import { AletheiaNotFoundError, UnauthorizedError } from "@aletheia/shared";
...
describe("buildTraceExport", () => {
  const originalObservationLimit =
    env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES;
  ...
  it("reemits the observation payload limit error as a download-safe error", async () => {
    ...
    env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES = 100;
    mockGetObservationsForTraceFromEventsTable.mockResolvedValue({
      observations: [
        makeObservation({
          input: "x".repeat(60),
