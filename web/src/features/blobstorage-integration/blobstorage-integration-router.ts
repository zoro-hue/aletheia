...
export const blobStorageIntegrationRouter = createTRPCRouter({
  get: protectedProjectProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        const isCloud = Boolean(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION);
  ...
  validate: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const testFileName = `${prefix || ""}aletheia-validation-test-${timestamp}.txt`;
        const testContent = `Aletheia blob storage validation test
