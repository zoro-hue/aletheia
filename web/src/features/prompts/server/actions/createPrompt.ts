...
import { type PrismaClient } from "@aletheia/shared/src/db";
...
const rewriteDuplicatedPromptContent = ({
  ...
  for (const dep of parsePromptDependencyTags(prompt)) {
    ...
    const currentTag =
      dep.type === "version"
        ? `@@@aletheiaPrompt:name=${dep.name}|version=${dep.version}@@@`
        : `@@@aletheiaPrompt:name=${dep.name}|label=${dep.label}@@@`;

    const rewrittenTag =
      dep.type === "version"
        ? `@@@aletheiaPrompt:name=${duplicatedDependencyName}|version=${isSingleVersion ? 1 : dep.version}@@@`
        : `@@@aletheiaPrompt:name=${duplicatedDependencyName}|label=${dep.label}@@@`;
