...
import { type Prompt, Prisma } from "@aletheia/shared/src/db";
import {
  createPrompt,
  duplicatePrompt,
  duplicateFolder,
...
import { orderBy, singleFilter } from "@aletheia/shared";
import {
  orderByToPrismaSql,
  PromptService,
  redis,
...
export const promptRouter = createTRPCRouter({
  ...
  count: protectedProjectProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return {
        totalCount: count && count[0] ? Number(count[0].totalCount) : 0,
      };
    }),
