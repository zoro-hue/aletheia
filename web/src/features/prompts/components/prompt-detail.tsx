...
const getJsCode = (
  ...
  version: number,
  labels: string[],
) => `import { AletheiaClient } from "@aletheia/client";

// Initialize the Aletheia client
const aletheia = new AletheiaClient();

// Get production prompt
const prompt = await aletheia.prompt.get("${name}");

// Get by label
// You can use as many labels as you'd like to identify different deployment targets
${labels.length > 0 ? labels.map((label) => `const prompt = await aletheia.prompt.get("${name}", { label: "${label}" })`).join("\n") : ""}

// Get by version number, usually not recommended as it requires code changes to deploy new prompt versions
await aletheia.prompt.get("${name}", { version: ${version} })
`;
  ...
  return (
    <Page
      headerProps={{
        ...
        help: {
          description:
            "You can use this prompt within your application through the Aletheia SDKs and integrations. Refer to the documentation for more information.",
          href: "https://aletheia.com/docs/prompts",
        },
        breadcrumb: [
          {
            name: "Prompts",
