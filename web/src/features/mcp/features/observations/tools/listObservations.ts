...
import { getObservationsV2FromEventsTableForPublicApi } from "@aletheia/shared/src/server";
import { z } from "zod";
import {
...
const ListObservationsBaseSchema = z.object({
  ...
  // see: https://linear.app/aletheia/issue/LFE-9852/bug-observations-api-accepts-multiple-environment-params-but-ignores
  environment: z
    .string()
    .optional()
    .describe(
...
export const [listObservationsTool, handleListObservations] = defineTool({
  ...
  description: [
    "Find and review observations in the current Aletheia project, such as generations, spans, events, agent steps, and tool calls.",
