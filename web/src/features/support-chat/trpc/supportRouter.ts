      ...
      if (!env.PYLON_API_KEY) {
        ...
        try {
          ...
          const pylonIssue = await createPylonIssue({
            ...
            customFields: [
              ...(input.url
                ? [{ slug: "aletheia_page_url", value: input.url }]
                : []),
              {
                slug: "question_type",
                values: [mapMessageTypeToPylonQuestionType(input.messageType)],
              },
              { slug: "aletheia_topic", value: input.topic },
              ...(input.integrationType
                ? [
                    {
                      slug: "aletheia_integration_type",
                      value: input.integrationType,
                    },
                  ]
                : []),
              { slug: "aletheia_metadata", value: pylonMetadata },
              {
                slug: "case_severity",
                value: caseSeverity,
              },
