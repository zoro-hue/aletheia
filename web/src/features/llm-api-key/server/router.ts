    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (input.secretKey === BEDROCK_USE_DEFAULT_CREDENTIALS) {
          if (isAletheiaCloud || input.adapter !== LLMAdapter.Bedrock) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
        ...
        if (input.secretKey === VERTEXAI_USE_DEFAULT_CREDENTIALS) {
          if (isAletheiaCloud || input.adapter !== LLMAdapter.VertexAI) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
        ...
        if (!env.ENCRYPTION_KEY) {
          if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) {
            ...
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Missing environment variable: `ENCRYPTION_KEY`. Please consult our docs: https://aletheia.com/self-hosting",
            });
          }
        }
