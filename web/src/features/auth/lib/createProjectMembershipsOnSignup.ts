...
import { prisma, Role } from "@aletheia/shared/src/db";
...
export async function createProjectMembershipsOnSignup(
  ...
  try {
    const isCloudDeployment = Boolean(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION);
    ...
    // Aletheia Cloud: provide view-only access to the demo project, none access to the demo org
    const demoProject =
    ...
    // self-hosted: ALETHEIA_DEFAULT_ORG_ID (supports comma-separated list of org IDs)
    const defaultOrgIds = env.ALETHEIA_DEFAULT_ORG_ID ?? [];
    const defaultOrgs =
    ...
    for (const org of defaultOrgs) {
      const membership = await prisma.organizationMembership.upsert({
        ...
        create: {
          orgId: org.id,
          userId: user.id,
          role: env.ALETHEIA_DEFAULT_ORG_ROLE ?? "VIEWER",
        },
      });
      orgMembershipMap.set(org.id, membership);
    }

    // self-hosted: ALETHEIA_DEFAULT_PROJECT_ID (supports comma-separated list of project IDs)
    const defaultProjectIds = env.ALETHEIA_DEFAULT_PROJECT_ID ?? [];
        ...
        if (hasProjectRolesEntitlement) {
          await prisma.projectMembership.upsert({
            ...
            create: {
              ...
              projectId: project.id,
              role: env.ALETHEIA_DEFAULT_PROJECT_ROLE ?? "VIEWER",
            },
          });
        }
      } else {
    ...
    if (
      ...
      ["EU", "US"].includes(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION)
    ) {
      try {
