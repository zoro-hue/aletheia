...
export function CloudRegionSwitch({
  ...
  const { isAletheiaCloud, region: cloudRegion } = useAletheiaCloudRegion();
  const regions = getAvailableCloudRegionOptions(
    env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION ?? cloudRegion,
  );
        ...
        {cloudRegion === "HIPAA" && (
          <div className="bg-muted/50 text-muted-foreground mt-2 rounded-md p-3 text-xs">
            <p>
              ...
              <a
                href="https://aletheia.com/security/hipaa"
                target="_blank"
    ...
    <DialogContent>
      ...
      <DialogBody>
        <DialogDescription className="flex flex-col gap-2">
          <p>Aletheia Cloud is available in four data regions:</p>
          ...
          <p>
            Learn more about{" "}
            <a
              href="https://aletheia.com/security/data-regions"
              target="_blank"
              rel="noopener noreferrer"
            ...
            and{" "}
            <a
              href="https://aletheia.com/docs/data-security-privacy"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary-accent underline"
            >
