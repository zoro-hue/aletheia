  ...
  const transformMapping = (
    ...
    return currentMapping.map((field) => {
      ...
      if (isObservationBased) {
        // Placeholder aletheiaObject (stripped in onSubmit)
        return {
          templateVariable: field.templateVariable,
          selectedColumnId,
          jsonSelector: field.jsonSelector,
          aletheiaObject: "event" as AletheiaObject,
          objectName: null,
        };
      }

      // Proper aletheiaObject for trace/dataset
      return {
        ...field,
        selectedColumnId,
        aletheiaObject: (isTraceTarget(targetObject)
          ? "trace"
          : "dataset_item") as AletheiaObject,
