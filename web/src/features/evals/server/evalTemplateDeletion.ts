...
import { type ApiAccessScope } from "@aletheia/shared/src/server";
...
export async function findEvalTemplateFamilyUsage({
  ...
  if (!template || (template.projectId && template.projectId !== projectId)) {
    throw new AletheiaNotFoundError("Evaluator not found");
  }
...
export async function deleteEvalTemplateFamily({
  ...
  if (!template.projectId) {
    throw new ForbiddenError("Aletheia-managed evaluators cannot be deleted");
  }
