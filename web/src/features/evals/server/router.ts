...
import { traceException } from "@aletheia/shared/src/server";
...
export const evalRouter = createTRPCRouter({
  ...
  globalJobConfigs: protectedProjectProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return env.ALETHEIA_MAX_HISTORIC_EVAL_CREATION_LIMIT;
    }),
  ...
  createTemplate: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
       ...
       *
       * Option 2: Clone a aletheia managed template
       * - Find the aletheia managed template
       * - Clone the aletheia managed template by creating a new project-level template from the cloned aletheia managed template
       */
      ...
      return ctx.prisma.$transaction(async (tx) => {
        ...
        // Create a new project-level template either by cloning a aletheia managed template or by creating a new project-level template
        ...
        if (input.referencedEvaluators === EvalReferencedEvaluators.UPDATE) {
           ...
           *
           * - Find the aletheia managed template
           * - Create a new project-level template from the cloned aletheia managed template
           * - Update all job configs that had referenced the aletheia managed template to now reference the cloned project-level template
           */
          if (input.cloneSourceId) {
            // find the aletheia managed template to clone
            ...
            if (!cloneSourceTemplate) {
              throw new TRPCError({
                code: "NOT_FOUND",
                message: "Aletheia managed template not found",
              });
            }
            ...
            // find all versions of the aletheia managed template
            ...
            if (Boolean(cloneSourceTemplateList.length)) {
              // update all job configs that had referenced any version of the aletheia managed template to now reference the cloned user managed template
