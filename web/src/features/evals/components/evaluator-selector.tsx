...
export function EvaluatorSelector({
  ...
  // Group templates by name and whether they are managed by Aletheia
  const groupedTemplates = visibleEvalTemplates.reduce(
    (acc, template) => {
      const group = template.projectId ? "custom" : "aletheia";
    ...
    {
      aletheia: {} as Record<string, EvalTemplate[]>,
  ...
  Object.values(groupedTemplates.aletheia).forEach(sortByCreatedAt);

  // Filter templates based on search
  const filteredTemplates = {
    aletheia: Object.entries(groupedTemplates.aletheia)
  ...
  const hasResults =
    filteredTemplates.aletheia.length > 0 ||
  ...
  return (
    <InputCommand className="flex h-full flex-col border-none">
      ...
      <InputCommandList className="max-h-full flex-1 overflow-y-auto">
        ...
        {filteredTemplates.aletheia.length > 0 && (
          <>
            <InputCommandGroup heading="Aletheia managed evaluators">
              {filteredTemplates.aletheia.map(([name, templateData]) => {
                const latestVersion = templateData[templateData.length - 1];
                const isInvalid = isTemplateInvalid(latestVersion);
