...
async function getAuthorizedTrace(params: {
  ...
  if (!clickhouseTrace) {
    throw new AletheiaNotFoundError("Trace not found");
  }
    ...
    for (const observation of observationRecords) {
      ...
      if (payloadSize >= env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES) {
        throw new TraceDownloadTooLargeError(
          `Observations in trace are too large: ${(payloadSize / 1e6).toFixed(2)}MB exceeds limit of ${(env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES / 1e6).toFixed(2)}MB`,
        );
      }
    }
  }
