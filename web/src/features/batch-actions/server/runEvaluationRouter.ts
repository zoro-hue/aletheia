  ...
  create: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (observationCount > env.ALETHEIA_MAX_HISTORIC_EVAL_CREATION_LIMIT) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Too many observations selected. Maximum allowed is ${env.ALETHEIA_MAX_HISTORIC_EVAL_CREATION_LIMIT}, but ${observationCount} observations match your filters. Please refine your filters to reduce the count.`,
          });
        }
