...
const getDatasetRunRecordOrThrow = async ({
  ...
  if (!datasetRuns[0]) {
    throw new AletheiaNotFoundError("Dataset run not found");
  }
...
export const getDatasetItemForApi = async ({
  ...
  if (!datasetItem) {
    throw new AletheiaNotFoundError("Dataset item not found");
  }
...
export const createDatasetRunItemForApi = async ({
  ...
  if (!traceId && observationId) {
    ...
    if (!observation) {
      throw new AletheiaNotFoundError("Observation not found");
    }
