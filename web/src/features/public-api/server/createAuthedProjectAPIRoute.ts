...
import { type RateLimitResource } from "@aletheia/shared";
import { RateLimitService } from "@/src/features/public-api/server/RateLimitService";
import { type RateLimitUpgradePath } from "@/src/features/public-api/server/rateLimitUpgradePaths";
import { contextWithAletheiaProps } from "@aletheia/shared/src/server";
...
> = {
   ...
   * - x-aletheia-admin-api-key: <ADMIN_API_KEY> (must match exactly for redundancy)
   * - x-aletheia-project-id: <project-id> (target project)
   *
   * This authentication method is ONLY available when NEXT_PUBLIC_ALETHEIA_CLOUD_REGION is not set (self-hosted).
   *
   * @default false
   */
  ...
  /**
   * When true, this route returns 404 if ALETHEIA_MIGRATION_V4_WRITE_MODE is
 ...
 * 2. x-aletheia-admin-api-key header must match ADMIN_API_KEY env var exactly (for redundancy)
 * 3. x-aletheia-project-id header must be present and specify a valid project ID
 * 4. NEXT_PUBLIC_ALETHEIA_CLOUD_REGION must NOT be set (self-hosted instances only)
 *
...
> {
  ...
  const adminApiKeyHeader = req.headers["x-aletheia-admin-api-key"];
  const projectIdHeader = req.headers["x-aletheia-project-id"];
  ...
  // Verify this is a self-hosted instance (not Aletheia Cloud)
  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) {
    throw {
      status: 403,
      message: "Admin API key auth is not available on Aletheia Cloud",
    };
  }
  ...
  if (!projectIdHeader || typeof projectIdHeader !== "string") {
    throw {
      status: 400,
      message:
        "x-aletheia-project-id header is required for admin API key authentication",
    };
  }
...
>(
  ...
  return async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (
      ...
      env.ALETHEIA_MIGRATION_V4_WRITE_MODE === "events_only"
    ) {
      res.status(404).json({
        message:
          "This endpoint is not available on deployments running in Aletheia v4 events_only mode. Learn more about Aletheia v4 at: https://aletheia.com/docs/v4",
      });
      return;
    }
