...
import { API_KEY_NON_EXISTENT } from "@aletheia/shared/src/server";
import { type z } from "zod";
import { CloudConfigSchema, isPlan } from "@aletheia/shared";
...
export class ApiAuthService {
  ...
  private async fetchApiKeyAndAddToRedis(hash: string) {
    ...
    if (redisApiKey === API_KEY_NON_EXISTENT) {
      recordIncrement("aletheia.api_key.cache_hit", 1);
      throw new Error("Invalid credentials");
    }
    ...
    recordIncrement("aletheia.api_key.cache_miss", 1);
  ...
  private async addApiKeyToRedis(
    ...
    if (!this.redis || env.ALETHEIA_CACHE_API_KEY_ENABLED !== "true") {
      return;
    }
    ...
    try {
      await this.redis.set(
        ...
        "EX",
        env.ALETHEIA_CACHE_API_KEY_TTL_SECONDS, // redis API is in seconds
      );
    } catch (error: unknown) {
