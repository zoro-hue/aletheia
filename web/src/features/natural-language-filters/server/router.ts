...
import { BEDROCK_USE_DEFAULT_CREDENTIALS } from "@aletheia/shared";
  ...
  createCompletion: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (!env.ALETHEIA_AWS_BEDROCK_MODEL) {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Bedrock environment variables not configured. Please set ALETHEIA_AWS_BEDROCK_* variables.",
          });
        }

        if (
          !env.ALETHEIA_AI_FEATURES_PUBLIC_KEY ||
          !env.ALETHEIA_AI_FEATURES_SECRET_KEY
        ) {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Aletheia AI filters environment variables not configured. Please set ALETHEIA_AI_FEATURES_PUBLIC_KEY and ALETHEIA_AI_FEATURES_SECRET_KEY variables.",
          });
        }

        const client = getAletheiaClient(
          env.ALETHEIA_AI_FEATURES_PUBLIC_KEY,
          env.ALETHEIA_AI_FEATURES_SECRET_KEY,
          env.ALETHEIA_AI_FEATURES_HOST,
          false,
        );
        ...
        const targetProjectId = aiTelemetryEnabled
          ? env.ALETHEIA_AI_FEATURES_PROJECT_ID
          : undefined;

        if (aiTelemetryEnabled && !targetProjectId) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Aletheia AI Features not configured.",
          });
        }
        ...
        const traceSinkParams = targetProjectId
          ? {
              ...
              metadata: {
                aletheia_ai_feature: "natural-language-filter",
                aletheia_user_id: ctx.session.user.id,
                aletheia_project_id: ctx.session.projectId,
              },
