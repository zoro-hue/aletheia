...
// operator choice, and lowering into the flat Aletheia `FilterState`.
//
...
/** Aletheia score filter columns (filter by score NAME via key-value ops). */
...
// accepts today (filterTypeCompatibility.ts in @aletheia/shared).
...
export function negationIssue(
  ...
  if (op === "^" || op === "$") {
    return `negation of ${label(op)} is not representable in the Aletheia filter contract`;
  }
  if (op === "exact" || op === "=") {
