...
import { getUserProjectRoles } from "@aletheia/shared/src/server";
...
export const getAnnotationQueueRecordOrThrow = async ({
  ...
  if (!queue) {
    throw new AletheiaNotFoundError("Annotation queue not found");
  }
...
export const getAnnotationQueueItemRecordOrThrow = async ({
  ...
  if (!item) {
    throw new AletheiaNotFoundError("Annotation queue item not found");
  }
