...
import { InternalServerError, AletheiaNotFoundError } from "@aletheia/shared";
import { Prisma, prisma } from "@aletheia/shared/src/db";
import {
  getCurrentSpan,
  logger,
  recordHistogram,
...
export async function createMediaUploadUrl(params: {
  ...
  try {
    ...
    const uploadBucket = env.ALETHEIA_S3_MEDIA_UPLOAD_BUCKET;
...
export async function getMedia(params: { projectId: string; mediaId: string }) {
  ...
  if (!media) throw new AletheiaNotFoundError("Media asset not found");
  if (!media.uploadHttpStatus) {
    throw new AletheiaNotFoundError("Media not yet uploaded");
  }
  ...
  const ttlSeconds = env.ALETHEIA_S3_MEDIA_DOWNLOAD_URL_EXPIRY_SECONDS;
...
export async function updateMediaUploadStatus(params: {
  ...
  try {
    ...
    recordIncrement("aletheia.media.upload_http_status", 1, {
      status_code: uploadHttpStatus,
    });

    if (uploadTimeMs) {
      recordHistogram("aletheia.media.upload_time_ms", uploadTimeMs, {
        status_code: uploadHttpStatus,
      });
    }
  } catch (error) {
...
function getBucketPath(params: {
  ...
  const prefix = env.ALETHEIA_S3_MEDIA_UPLOAD_PREFIX ?? "";
