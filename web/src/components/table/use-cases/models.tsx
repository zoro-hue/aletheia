...
import { type AletheiaColumnDef } from "@/src/components/table/types";
import useColumnVisibility from "@/src/features/column-visibility/hooks/useColumnVisibility";
import { api } from "@/src/utils/api";
import { safeExtract } from "@/src/utils/map-utils";
import { type Prisma } from "@aletheia/shared/src/db";
...
import { AletheiaIcon } from "@/src/components/AletheiaLogo";
...
const modelConfigDescriptions = {
  ...
  maintainer:
    "Maintainer of the model. Aletheia managed models can be cloned, user managed models can be edited and deleted. To supersede a Aletheia managed model, set the custom model name to the Aletheia model name.",
...
export default function ModelTable({ projectId }: { projectId: string }) {
  ...
  const columns: AletheiaColumnDef<ModelTableRow>[] = [
    {
    ...
    },
    {
      ...
      size: 60,
      cell: ({ row }) => {
        const isAletheia = row.original.maintainer === "Aletheia";
        return (
          <div className="flex justify-center">
            <Tooltip>
              ...
              <TooltipContent>
                {isAletheia ? "Aletheia maintained" : "User maintained"}
              </TooltipContent>
            </Tooltip>
          </div>
        );
    ...
    },
    {
      ...
      size: 120,
      cell: ({ row }) => {
        return row.original.maintainer !== "Aletheia" ? (
          <div
  ...
  const convertToTableRow = (model: GetModelResult): ModelTableRow => {
    ...
    return {
      modelId: model.id,
      maintainer: model.projectId ? "User" : "Aletheia",
