"use client";
import { type OrderByState } from "@aletheia/shared";
import React, {
  useState,
  useMemo,
  useCallback,
...
interface DataTableProps<TData, TValue> {
  columns: AletheiaColumnDef<TData, TValue>[];
                ...
                <TableRow key={headerGroup.id}>
                  {headerGroup.headers.map((header) => {
                    const columnDef = header.column
                      .columnDef as AletheiaColumnDef<ModelTableRow>;
...
interface TableBodyComponentProps<TData> {
  ...
  rowheighttw?: string;
  rowHeight?: RowHeight;
  columns: AletheiaColumnDef<TData, any>[];
  data: AsyncTableData<TData[]>;
  help?: { description: string; href: string };
  noResultsMessage?: React.ReactNode;
  onRowClick?: (row: TData, event?: React.MouseEvent) => void;
        ...
        Array.from({ length: skeletonRowCount }).map((_, rowIndex) => (
          <TableRow key={`loading-row-${rowIndex}`} aria-hidden="true">
            {visibleColumns.map((column, columnIndex) => {
              const columnDef = column.columnDef as AletheiaColumnDef<TData>;
          ...
          >
            {row.getVisibleCells().map((cell) => {
              ...
              const columnDef = cell.column
                .columnDef as AletheiaColumnDef<TData>;
