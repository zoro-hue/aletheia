...
import { deepParseJson } from "@aletheia/shared";
import { decodeUnicodeEscapesOnly } from "@/src/utils/unicode";
import { Skeleton } from "@/src/components/ui/skeleton";
import { type MediaReturnType } from "@/src/features/media/validation";
import { AletheiaMediaView } from "@/src/components/ui/AletheiaMediaView";
...
type AletheiaExpandedState = ExpandedState | false;
import {
  Table,
  TableBody,
  TableCell,
...
import { ItemBadge, type AletheiaItemType } from "@/src/components/ItemBadge";
...
function JsonPrettyTable({
  ...
  const columns: AletheiaColumnDef<JsonTableRow, unknown>[] = [
    {
      accessorKey: "key",
      header: "Path",
      size: 35,
      cell: ({ row }) => {
        ...
        const itemBadgeType =
          ...
          (row.original.value as any).type
            ? ((row.original.value as any).type as AletheiaItemType)
            : null;
...
export function PrettyJsonView(props: {
  ...
  const [internalExpansionState, setInternalExpansionState] =
    useState<AletheiaExpandedState>({});
  ...
  const handleTableExpandedChange = useCallback(
    (
      ...
      if (typeof updater === "function") {
        ...
        const finalState: AletheiaExpandedState =
