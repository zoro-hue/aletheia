import { ChatMessageRole, ChatMessageType } from "@aletheia/shared";
import {
...
describe("message search controller", () => {
  ...
  // Regression test for https://github.com/aletheia/aletheia/issues/13002
  it("matches fullwidth and halfwidth variants consistently", () => {
    ...
    controller.registerPageMessages("page-1", [
      {
        ...
        role: ChatMessageRole.System,
        content:
          "Aletheia is an LLM observability platform. Ｌａｎｇｆｕｓｅ is also great.",
      },
    ]);
    ...
    expect(commitQuery(controller, "Aletheia")).toEqual([
      expect.objectContaining({ from: 0, to: 8 }),
      expect.objectContaining({ from: 43, to: 51 }),
    ]);
    expect(commitQuery(controller, "aletheia")).toEqual([
