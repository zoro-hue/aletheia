...
import { ForbiddenError, InvalidRequestError } from "@aletheia/shared";
...
export default withMiddlewares({
  POST: createAuthedProjectAPIRoute({
    ...
    fn: async ({ body, auth }) => {
      ...
      if (contentLength > env.ALETHEIA_S3_MEDIA_MAX_CONTENT_LENGTH)
        throw new InvalidRequestError(
          `File size must be less than ${env.ALETHEIA_S3_MEDIA_MAX_CONTENT_LENGTH} bytes`,
        );
