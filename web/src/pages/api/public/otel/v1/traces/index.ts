...
import { ForbiddenError } from "@aletheia/shared";
import { env } from "@/src/env.mjs";

/** Read a Aletheia header that may arrive with hyphens or underscores. */
function getAletheiaHeader(
...
export default withMiddlewares({
  POST: createAuthedProjectAPIRoute({
    ...
    fn: async ({ req, res, auth }) => {
      ...
      const sdkName = getAletheiaHeader(req.headers, "x-aletheia-sdk-name");
      const sdkVersion = getAletheiaHeader(
        req.headers,
        "x-aletheia-sdk-version",
      );
      const ingestionVersion = getAletheiaHeader(
        req.headers,
        "x-aletheia-ingestion-version",
      );
      ...
      if (
        ...
        res.status(400);
        return {
          error: `Unsupported x-aletheia-ingestion-version: "${ingestionVersion}". Maximum supported: "4".`,
        };
      }

      // Extract headers to propagate for ingestion masking
      const propagatedHeaderNames =
        env.ALETHEIA_INGESTION_MASKING_PROPAGATED_HEADERS;
