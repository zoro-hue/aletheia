...
import { jsonSchema } from "@aletheia/shared";
...
import { processEventBatch } from "@aletheia/shared/src/server";
...
export default async function handler(
  ...
  try {
    ...
    Object.keys(req.headers).forEach((header) => {
      if (
        header.toLowerCase().startsWith("x-aletheia") ||
        header.toLowerCase().startsWith("x_aletheia")
      ) {
        currentSpan?.setAttributes({
          [`aletheia.header.${header.slice(11).toLowerCase().replaceAll("_", "-")}`]:
            req.headers[header],
        });
      }
    });
  ...
  for (const event of batch) {
    ...
    if (type && EVENTS_ONLY_ALLOWED_TYPES.has(type)) {
      ...
      rejectedErrors.push({
        ...
        error: `Event type "${type ?? "unknown"}" is not accepted by /api/public/ingestion when ALETHEIA_MIGRATION_V4_WRITE_MODE is events_only. This endpoint only accepts score, log, and dataset-run-item events.`,
      });
    }
  }
