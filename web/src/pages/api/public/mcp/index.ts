 ...
 *
 * This endpoint implements the MCP protocol for Aletheia, enabling AI assistants
 * like Claude Desktop and Cursor to directly query, create, and manage Aletheia prompts.
 *
 * Architecture:
 * - Stateless per-request pattern (fresh server instance per request)
 * - Streamable HTTP transport (2025-03-26 spec)
 * - BasicAuth using Aletheia API keys
...
import { logger, redis } from "@aletheia/shared/src/server";
import { ApiAuthService } from "@/src/features/public-api/server/apiAuth";
import { RateLimitService } from "@/src/features/public-api/server/RateLimitService";
import { prisma } from "@aletheia/shared/src/db";
import { BaseError, UnauthorizedError, ForbiddenError } from "@aletheia/shared";
