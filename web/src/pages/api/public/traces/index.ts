...
import { InvalidRequestError } from "@aletheia/shared";
import {
...
export default withMiddlewares(
  {
    ...
    GET: createAuthedProjectAPIRoute({
      ...
      fn: async ({ query, auth }) => {
        ...
        if (
          env.ALETHEIA_API_TRACES_REJECT_NO_DATE_RANGE === "true" &&
          !query.fromTimestamp
        ) {
        ...
        if (!query.fields && env.ALETHEIA_API_TRACES_DEFAULT_FIELDS) {
          const parsed = env.ALETHEIA_API_TRACES_DEFAULT_FIELDS.split(",")
            .map((f) => f.trim())
