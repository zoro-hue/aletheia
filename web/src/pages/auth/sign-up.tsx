...
import { noUrlCheck, StringNoHTMLNonEmpty } from "@aletheia/shared";
...
function StandardSignupFlow({
  ...
  const [lastUsedAuthMethod, setLastUsedAuthMethod] =
    useLocalStorage<NextAuthProvider | null>(
      "aletheia_last_used_auth_method",
      null,
    );
...
function VerifiedSignupFlow({
  ...
  if (phase === "otp") {
    return (
      <>
        <Head>
          <title>Verify your email | Aletheia</title>
        </Head>
        <div className="flex flex-1 flex-col py-6 sm:min-h-full sm:justify-center sm:px-6 sm:py-12 lg:px-8">
          <div className="sm:mx-auto sm:w-full sm:max-w-md">
            <AletheiaIcon className="mx-auto" />
