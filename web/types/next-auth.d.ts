...
declare module "next-auth" {
  interface Session extends DefaultSession {
    ...
    environment: {
      ...
      // Enables features that are only available under an enterprise/commercial license when self-hosting Aletheia
      selfHostedInstancePlan: Plan | null;
      // V4 migration write mode. Mirrors ALETHEIA_MIGRATION_V4_WRITE_MODE so the
  ...
  interface User extends DefaultUser {
    ...
    canCreateOrganizations: boolean; // default true, allowlist can be set via ALETHEIA_ALLOWED_ORGANIZATION_CREATORS
    organizations: {
