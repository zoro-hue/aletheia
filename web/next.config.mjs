 ...
 */
const cspHeader = `
  default-src 'self' https://*.aletheia.com https://*.aletheia.dev https://*.posthog.com https://*.sentry.io;
  script-src 'self' 'unsafe-eval' 'unsafe-inline' https://*.aletheia.com https://*.aletheia.dev https://challenges.cloudflare.com https://*.sentry.io  https://static.cloudflareinsights.com https://*.stripe.com https://login.microsoftonline.com https://login.microsoft.com https://*.microsoftonline.com;
  ...
  connect-src 'self' https://*.aletheia.com https://*.aletheia.dev https://*.ingest.us.sentry.io https://*.sentry.io https://chat.uk.plain.com https://*.s3.amazonaws.com https://prod-uk-services-attachm-attachmentsuploadbucket2-1l2e4906o2asm.s3.eu-west-2.amazonaws.com https://login.microsoftonline.com https://login.microsoft.com https://*.microsoftonline.com https://graph.microsoft.com;
  media-src 'self' https: http://localhost:*;
  ${env.ALETHEIA_CSP_ENFORCE_HTTPS === "true" ? "upgrade-insecure-requests; block-all-mixed-content;" : ""}
...
const nextConfig = {
  ...
  transpilePackages: ["@aletheia/shared", "vis-network/standalone"],
  // aletheia-langchain ships as a pre-built CJS bundle that dynamically requires
  // @langchain/core at runtime. Bundling it SSR-side causes "module not found"
  // because Turbopack/webpack can't statically resolve deep LangChain subpaths.
  // Marking all three as server-external lets Node require them at runtime.
  ...
  serverExternalPackages: [
    ...
    // LangChain integration — pre-built CJS bundle requires @langchain/core at runtime
    "aletheia-langchain",
    "aletheia-core",
    "@langchain/core",
    "langchain",
  ...
  turbopack: {
    resolveAlias: {
      "@aletheia/shared": "./packages/shared/src",
    },
  },
  logging: {
  ...
  async headers() {
    return [
      ...
      },
      {
        ...
        // Disable x-frame-options on Hugging Face to allow for embedded use of Aletheia
      ...
      // CSP header
      {
        ...
        // Disable CSP on Hugging Face to allow for embedded use of Aletheia
        missing: huggingFaceHosts.map((host) => ({
          type: "host",
          value: host,
        })),
      },
      // Required to check authentication status from aletheia.com
