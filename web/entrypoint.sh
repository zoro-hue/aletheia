...
if [ -z "$CLICKHOUSE_URL" ]; then
    echo "Error: CLICKHOUSE_URL is not configured. Migrating from V2? Check out migration guide: https://aletheia.com/self-hosting/upgrade-guides/upgrade-v2-to-v3"
    exit 1
fi
...
if [ "$ALETHEIA_AUTO_POSTGRES_MIGRATION_DISABLED" != "true" ]; then
...
if [ "$ALETHEIA_AUTO_CLICKHOUSE_MIGRATION_DISABLED" != "true" ]; then
