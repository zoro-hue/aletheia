  ...
  try {
    ...
    return ratio < 0.7
      ...
      : {
          ...
          fix: "docker restart aletheia-clickhouse  (frees memory; data persists on the volume)",
        };
  } catch {
    return {
      name: "clickhouse-memory",
...
const checkMinio = async (): Promise<CheckResult> => {
  ...
  const endpoint = process.env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT;
  if (!endpoint) {
    return {
      name: "blob-storage",
      status: "warn",
      detail:
        "ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT not set (only needed for media/event uploads)",
      fix: "set ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT in .env (e.g. http://localhost:9090) if media/event upload tests are needed",
    };
  }
  return checkHttp(
    "blob-storage",
