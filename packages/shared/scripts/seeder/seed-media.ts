...
const MEDIA_FILES: Record<string, MediaFile> = {
  image: {
    name: "aletheia-logo.png",
    contentType: "image/png",
    filePath: path.join(TEST_FILES_DIR, "aletheia-logo.png"),
  },
  pdf: {
    name: "bitcoin.pdf",
...
export function getSeedMediaFixture(
  ...
  const fixture = {
    ...
    referenceString: `@@@aletheiaMedia:type=${mediaFile.contentType}|id=${mediaId}|source=${SEED_MEDIA_REFERENCE_SOURCE}@@@`,
  };
...
async function uploadAndCreateMediaRecord(
  ...
  if (!env.ALETHEIA_S3_MEDIA_UPLOAD_BUCKET) {
    logger.warn(
      "[seed-media] ALETHEIA_S3_MEDIA_UPLOAD_BUCKET not configured, skipping media seeding",
    );
    return;
  }
  ...
  try {
    const storageClient = StorageServiceFactory.getInstance({
      bucketName: env.ALETHEIA_S3_MEDIA_UPLOAD_BUCKET,
  ...
  await prisma.$queryRaw`
    ...
    DO UPDATE SET
      "bucket_name" = ${env.ALETHEIA_S3_MEDIA_UPLOAD_BUCKET},
