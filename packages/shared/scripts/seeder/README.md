...
One command to put any local test state into Aletheia — for developers and
for coding agents.
...
- the v4 read path is the per-user "Fast (Preview)" sidebar toggle or
  `ALETHEIA_MIGRATION_V4_WRITE_MODE=events_only` server-side; the trace URL
...
- **API writer**: build the same logical data as public ingestion batches
  against `ALETHEIA_BASE_URL` + keys, with batch limits and `--wait`
...
- **dx presets**: a `scenarios/presets.ts` invoked from the examples seed,
  selected via `ALETHEIA_SEED_PRESET`, to give default dx data more variety
