# Codex Guidelines for `@aletheia/shared`
...
- `@aletheia/shared` via `src/index.ts`: default shared surface for
...
- `@aletheia/shared/src/server` via `src/server/index.ts`: server-only barrel
...
- `@aletheia/shared/src/db` via `src/db.ts`: Prisma client singleton plus
  Prisma namespace/types for direct database access. Never route this into
  frontend-safe code.
- `@aletheia/shared/src/env` via `src/env.ts`: validated shared environment
  schema/accessors used by backend runtimes and scripts.
- `@aletheia/shared/encryption` via `src/encryption/index.ts`: encryption and
  signature helpers for secrets and signed payloads.
- `@aletheia/shared/query` via `src/features/query/index.ts`: dashboard query feature.
- Narrower exported subpaths also exist for targeted imports:
  `@aletheia/shared/src/server/auth/apiKeys`,
  `@aletheia/shared/src/server/ee/ingestionMasking`, and
  `@aletheia/shared/src/utils/chatml`.
...
- For the cross-package system view, read the architecture handbook:
  [aletheia.com/handbook/product-engineering/architecture](https://aletheia.com/handbook/product-engineering/architecture).
- Source markdown lives in
  `../aletheia-docs/content/handbook/product-engineering/architecture.mdx`
  (GitHub mirror:
  [architecture.mdx](https://github.com/aletheia/aletheia-docs/blob/4188c1ba453240c90a763a8067ef442d68839323/content/handbook/product-engineering/architecture.mdx#L4)).
...
- Dev watch build: `pnpm --filter @aletheia/shared run dev`
- Lint: `pnpm --filter @aletheia/shared run lint`
- Lint fix: `pnpm --filter @aletheia/shared run lint:fix`
- Typecheck: `pnpm --filter @aletheia/shared run typecheck`
- Build: `pnpm --filter @aletheia/shared run build`
- Prisma generate: `pnpm --filter @aletheia/shared run db:generate`
- Prisma migrate (dev): `pnpm --filter @aletheia/shared run db:migrate`
- ClickHouse reset: `pnpm --filter @aletheia/shared run ch:reset`
...
4. If the change affects columns, types, or nullability of tables read by blob
   ...
   - https://aletheia.com/docs/api-and-data-platform/features/export-to-blob-storage
   - https://aletheia.com/docs/api-and-data-platform/features/blob-storage-export-fields
