...
export const aletheiaS3EventKeyMaxSegmentBytesSchema = z.coerce
  .number()
  .int()
  .min(64)
  .max(2048)
...
const EnvSchema = z.object({
  ...
  ALETHEIA_BULLMQ_SKIP_REDIS_VERSION_CHECK: z
    .enum(["true", "false"])
    .default("false"),
  ...
  ENCRYPTION_KEY: z
    ...
    .optional(),
  ALETHEIA_CACHE_MODEL_MATCH_ENABLED: z.enum(["true", "false"]).default("true"),
  ALETHEIA_CACHE_MODEL_MATCH_TTL_SECONDS: z.coerce.number().default(86400), // 24 hours
  ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_LOCAL_CACHE_MODEL_MATCH_TTL_MS: z.coerce
    .number()
    .positive()
    .default(10_000),
  ALETHEIA_LOCAL_CACHE_MODEL_MATCH_MAX: z.coerce
    .number()
    .int()
    .positive()
    .default(20_000),
  ALETHEIA_CACHE_PROMPT_ENABLED: z.enum(["true", "false"]).default("true"),
  ALETHEIA_CACHE_PROMPT_TTL_SECONDS: z.coerce.number().default(3600), // 1h
  ...
  // Aletheia detects the ClickHouse version on startup and applies known
  ...
  CLICKHOUSE_USE_QUERY_CONDITION_CACHE: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_ENABLE_SINGLE_LEVEL_QUERY_OPTIMIZATION: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_ROOT_EVENT_CONDITION_MAX_WINDOW_HOURS: z.coerce
    .number()
    .int()
    .nonnegative()
    .default(168), // 7 days
  ALETHEIA_INGESTION_QUEUE_DELAY_MS: z.coerce
    .number()
    .nonnegative()
    .default(15_000),
  ALETHEIA_INGESTION_QUEUE_SHARD_COUNT: z.coerce.number().positive().default(1),
  ALETHEIA_INGESTION_SECONDARY_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_OTEL_INGESTION_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_OTEL_INGESTION_SECONDARY_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_OTEL_MAX_SPAN_BYTES: z.coerce.number().positive().default(9_500_000), // 9.5MB — just under ClickHouse's 10MB min_chunk_bytes_for_parallel_parsing default
  ALETHEIA_EVAL_EXECUTION_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_EVAL_EXECUTION_SECONDARY_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_LLM_AS_JUDGE_EXECUTION_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_CODE_EVAL_EXECUTION_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_CODE_EVAL_DISPATCHER: z
    .enum(["insecure-local", "aws-lambda"])
    .optional(),
  ALETHEIA_CODE_EVAL_AWS_LAMBDA_ENDPOINT: z.string().optional(),
  ALETHEIA_CODE_EVAL_AWS_LAMBDA_NODE_FUNCTION_NAME: z
    .string()
    .default("code-based-eval-executor-node"),
  ALETHEIA_CODE_EVAL_AWS_LAMBDA_PYTHON_FUNCTION_NAME: z
    .string()
    .default("code-based-eval-executor-python"),
  ALETHEIA_CODE_EVAL_LOCAL_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(2_000),
  ALETHEIA_TRACE_UPSERT_QUEUE_SHARD_COUNT: z.coerce
    .number()
    .positive()
    .default(1),
  ALETHEIA_TRACE_UPSERT_QUEUE_ATTEMPTS: z.coerce.number().positive().default(2),
  ALETHEIA_TRACE_DELETE_DELAY_MS: z.coerce
    .number()
    .nonnegative()
    .default(5_000),
  ALETHEIA_DELETE_SKIP_PROJECT_IDS: z
    .string()
    .optional()
    .transform((s) => (s ? s.split(",").map((id) => id.trim()) : [])),
  SALT: z.string().optional(), // used by components imported by web package
  ALETHEIA_LOG_LEVEL: z
    .enum(["trace", "debug", "info", "warn", "error", "fatal"])
    .optional(),
  ALETHEIA_LOG_FORMAT: z.enum(["text", "json"]).default("text"),
  ALETHEIA_LOG_PROPAGATED_HEADERS: z
    .string()
    .optional()
    .transform((s) =>
  ...
  ENABLE_AWS_CLOUDWATCH_METRIC_PUBLISHING: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_S3_CONCURRENT_WRITES: z.coerce.number().positive().default(50),
  ALETHEIA_S3_UPLOAD_ENABLE_BUFFERED: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_S3_UPLOAD_MAX_PART_ATTEMPTS: z.coerce
    .number()
    .min(1)
    .max(10)
    .default(3),
  ALETHEIA_S3_UPLOAD_MAX_CONCURRENT_PARTS: z.coerce
    .number()
    .min(1)
    .max(10)
    .default(3),
  ALETHEIA_S3_EVENT_UPLOAD_BUCKET: z.string(), // Aletheia requires a bucket name for S3 Event Uploads.
  ...
  ALETHEIA_AZURE_SKIP_CONTAINER_CHECK: z
    .enum(["true", "false"])
    .default("true"),
  ALETHEIA_USE_GOOGLE_CLOUD_STORAGE: z.enum(["true", "false"]).default("false"),
  ALETHEIA_GOOGLE_CLOUD_STORAGE_CREDENTIALS: z.string().optional(),
  GOOGLE_CLOUD_UNIVERSE_DOMAIN: z.string().default("googleapis.com"),
  ALETHEIA_USE_OCI_NATIVE_OBJECT_STORAGE: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_OCI_AUTH_TYPE: z
    .enum([
      "workload_identity",
      "instance_principal",
      "resource_principal",
    ...
    .optional(),
  ALETHEIA_OCI_CONFIG_FILE: z.string().optional(),
  ALETHEIA_OCI_CONFIG_PROFILE: z.string().optional(),
  ...
  ALETHEIA_S3_LIST_MAX_KEYS: z.coerce.number().positive().default(200),
  ALETHEIA_S3_RATE_ERROR_SLOWDOWN_ENABLED: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_S3_RATE_ERROR_SLOWDOWN_TTL_SECONDS: z.coerce
    .number()
    .positive()
    .default(3600), // 1 hour
  ALETHEIA_S3_CORE_DATA_EXPORT_IS_ENABLED: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_S3_CORE_DATA_EXPORT_SSE: z.enum(["AES256", "aws:kms"]).optional(),
  ALETHEIA_S3_CORE_DATA_EXPORT_SSE_KMS_KEY_ID: z.string().optional(),
  ALETHEIA_POSTGRES_METERING_DATA_EXPORT_IS_ENABLED: z
    .enum(["true", "false"])
    .default("false"),

  ALETHEIA_CUSTOM_SSO_EMAIL_CLAIM: z.string().default("email"),
  ALETHEIA_CUSTOM_SSO_NAME_CLAIM: z.string().default("name"),
  ALETHEIA_CUSTOM_SSO_SUB_CLAIM: z.string().default("sub"),
  ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES: z.coerce
    .number()
    .default(80e6), // 80MB
  ALETHEIA_CLICKHOUSE_DELETION_TIMEOUT_MS: z.coerce.number().default(600_000), // 10 minutes
  ALETHEIA_CLICKHOUSE_QUERY_MAX_ATTEMPTS: z.coerce.number().default(3), // Maximum attempts for socket hang up errors
  ALETHEIA_SKIP_S3_LIST_FOR_OBSERVATIONS_PROJECT_IDS: z.string().optional(),
  ALETHEIA_INGESTION_PROCESSING_SAMPLED_PROJECTS: z
    .string()
    .optional()
    .transform((val) => {
      try {
  ...
  ALETHEIA_WEBHOOK_WHITELISTED_IPS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_WEBHOOK_WHITELISTED_IP_SEGMENTS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_WEBHOOK_WHITELISTED_HOST: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_LLM_CONNECTION_WHITELISTED_IPS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_LLM_CONNECTION_WHITELISTED_IP_SEGMENTS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IP_SEGMENTS: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
    ),
  ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_HOST: z
    .string()
    .optional()
    .transform((s) =>
      s ? s.split(",").map((s) => s.toLowerCase().trim()) : [],
  ...
  ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT: z.coerce
    .number()
    .int()
    .positive()
    .default(1_000),

  ALETHEIA_CLICKHOUSE_DATA_EXPORT_REQUEST_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
  ...
  ALETHEIA_FETCH_LLM_COMPLETION_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
  ...
  ALETHEIA_API_CLICKHOUSE_DISABLE_OBSERVATIONS_FINAL: z
    .enum(["true", "false"])
    .default("false"),
  ...
  // Dataset Service
  ALETHEIA_DATASET_SERVICE_WRITE_TO_VERSIONED_IMPLEMENTATION: z
    .enum(["true", "false"])
    .default("true"),
  ALETHEIA_DATASET_SERVICE_READ_FROM_VERSIONED_IMPLEMENTATION: z
    .enum(["true", "false"])
    .default("true"),

  // API ClickHouse query options
  ALETHEIA_API_CLICKHOUSE_PROPAGATE_OBSERVATIONS_TIME_BOUNDS: z
    .enum(["true", "false"])
    .default("false"),
  ...
  ALETHEIA_INGESTION_MASKING_CALLBACK_URL: z.url().optional(),
  ALETHEIA_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: z.coerce
    .number()
    .positive()
    .default(500),
  ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: z
    .enum(["true", "false"])
    .default("false"),
  ALETHEIA_INGESTION_MASKING_MAX_RETRIES: z.coerce
    .number()
    .nonnegative()
    .default(1),
