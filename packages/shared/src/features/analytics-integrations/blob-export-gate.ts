...
// This is a client-safe file that can be imported from @aletheia/shared.
...
// NEXT_PUBLIC_ALETHEIA_BLOB_EXPORT_CUTOFF overrides the default for local dev testing.
const _override = process.env.NEXT_PUBLIC_ALETHEIA_BLOB_EXPORT_CUTOFF
  ? new Date(process.env.NEXT_PUBLIC_ALETHEIA_BLOB_EXPORT_CUTOFF)
  : null;
...
// see the note above.
// NEXT_PUBLIC_ALETHEIA_BLOB_EXPORTER_CUTOFF overrides the default for local dev testing.
const _exporterOverride = process.env.NEXT_PUBLIC_ALETHEIA_BLOB_EXPORTER_CUTOFF
  ? new Date(process.env.NEXT_PUBLIC_ALETHEIA_BLOB_EXPORTER_CUTOFF)
  : null;
 ...
 * via ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN (server-side only — pass the flag
