...
const aletheiaObject = z.enum(aletheiaObjects);
export type AletheiaEvaluationObject = z.infer<typeof aletheiaObject>;
...
export const variableMapping = z
  .object({
    ...
    aletheiaObject: aletheiaObject,
    selectedColumnId: z.string(),
    jsonSelector: z.string().nullish(),
  })
  .refine(
    (value) =>
      value.aletheiaObject === "trace" ||
      value.aletheiaObject === "dataset_item" ||
...
// WIP version for forms - aletheiaObject optional to support both:
// - Trace/Dataset evals: Include aletheiaObject and objectName to specify which observation
...
export const wipVariableMapping = z.object({
  ...
  objectName: z.string().nullish(),
  aletheiaObject: aletheiaObject.optional(),
