...
export function blobStorageEndpointWhitelistFromEnv(): OutboundUrlValidationWhitelist {
  ...
  return {
    hosts: env.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_HOST || [],
    ips: env.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS || [],
    ip_ranges: env.ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IP_SEGMENTS || [],
  };
}
...
export async function validateBlobStorageEndpoint(
  ...
  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION && url.protocol !== "https:") {
    throw new Error(
      "Only HTTPS blob storage endpoints are allowed on Aletheia Cloud",
    );
  }
...
function getEffectiveWhitelist(
  ...
  return env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION
...
function isBlobStorageEndpointValidationEnabled(
  ...
  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) return true;
...
function getSelfHostedWhitelistGuidance(): string {
  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) return "";

  return " For self-hosted deployments with internal blob storage endpoints, configure ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_HOST, ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IPS, or ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_IP_SEGMENTS.";
}
