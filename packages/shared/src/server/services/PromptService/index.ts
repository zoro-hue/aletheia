...
export class PromptService {
  ...
  constructor(
    ...
    if (cacheEnabled !== undefined) {
      ...
      this.cacheEnabled =
        Boolean(redis) && env.ALETHEIA_CACHE_PROMPT_ENABLED === "true";
    }

    this.ttlSeconds = env.ALETHEIA_CACHE_PROMPT_TTL_SECONDS;
  }
      ...
      const resolve = async (
        ...
        if (promptDependencies && promptDependencies.length) {
          ...
          for (const dep of promptDependencies) {
            ...
            const versionPattern = `@@@aletheiaPrompt:name=${escapeRegex(depPrompt.name)}\\|version=${escapeRegex(depPrompt.version)}@@@`;
            const labelPatterns = depPrompt.labels.map(
              (label) =>
                `@@@aletheiaPrompt:name=${escapeRegex(depPrompt.name)}\\|label=${escapeRegex(label)}@@@`,
            );
