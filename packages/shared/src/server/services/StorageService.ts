...
function createS3RequestHandler(
  ...
  const maxSockets = env.ALETHEIA_S3_CONCURRENT_WRITES;
...
function addS3SigningDiagnosticsMiddleware(
  ...
  client.middlewareStack.add(
    ...
    >[0],
    {
      step: "deserialize",
      priority: "high",
      name: "aletheiaS3SigningDiagnostics",
      tags: ["ALETHEIA", "DIAGNOSTICS"],
      override: true,
    },
  );
}
...
function createSecureAzureBlobRequestPolicyFactory(
  ...
  const { httpAgent, httpsAgent } = getSecureOutboundHttpAgents(
    connectionValidation,
    { maxSockets: env.ALETHEIA_S3_CONCURRENT_WRITES },
  );
  ...
  public static getInstance(params: {
    ...
    if (
      params.useAzureBlob !== undefined
        ? params.useAzureBlob
        : env.ALETHEIA_USE_AZURE_BLOB === "true"
    ) {
    ...
    if (
      params.useGoogleCloudStorage !== undefined
        ...
        : env.ALETHEIA_USE_GOOGLE_CLOUD_STORAGE === "true"
    ) {
      // connectionValidation is intentionally not applied here: GCS is selected
      // by deployment env for Aletheia-owned storage, not by user-configured
      ...
      const googleParams = {
        ...
        googleCloudCredentials:
          ...
          env.ALETHEIA_GOOGLE_CLOUD_STORAGE_CREDENTIALS,
      };
    ...
    if (
      params.useOCIObjectStorage !== undefined
        ...
        : env.ALETHEIA_USE_OCI_NATIVE_OBJECT_STORAGE === "true"
    ) {
...
class AzureBlobStorageService implements StorageService {
  ...
  private async createContainerIfNotExists(): Promise<void> {
    ...
    if (env.ALETHEIA_AZURE_SKIP_CONTAINER_CHECK === "true") {
      return;
    }
    ...
    try {
      ...
      for await (const blob of result) {
        if (blob.name.startsWith(prefix)) {
          ...
          if (files.length >= env.ALETHEIA_S3_LIST_MAX_KEYS) {
            break;
          }
        }
      }
...
class S3StorageService implements StorageService {
  ...
  public async uploadFileBuffered({
    ...
    if (env.ALETHEIA_S3_UPLOAD_ENABLE_BUFFERED !== "true") {
    ...
    const uploader = new BufferedStreamUploader({
      strategy,
      partSizeBytes,
      maxPartAttempts: env.ALETHEIA_S3_UPLOAD_MAX_PART_ATTEMPTS,
      maxConcurrentParts: env.ALETHEIA_S3_UPLOAD_MAX_CONCURRENT_PARTS,
      key: fileName,
    });
  ...
  public async listFiles(
    ...
    const listCommand = new ListObjectsV2Command({
      Bucket: this.bucketName,
      Prefix: prefix,
      MaxKeys: env.ALETHEIA_S3_LIST_MAX_KEYS,
    });
  ...
  public async listFiles(
    ...
    try {
      const [files] = await this.bucket.getFiles({
        prefix,
        maxResults: env.ALETHEIA_S3_LIST_MAX_KEYS,
      });
...
class OCIObjectStorageService implements StorageService {
  ...
  private async initClient(params: { endpoint?: string; region?: string }) {
    ...
    switch (env.ALETHEIA_OCI_AUTH_TYPE) {
      ...
      case "oci_profile": {
        provider = new common.ConfigFileAuthenticationDetailsProvider(
          env.ALETHEIA_OCI_CONFIG_FILE,
          env.ALETHEIA_OCI_CONFIG_PROFILE,
        );
      ...
      default:
        throw new Error(
          "OCI auth not configured: set ALETHEIA_OCI_AUTH_TYPE to " +
