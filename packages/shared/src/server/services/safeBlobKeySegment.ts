 ...
 * (`env.ALETHEIA_S3_EVENT_KEY_MAX_SEGMENT_BYTES` in production).
 */
export function safeBlobKeySegment(
  segment: string,
  maxBytes: number = env.ALETHEIA_S3_EVENT_KEY_MAX_SEGMENT_BYTES,
): string {
 ...
 * `ALETHEIA_S3_EVENT_KEY_MAX_SEGMENT_BYTES`. Caller appends the suffix.
 *
...
export function safeBlobFilenameStem(
  ...
  return safeBlobKeySegment(
    stem,
    env.ALETHEIA_S3_EVENT_KEY_MAX_SEGMENT_BYTES - filenameSuffix.length,
  );
}
