 ...
 *
 * Logic flow:
 * 1. Check ALETHEIA_API_CLICKHOUSE_DISABLE_OBSERVATIONS_FINAL (takes precedence)
 * 2. If ALETHEIA_SKIP_FINAL_FOR_OTEL_PROJECTS is enabled, check Redis for project OTEL usage
 * 3. Returns boolean
 */
export async function shouldSkipObservationsFinal(
  ...
  if (env.ALETHEIA_API_CLICKHOUSE_DISABLE_OBSERVATIONS_FINAL === "true") {
...
export function shouldUseObservationsSubqueryRewrite(): boolean {
  return env.ALETHEIA_OBSERVATIONS_V2_SUBQUERY_REWRITE === "true";
}

export function shouldRunObservationsShadowQuery(): boolean {
  return (
    env.ALETHEIA_OBSERVATIONS_V2_SHADOW_QUERY === "true" &&
    Math.random() < env.ALETHEIA_OBSERVATIONS_V2_SHADOW_QUERY_SAMPLE_RATE
  );
}
