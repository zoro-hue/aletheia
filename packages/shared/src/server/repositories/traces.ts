...
export const getTracesBySessionId = async (
  ...
  traces.forEach((trace) => {
    recordDistribution(
      "aletheia.traces_by_session_id_age",
    ...
    fn: (input) => {
      const inputColumn = excludeInputOutput
        ...
        : renderingProps.truncated
          ? `leftUTF8(input, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT})`
          : "input";
      const outputColumn = excludeInputOutput
        ? "''"
        : renderingProps.truncated
          ? `leftUTF8(output, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT})`
          : "output";
...
export const getTracesForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      aletheia_trace_name: record.name,
      aletheia_url: `${baseUrl}/project/${projectId}/traces/${encodeURIComponent(record.id as string)}`,
      aletheia_user_url: record.user_id
        ? `${baseUrl}/project/${projectId}/users/${encodeURIComponent(record.user_id as string)}`
        : undefined,
      aletheia_cost_usd: record.total_cost,
      aletheia_count_observations: record.observation_count,
...
async function buildTracesBaseQuery(
  ...
  const propagateObservationsTimeBounds =
    env.ALETHEIA_API_CLICKHOUSE_PROPAGATE_OBSERVATIONS_TIME_BOUNDS === "true";
