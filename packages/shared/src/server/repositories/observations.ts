...
import { InternalServerError, AletheiaNotFoundError } from "../../errors";
...
export const getObservationsForTrace = async <IncludeIO extends boolean>(
  ...
  for (const observation of records) {
    ...
    if (payloadSize >= env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES) {
      const errorMessage = `Observations in trace are too large: ${(payloadSize / 1e6).toFixed(2)}MB exceeds limit of ${(env.ALETHEIA_API_TRACE_OBSERVATIONS_SIZE_LIMIT_BYTES / 1e6).toFixed(2)}MB`;
...
const getObservationByIdInternal = async ({
  ...
  SELECT
    ...
    status_message,
    version,
    ${fetchWithInputOutput ? (renderingProps.truncated ? `leftUTF8(input, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as input, leftUTF8(output, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as output,` : "input, output,") : ""}
    provided_model_name,
    internal_model_id,
    model_parameters,
    provided_usage_details,
...
export const getGenerationsForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      aletheia_generation_name: record.name,
      ...
      aletheia_user_url: record.trace_user_id
