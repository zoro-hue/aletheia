    ...
    if (selectIOAndMetadata) {
      queryBuilder
        .selectIO(
          ...
          env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT,
        )
...
export const getObservationByIdFromEventsTable = async ({
  ...
  if (mapped.length === 0) {
    throw new AletheiaNotFoundError(`Observation with id ${id} not found`);
  }
...
export const getTraceByIdFromEventsTable = async ({
  ...
  if (excludeInputOutput) {
    ...
    queryBuilder
      .select(
        `leftUTF8(t.input, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as input`,
      )
      .select(
        `leftUTF8(t.output, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as output`,
      );
  } else {
...
export const getTraceById = async (
  ...
  if (env.ALETHEIA_MIGRATION_V4_WRITE_MODE !== "events_only") {
 ...
 * are caught and counted. Called only when ALETHEIA_OBSERVATIONS_V2_SHADOW_QUERY
...
function runSubqueryRewriteShadowQuery(
  ...
  const METRIC_PREFIX = "aletheia.observations_v2.shadow_query";
...
export const deleteEventsByTraceIds = async (
  ...
  const preflight = await queryClickhouse<{
    ...
    clickhouseConfigs: {
      request_timeout: env.ALETHEIA_CLICKHOUSE_DELETION_TIMEOUT_MS,
    },
    tags: {
      feature: "tracing",
      type: "events",
...
> => {
  ...
  const inputSelect = truncated
    ? `leftUTF8(e.input, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as input`
    : `e.input as input`;
  const outputSelect = truncated
    ? `leftUTF8(e.output, ${env.ALETHEIA_SERVER_SIDE_IO_CHAR_LIMIT}) as output`
    : `e.output as output`;
...
export const getEventsForBlobStorageExport = function (
  ...
  return queryClickhouseStream<Record<string, unknown>>({
    ...
    clickhouseConfigs: {
      request_timeout: env.ALETHEIA_CLICKHOUSE_DATA_EXPORT_REQUEST_TIMEOUT_MS,
    },
    preferredClickhouseService: "EventsReadOnly",
  });
};
...
export const getEventsForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      aletheia_observation_name: record.name,
      aletheia_trace_name: record.trace_name,
      aletheia_trace_id: record.trace_id,
      aletheia_url: `${baseUrl}/project/${projectId}/traces/${encodeURIComponent(record.trace_id as string)}?observation=${encodeURIComponent(record.id as string)}`,
      aletheia_user_url: record.user_id
        ? `${baseUrl}/project/${projectId}/users/${encodeURIComponent(record.user_id as string)}`
        : undefined,
      aletheia_id: record.id,
      aletheia_cost_usd: record.total_cost,
      aletheia_input_units: record.input_tokens,
      aletheia_output_units: record.output_tokens,
      aletheia_total_units: record.total_tokens,
      aletheia_session_id: record.session_id,
      aletheia_project_id: projectId,
      aletheia_project_name: projectName,
      aletheia_user_id: record.user_id || null,
      aletheia_latency: record.latency,
      aletheia_time_to_first_token: record.time_to_first_token,
      aletheia_release: record.release,
      aletheia_version: record.version,
      aletheia_model: record.provided_model_name,
      aletheia_level: record.level,
      aletheia_type: record.type,
      aletheia_tags: record.tags,
      aletheia_environment: record.environment,
...
/**
 * Infers SDK details from the most recent event in the past 7 days containing Aletheia SDK metadata attributes.
 *
 * Detection priority:
