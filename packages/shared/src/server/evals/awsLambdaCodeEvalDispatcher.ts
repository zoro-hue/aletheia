...
export class AwsLambdaCodeEvalDispatcher implements CodeEvalDispatcher {
  ...
  private async dispatchWithTracing(
    ...
    span.setAttributes({
      ...
      "aletheia.code_eval.lambda.function_name": functionName,
      "aletheia.org.id": input.scope.organizationId,
      "aletheia.project.id": input.scope.projectId,
    });

    try {
      const serializedPayload = assertDispatchInputWithinLimits(input);
      span.setAttribute(
        "aletheia.code_eval.payload.bytes",
      ...
      setSpanAttributes(span, {
        ...
        "aletheia.code_eval.lambda.status_code": response.StatusCode,
        "aletheia.code_eval.lambda.executed_version": response.ExecutedVersion,
        "aletheia.code_eval.lambda.function_error": response.FunctionError,
      });
      ...
      span.setAttribute(
        "aletheia.code_eval.lambda.response_payload.bytes",
      ...
      setSpanAttributes(span, {
        ...
        "aletheia.code_eval.aws_error.name":
          error instanceof Error ? error.name : undefined,
        "aletheia.code_eval.aws_error.code":
...
function setDispatcherErrorSpanAttributes(
  ...
  span.setAttributes({
    "aletheia.code_eval.error.code": error.code,
    "aletheia.code_eval.error.retryable": error.retryable,
  });
}
