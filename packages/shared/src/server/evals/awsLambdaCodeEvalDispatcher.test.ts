...
describe("AwsLambdaCodeEvalDispatcher observability", () => {
  ...
  it("dispatches inside a dedicated trace with code evaluator context", async () => {
    ...
    expectSpanAttributes({
      ...
      "aletheia.code_eval.lambda.function_name":
        "code-based-eval-executor-node",
      "aletheia.org.id": "org-1",
      "aletheia.project.id": "project-1",
    });
    expect(mocks.span.setAttribute).toHaveBeenCalledWith("eval.score.count", 1);
    expectSpanAttributes({
      "aletheia.code_eval.lambda.status_code": 200,
      "aletheia.code_eval.lambda.executed_version": "$LATEST",
  ...
  it("records the original AWS SDK error before throwing the derived dispatcher error", async () => {
    ...
    expectSpanAttributes({
      "aletheia.code_eval.error.code": "LAMBDA_CONCURRENCY_LIMIT",
      "aletheia.code_eval.error.retryable": true,
    });
    expectSpanAttributes({
      "aletheia.code_eval.aws_error.name": "TooManyRequestsException",
      "aletheia.code_eval.aws_error.code": "TooManyRequests",
  ...
  it("records dispatch context and error code for preflight limit failures", async () => {
    ...
    expectSpanAttributes({
      "aletheia.code_eval.error.code": "SOURCE_TOO_LARGE",
      "aletheia.code_eval.error.retryable": false,
    });
  });
  ...
  it("records the original Lambda FunctionError before throwing the derived user-facing error", async () => {
    ...
    expectSpanAttributes({
      "aletheia.code_eval.error.code": "USER_CODE_ERROR",
      "aletheia.code_eval.error.retryable": false,
    });
    expectSpanAttributes({
      "aletheia.code_eval.lambda.status_code": 200,
      "aletheia.code_eval.lambda.function_error": "Handled",
