 ...
 * 23:45 and 00:15, and env.ALETHEIA_INGESTION_QUEUE_DELAY_MS otherwise.
...
const getDelay = (delay: number | null, source: "api" | "otel") => {
  ...
  if ((hours === 23 && minutes >= 45) || (hours === 0 && minutes <= 15)) {
    return env.ALETHEIA_INGESTION_QUEUE_DELAY_MS;
  }
  ...
  return Math.min(5000, env.ALETHEIA_INGESTION_QUEUE_DELAY_MS);
};
 ...
 * @property isAletheiaInternal - Whether the events are being ingested by Aletheia internally (e.g. traces created for prompt experiments).
...
export const processEventBatch = async (
  ...
  const {
    delay = null,
    source = "api",
    isAletheiaInternal = false,
    forwardToEventsTable,
  } = options;

  // add context of api call to the span
  const currentSpan = getCurrentSpan();
  recordIncrement("aletheia.ingestion.event", input.length, { source });
  recordDistribution("aletheia.ingestion.event_distribution", input.length, {
    source,
  });

  currentSpan?.setAttribute("aletheia.ingestion.batch_size", input.length);
  currentSpan?.setAttribute(
    "aletheia.project.id",
    authCheck.scope.projectId ?? "",
  );
  if (authCheck.scope.orgId)
    currentSpan?.setAttribute("aletheia.org.id", authCheck.scope.orgId);
  if (authCheck.scope.plan)
    currentSpan?.setAttribute("aletheia.org.plan", authCheck.scope.plan);
  ...
  const ingestionSchema = createIngestionEventSchema(isAletheiaInternal);
  ...
  const projectIdsToSkipS3List =
    env.ALETHEIA_SKIP_S3_LIST_FOR_OBSERVATIONS_PROJECT_IDS?.split(",") ?? [];
  ...
  await Promise.all(
    Object.keys(sortedBatchByEventBodyId).map(async (id) => {
      ...
      if (!isSampled) {
        recordIncrement("aletheia.ingestion.sampling", eventData.data.length, {
...
export const aggregateBatchResult = (
  ...
  errors.forEach((error) => {
    ...
    } else if (error.error instanceof AletheiaNotFoundError) {
      returnedErrors.push({
        id: error.id,
        status: 404,
