 ...
 * relative to `ALETHEIA_S3_EVENT_UPLOAD_PREFIX`.
 *
...
/**
 * Parses an S3 event-upload key (relative to `ALETHEIA_S3_EVENT_UPLOAD_PREFIX`)
...
export function buildEventBucketPrefix(params: {
  ...
  return `${env.ALETHEIA_S3_EVENT_UPLOAD_PREFIX}${params.projectId}/${params.entityType}/${safeBlobKeySegment(params.entityId)}/`;
}
...
export function rawEventBucketPrefix(params: {
  ...
  return `${env.ALETHEIA_S3_EVENT_UPLOAD_PREFIX}${params.projectId}/${params.entityType}/${params.rawEntityIdSegment}/`;
}
