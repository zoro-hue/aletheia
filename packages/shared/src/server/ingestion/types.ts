...
const PublicEnvironmentName = z
  ...
  .toLowerCase()
  .transform((val) => {
    // Strip leading "aletheia" prefix (with optional separator)
    const stripped = val.replace(/^aletheia[-_]?/, "");
 ...
 * @param isAletheiaInternal - Whether the events are being ingested by Aletheia internally (e.g. traces created for prompt experiments).
 * @returns The ingestion event schema.
 */
export const createIngestionEventSchema = (isAletheiaInternal = false) => {
  return isAletheiaInternal
