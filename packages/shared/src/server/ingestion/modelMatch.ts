...
const modelMatchLocalCache = new LocalCache<ModelWithPrices>({
  ...
  enabled: env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED === "true",
  ttlMs: getPositiveNumberOrDefault(
    env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_TTL_MS,
    DEFAULT_LOCAL_CACHE_MODEL_MATCH_TTL_MS,
  ),
  max: getPositiveNumberOrDefault(
    env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_MAX,
  ...
  return instrumentAsync(
    ...
    async (span) => {
      if (logger.isLevelEnabled("debug")) {
        logger.debug(
          formatModelMatchDebugMessage("Resolving model match", {
            ...
            model: p.model,
            localCacheEnabled:
              env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED === "true",
            redisCacheEnabled:
              env.ALETHEIA_CACHE_MODEL_MATCH_ENABLED === "true",
          }),
        );
      }
      ...
      const { source, value } = await modelMatchLocalCache.getOrLoad(
        ...
        async () => {
          ...
          if (postgresModel) {
            ...
            if (env.ALETHEIA_CACHE_MODEL_MATCH_ENABLED === "true") {
      ...
      if (source === "postgres") {
        span.setAttribute(
          "model_cache_set",
          String(env.ALETHEIA_CACHE_MODEL_MATCH_ENABLED === "true"),
        );
      }
...
const getModelWithPricesFromRedis = async (
  ...
  if (env.ALETHEIA_CACHE_MODEL_MATCH_ENABLED === "false") {
    return null;
  }
  ...
  try {
    ...
    if (!redisValue) {
      recordIncrement("aletheia.model_match.cache_miss", 1);
      return null;
    }

    recordIncrement("aletheia.model_match.cache_hit", 1);
...
const NOT_FOUND_TOKEN = "ALETHEIA_MODEL_MATCH_NOT_FOUND" as const;
...
const addModelNotFoundTokenToRedis = async (p: ModelMatchProps) => {
  try {
    ...
    await redis?.set(
      ...
      NOT_FOUND_TOKEN,
      "EX",
      env.ALETHEIA_CACHE_MODEL_MATCH_TTL_SECONDS,
    );
  } catch (error) {
    logger.error(
...
export async function clearModelCacheForProject(
  ...
  if (env.ALETHEIA_CACHE_MODEL_MATCH_ENABLED === "false" || !redis) {
    return;
  }
