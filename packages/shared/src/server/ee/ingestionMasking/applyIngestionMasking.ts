...
export function getIngestionMaskingConfig(
  ...
  const callbackUrl = e.ALETHEIA_INGESTION_MASKING_CALLBACK_URL;
  if (!callbackUrl) {
    return null;
  }
  ...
  return {
    callbackUrl,
    timeoutMs: e.ALETHEIA_INGESTION_MASKING_CALLBACK_TIMEOUT_MS,
    failClosed: e.ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED === "true",
    maxRetries: e.ALETHEIA_INGESTION_MASKING_MAX_RETRIES,
    propagatedHeaders: e.ALETHEIA_INGESTION_MASKING_PROPAGATED_HEADERS,
  };
}
...
export function isIngestionMaskingEnabled(envOverride?: SharedEnv): boolean {
  ...
  if (!isEnterpriseLicenseAvailable(envOverride)) {
    logger.warn(
      "Ingestion masking callback URL is configured but enterprise license is not available. Masking will be disabled. Ingestion masking requires Aletheia Cloud or a self-hosted enterprise license (aletheia_ee_*).",
    );
    return false;
  }
...
async function makeCallbackRequest<T>(params: {
  ...
  try {
    ...
    recordHistogram(
      "aletheia.ingestion.masking.callback_duration_ms",
      duration,
      {
...
export async function applyIngestionMasking<T>(
  ...
  for (let attempt = 1; attempt <= config.maxRetries + 1; attempt++) {
    ...
    if (result.success) {
      recordIncrement("aletheia.ingestion.masking.success", 1, {
        attempt: attempt.toString(),
      });
  ...
  recordIncrement("aletheia.ingestion.masking.failure", 1, {
