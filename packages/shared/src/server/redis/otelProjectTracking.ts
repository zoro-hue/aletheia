 ...
 * Only performs the operation if ALETHEIA_SKIP_FINAL_FOR_OTEL_PROJECTS is enabled.
 */
export async function markProjectAsOtelUser(projectId: string): Promise<void> {
  // Check if feature is enabled
  if (env.ALETHEIA_SKIP_FINAL_FOR_OTEL_PROJECTS !== "true") {
    return;
  }

  try {
    const key = `aletheia:project:${projectId}:otel:active`;
