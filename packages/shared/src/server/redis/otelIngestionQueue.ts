...
export class OtelIngestionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      { length: env.ALETHEIA_OTEL_INGESTION_QUEUE_SHARD_COUNT },
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(
            shardingKey,
            env.ALETHEIA_OTEL_INGESTION_QUEUE_SHARD_COUNT,
          )
        : 0);
...
export class SecondaryOtelIngestionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      { length: env.ALETHEIA_OTEL_INGESTION_SECONDARY_QUEUE_SHARD_COUNT },
      (_, i) =>
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(
            shardingKey,
            env.ALETHEIA_OTEL_INGESTION_SECONDARY_QUEUE_SHARD_COUNT,
          )
        : 0);
