...
export class EvalExecutionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      { length: env.ALETHEIA_EVAL_EXECUTION_QUEUE_SHARD_COUNT },
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(
            shardingKey,
            env.ALETHEIA_EVAL_EXECUTION_QUEUE_SHARD_COUNT,
          )
        : 0);
...
export class SecondaryEvalExecutionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      {
        length: env.ALETHEIA_EVAL_EXECUTION_SECONDARY_QUEUE_SHARD_COUNT,
      },
      (_, i) =>
  ...
  > | null {
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(
            shardingKey,
            env.ALETHEIA_EVAL_EXECUTION_SECONDARY_QUEUE_SHARD_COUNT,
          )
        : 0);
