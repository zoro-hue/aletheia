...
const S3_SLOWDOWN_PREFIX = "aletheia:s3-slowdown";

function isSlowdownEnabled(): boolean {
  return env.ALETHEIA_S3_RATE_ERROR_SLOWDOWN_ENABLED === "true";
}
...
export async function markProjectS3Slowdown(projectId: string): Promise<void> {
  ...
  const ttlSeconds = env.ALETHEIA_S3_RATE_ERROR_SLOWDOWN_TTL_SECONDS;
  ...
  try {
    ...
    recordIncrement("aletheia.s3_slowdown.marked", 1);
  } catch (error) {
