...
export class IngestionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      { length: env.ALETHEIA_INGESTION_QUEUE_SHARD_COUNT },
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(shardingKey, env.ALETHEIA_INGESTION_QUEUE_SHARD_COUNT)
        : 0);
...
export class SecondaryIngestionQueue {
  ...
  public static getShardNames() {
    return Array.from(
      { length: env.ALETHEIA_INGESTION_SECONDARY_QUEUE_SHARD_COUNT },
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(
            shardingKey,
            env.ALETHEIA_INGESTION_SECONDARY_QUEUE_SHARD_COUNT,
          )
        : 0);
