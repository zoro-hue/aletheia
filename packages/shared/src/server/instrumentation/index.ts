...
export const addUserToSpan = (
  ...
  if (attributes.projectId) {
    ...
    activeSpan.setAttribute("aletheia.project.id", attributes.projectId);
  }
  if (attributes.orgId) {
    baggage = baggage.setEntry("aletheia.org.id", {
      value: attributes.orgId,
    });
    activeSpan.setAttribute("aletheia.org.id", attributes.orgId);
  }
  if (attributes.plan) {
    baggage = baggage.setEntry("aletheia.org.plan", {
      value: attributes.plan,
    });
    activeSpan.setAttribute("aletheia.org.plan", attributes.plan);
  }
  if (attributes.apiKeyId) {
    baggage = baggage.setEntry("aletheia.api_key.id", {
      value: attributes.apiKeyId,
    });
    activeSpan.setAttribute("aletheia.api_key.id", attributes.apiKeyId);
  }
  if (attributes.publicKey) {
    baggage = baggage.setEntry("aletheia.api_key.public_key", {
 ...
 *
 * Example: `legacy-ingestion-queue` -> `aletheia.queue.legacy_ingestion`
 */
export const convertQueueNameToMetricName = (queueName: string): string => {
  return (
    "aletheia.queue." + queueName.replace(/-/g, "_").replace(/_queue$/, "")
  );
};
