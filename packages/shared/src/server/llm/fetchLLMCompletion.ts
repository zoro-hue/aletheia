...
const isAletheiaCloud = Boolean(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION);
...
type LLMCompletionParams = {
  ...
  shouldUseAletheiaAPIKey?: boolean;
};
...
> {
  ...
  if (traceSinkParams) {
    // Safeguard: All internal traces must use AletheiaInternalTraceEnvironment enum values
    ...
    if (!traceSinkParams.environment?.startsWith("aletheia")) {
      logger.warn(
        "Skipping trace creation: internal traces must use AletheiaInternalTraceEnvironment enum",
        {
      ...
      finalCallbacks.push(internalTracingHandler.handler as any);
    }
  }
  ...
  const timeoutMs = env.ALETHEIA_FETCH_LLM_COMPLETION_TIMEOUT_MS;
  ...
  if (modelParams.adapter === LLMAdapter.Anthropic) {
    ...
    const { region } = shouldUseAletheiaAPIKey
    ...
    const isSelfHosted = !isAletheiaCloud;
    const { credentials, clientOptions } = resolveBedrockAuth({
      secretKey: apiKey,
      allowDefaultCredentials: isSelfHosted || shouldUseAletheiaAPIKey,
    });
    ...
    const shouldUseDefaultCredentials =
      apiKey === VERTEXAI_USE_DEFAULT_CREDENTIALS && !isAletheiaCloud;
