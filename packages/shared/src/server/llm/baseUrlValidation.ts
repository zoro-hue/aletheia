...
export function llmBaseUrlWhitelistFromEnv(): LlmBaseUrlValidationWhitelist {
  ...
  return {
    hosts: env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST || [],
    ips: env.ALETHEIA_LLM_CONNECTION_WHITELISTED_IPS || [],
    ip_ranges: env.ALETHEIA_LLM_CONNECTION_WHITELISTED_IP_SEGMENTS || [],
  };
}
...
export async function validateLlmConnectionBaseURL(
  ...
  const effectiveWhitelist = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION
    ? {
        hosts: [],
        ips: [],
        ip_ranges: [],
  ...
  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION && url.protocol !== "https:") {
    throw new Error("Only HTTPS base URLs are allowed on Aletheia Cloud");
  }
}
