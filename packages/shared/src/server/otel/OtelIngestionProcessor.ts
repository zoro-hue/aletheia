...
import { AletheiaOtelSpanAttributes } from "./attributes";
 ...
 * resource spans into Aletheia ingestion events.
 *
 * Manages trace deduplication internally and provides a clean interface
 * for converting OTEL spans to Aletheia events.
 */
...
export class OtelIngestionProcessor {
  private static readonly OTEL_CONVERSION_FAILURE_METRIC =
    "aletheia.ingestion.otel.conversion_failure";
  ...
  async publishToOtelIngestionQueue(resourceSpans: ResourceSpan[]) {
    const fileKey = `${env.ALETHEIA_S3_EVENT_UPLOAD_PREFIX}otel/${this.projectId}/${this.getCurrentTimePath()}/${randomUUID()}.json`;
          ...
          .flatMap((resourceSpan) => {
            ...
            for (const scopeSpan of resourceSpan?.scopeSpans ?? []) {
              ...
              for (const span of scopeSpan?.spans ?? []) {
                ...
                const isAletheiaSDKSpans =
                  scopeSpan.scope?.name?.startsWith("aletheia-sdk") ?? false;
                ...
                recordDistribution(
                  "aletheia.ingestion.otel.event.byte_length",
                  eventBytes,
                  {
                    source: "otel",
                ...
                events.push({
                  ...
                  version:
                    spanAttributes?.[AletheiaOtelSpanAttributes.VERSION] ??
                  ...
                  level:
                    spanAttributes[
                      AletheiaOtelSpanAttributes.OBSERVATION_LEVEL
                    ] ??
                    (span.status?.code === 2
                  ...
                  statusMessage:
                    spanAttributes[
                      AletheiaOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE
                    ] ??
                    span.status?.message ??
                    null,
                  ...
                  promptName:
                    spanAttributes?.[
                      AletheiaOtelSpanAttributes.OBSERVATION_PROMPT_NAME
                    ] ??
                    spanAttributes["aletheia.prompt.name"] ??
                    this.parseAletheiaPromptFromAISDK(spanAttributes)?.name ??
                    null,
                  promptVersion:
                    spanAttributes?.[
                      AletheiaOtelSpanAttributes.OBSERVATION_PROMPT_VERSION
                    ] ??
                    spanAttributes["aletheia.prompt.version"] ??
                    this.parseAletheiaPromptFromAISDK(spanAttributes)
                      ?.version ??
                    null,
                  ...
                  traceName:
                    spanAttributes?.[AletheiaOtelSpanAttributes.TRACE_NAME] ??
                    null,
                  userId: this.extractUserId(spanAttributes),
                  sessionId: this.extractSessionId(spanAttributes),
                  release:
                    (spanAttributes?.[
                      AletheiaOtelSpanAttributes.RELEASE
                    ] as string) ??
                    resourceAttributes?.[AletheiaOtelSpanAttributes.RELEASE] ??
                    null,
  ...
  /**
   * Process resource spans and convert them to Aletheia ingestion events.
      ...
      async (span) => {
        ...
        try {
          ...
          for (const key of Object.keys(
            ...
            recordIncrement(
              "aletheia.ingestion.otel.trace_create_event",
  ...
  private processResourceSpan(
    ...
    for (const scopeSpan of resourceSpan?.scopeSpans ?? []) {
      ...
      if (isAletheiaSDKSpans) {
        recordIncrement("aletheia.otel.ingestion.aletheia_sdk_batch", 1);
      }
  ...
  private processSpan(
    ...
    const isRootSpan =
      !parentObservationId ||
      String(attributes[AletheiaOtelSpanAttributes.AS_ROOT]) === "true";
  ...
  private createTraceEvent(params: CreateTraceEventParams): IngestionEventType {
    ...
    if (isRootSpan) {
      ...
      trace = {
        ...
        version:
          (attributes?.[AletheiaOtelSpanAttributes.VERSION] as string) ??
          resourceAttributes?.["service.version"] ??
          null,
        release:
          (attributes?.[AletheiaOtelSpanAttributes.RELEASE] as string) ??
          resourceAttributes?.[AletheiaOtelSpanAttributes.RELEASE] ??
          null,
    ...
    if (hasTraceUpdates && !isRootSpan) {
      trace = {
        ...trace,
        name: attributes[AletheiaOtelSpanAttributes.TRACE_NAME] as string,
        metadata: {
          ...resourceAttributeMetadata,
          ...this.extractMetadata(attributes, "trace"),
        ...
        tags: this.extractTags(attributes),
        environment: this.extractEnvironment(attributes, resourceAttributes),
        input: attributes[AletheiaOtelSpanAttributes.TRACE_INPUT],
        output: attributes[AletheiaOtelSpanAttributes.TRACE_OUTPUT],
      };
    }
  ...
  private extractPublic(
    ...
    const value =
      attributes?.[AletheiaOtelSpanAttributes.TRACE_PUBLIC] ??
      attributes?.["aletheia.public"];
  ...
  private extractIsAppRoot(attributes?: Record<string, unknown>): boolean {
    const value = attributes?.[AletheiaOtelSpanAttributes.IS_APP_ROOT];
  ...
  private createObservationEvent(
    ...
    const observation = {
      ...
      level:
        attributes[AletheiaOtelSpanAttributes.OBSERVATION_LEVEL] ??
        (span.status?.code === 2
          ? ObservationLevel.ERROR
          : scopeSpan?.scope?.name === "livekit-agents" &&
              LIVEKIT_DEBUG_SPAN_NAMES.has(span.name)
            ...
            : ObservationLevel.DEFAULT),
      statusMessage:
        attributes[AletheiaOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE] ??
        span.status?.message ??
        null,
      version:
        attributes[AletheiaOtelSpanAttributes.VERSION] ??
        resourceAttributes?.["service.version"] ??
        null,
      modelParameters: this.extractModelParameters(
        attributes,
      ...
      promptName:
        attributes?.[AletheiaOtelSpanAttributes.OBSERVATION_PROMPT_NAME] ??
        attributes["aletheia.prompt.name"] ??
        this.parseAletheiaPromptFromAISDK(attributes)?.name ??
        null,
      promptVersion:
        attributes?.[AletheiaOtelSpanAttributes.OBSERVATION_PROMPT_VERSION] ??
        attributes["aletheia.prompt.version"] ??
        this.parseAletheiaPromptFromAISDK(attributes)?.version ??
        null,
  ...
  private hasTraceUpdates(attributes: Record<string, unknown>): boolean {
    const hasExactMatchingAttributeName = [
      AletheiaOtelSpanAttributes.TRACE_NAME,
      AletheiaOtelSpanAttributes.TRACE_INPUT,
      AletheiaOtelSpanAttributes.TRACE_OUTPUT,
      AletheiaOtelSpanAttributes.TRACE_METADATA,
      AletheiaOtelSpanAttributes.TRACE_USER_ID,
      AletheiaOtelSpanAttributes.TRACE_SESSION_ID,
      AletheiaOtelSpanAttributes.TRACE_PUBLIC,
      AletheiaOtelSpanAttributes.TRACE_TAGS,
      AletheiaOtelSpanAttributes.TRACE_COMPAT_USER_ID,
      AletheiaOtelSpanAttributes.TRACE_COMPAT_SESSION_ID,
      // OpenAI and Langchain integrations
      `${AletheiaOtelSpanAttributes.OBSERVATION_METADATA}.aletheia_user_id`,
      `${AletheiaOtelSpanAttributes.OBSERVATION_METADATA}.aletheia_session_id`,
      `${AletheiaOtelSpanAttributes.OBSERVATION_METADATA}.aletheia_tags`,
      `${AletheiaOtelSpanAttributes.TRACE_METADATA}.aletheia_session_id`,
      `${AletheiaOtelSpanAttributes.TRACE_METADATA}.aletheia_user_id`,
      `${AletheiaOtelSpanAttributes.TRACE_METADATA}.aletheia_tags`,
      // Vercel AI SDK
    ...
    const hasTraceMetadataKey = attributeKeys.some((key) =>
      key.startsWith(AletheiaOtelSpanAttributes.TRACE_METADATA),
    );
  ...
  private parseJsonObjectAttribute(
    ...
    if (typeof raw !== "string") {
      ...
      recordIncrement("aletheia.ingestion.otel.bad_json_attribute", 1, {
        ...metricTags,
        reason: "non_string",
      });
      return {};
  ...
  private logOversizedSpan(
    ...
    const maxBytes = env.ALETHEIA_OTEL_MAX_SPAN_BYTES;
    ...
    recordIncrement("aletheia.ingestion.otel.oversized_span", 1, {
      project_id: this.projectId,
    });
  }
  ...
  private extractInputAndOutput(params: {
    ...
    const potentialInputOutputKeys = [
      ...
      AletheiaOtelSpanAttributes.TRACE_INPUT,
      AletheiaOtelSpanAttributes.TRACE_OUTPUT,
      AletheiaOtelSpanAttributes.OBSERVATION_INPUT,
      AletheiaOtelSpanAttributes.OBSERVATION_OUTPUT,
      // Vercel AI SDK
      "ai.prompt.messages",
      "ai.prompt",
      "ai.toolCall.args",
    ...
    Object.keys(attributes).forEach((key) => {
      if (
        ...
        key.startsWith("aletheia.trace.metadata") ||
        key.startsWith("aletheia.observation.metadata") ||
    ...
    // Aletheia
    input =
      domain === "trace" && attributes[AletheiaOtelSpanAttributes.TRACE_INPUT]
        ? attributes[AletheiaOtelSpanAttributes.TRACE_INPUT]
        : attributes[AletheiaOtelSpanAttributes.OBSERVATION_INPUT];
    output =
      domain === "trace" && attributes[AletheiaOtelSpanAttributes.TRACE_OUTPUT]
        ? attributes[AletheiaOtelSpanAttributes.TRACE_OUTPUT]
        : attributes[AletheiaOtelSpanAttributes.OBSERVATION_OUTPUT];
  ...
  private extractEnvironment(
    ...
    const environmentAttributeKeys = [
      AletheiaOtelSpanAttributes.ENVIRONMENT,
  ...
  private extractMetadata(
    ...
    const metadataKeyPrefix =
      domain === "observation"
        ? AletheiaOtelSpanAttributes.OBSERVATION_METADATA
        : AletheiaOtelSpanAttributes.TRACE_METADATA;

    const topLevelMetadata = this.parseMetadataAttribute(
      attributes[metadataKeyPrefix] || attributes["aletheia.metadata"],
    );
    const aletheiaMetadata = this.extractPrefixedMetadataAttributes({
      attributes,
      prefixes: [
        metadataKeyPrefix,
        "aletheia.metadata",
      ...
      excludedKeys: new Set([
        ...
        "ai.telemetry.metadata.sessionId",
        "ai.telemetry.metadata.tags",
        "ai.telemetry.metadata.aletheiaPrompt",
      ]),
    });
  ...
  private extractModelParameters(
    ...
    if (attributes[AletheiaOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS]) {
      try {
        return this.sanitizeModelParams(
          JSON.parse(
            attributes[
              AletheiaOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS
            ] as string,
          ),
        );
      } catch {
  ...
  private sanitizeModelParams<T>(params: T): Record<string, string> | T {
    // Model params in Aletheia must be key value pairs where value is string
  ...
  private extractModelName(
    ...
    const modelNameKeys = [
      AletheiaOtelSpanAttributes.OBSERVATION_MODEL,
  ...
  private extractUsageDetails(
    ...
    const fromAttribute = this.parseJsonObjectAttribute(
      attributes,
      AletheiaOtelSpanAttributes.OBSERVATION_USAGE_DETAILS,
      context,
    );
  ...
  private extractCostDetails(
    ...
    const fromAttribute = this.parseJsonObjectAttribute(
      attributes,
      AletheiaOtelSpanAttributes.OBSERVATION_COST_DETAILS,
      context,
    );
    if (fromAttribute !== null) return fromAttribute;
  ...
  private extractCompletionStartTime(
    ...
    try {
      const value = attributes[
        AletheiaOtelSpanAttributes.OBSERVATION_COMPLETION_START_TIME
      ] as any;
  ...
  private extractTags(attributes: Record<string, unknown>): string[] {
    const tagsValue =
      attributes[AletheiaOtelSpanAttributes.TRACE_TAGS] ||
      attributes["aletheia.tags"] ||
      attributes[
        `${AletheiaOtelSpanAttributes.OBSERVATION_METADATA}.aletheia_tags`
      ] ||
      attributes[
  ...
  private extractExperimentFields(attributes: Record<string, unknown>): {
    ...
    const experimentId = attributes[AletheiaOtelSpanAttributes.EXPERIMENT_ID];
    const experimentName =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_NAME];
    const experimentDescription =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_DESCRIPTION];
    const experimentDatasetId =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_DATASET_ID];
    const experimentItemId =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_ITEM_ID];
    const experimentItemRootSpanId =
      attributes[
        AletheiaOtelSpanAttributes.EXPERIMENT_ITEM_ROOT_OBSERVATION_ID
      ];
    const experimentItemExpectedOutput =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_ITEM_EXPECTED_OUTPUT];
    const experimentItemVersion =
      attributes[AletheiaOtelSpanAttributes.EXPERIMENT_ITEM_VERSION];

    // Extract experiment metadata
    const experimentMetadataFlattened = flattenJsonToPathArrays(
      this.extractMetadataFromPrefix({
        attributes,
        prefix: AletheiaOtelSpanAttributes.EXPERIMENT_METADATA,
      }),
    );
    ...
    const experimentItemMetadataFlattened = flattenJsonToPathArrays(
      this.extractMetadataFromPrefix({
        attributes,
        prefix: AletheiaOtelSpanAttributes.EXPERIMENT_ITEM_METADATA,
      }),
    );
  ...
  private parseAletheiaPromptFromAISDK(
    attributes: Record<string, unknown>,
  ): { name: string; version: number } | undefined {
    const aiSDKPrompt = attributes["ai.telemetry.metadata.aletheiaPrompt"];
    ...
    try {
      const results = await Promise.all(
        [...traceIds].map(async (traceId) => {
          const key = `aletheia:project:${this.projectId}:trace:${traceId}:seen`;
