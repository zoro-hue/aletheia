...
/**
 * Parsed Aletheia media reference.
 * Extracted from magic string format: @@@aletheiaMedia:type=X|id=Y|source=Z@@@
 */
...
/**
 * Schema that parses Aletheia media reference magic strings.
 * Format: @@@aletheiaMedia:type=image/jpeg|id=<uuid>|source=base64@@@
 *
...
export const MediaReferenceStringSchema = z
  ...
  .transform((str, ctx) => {
    ...
    const magicStringPattern = /^@@@aletheiaMedia:(.*)@@@$/;

    const match = str.match(magicStringPattern);
    if (!match) {
      ctx.addIssue({
        code: "custom",
        message: "Invalid aletheiaMedia magic string format",
      });
      return z.NEVER;
    }
 ...
 * Supports URL, Aletheia media reference, or base64-encoded images.
 */
