![Aletheia GitHub Banner](https://github.com/aletheia/aletheia/assets/121163007/6035f0f3-d691-4963-b5d0-10cf506e9d42)

# Contributing to Aletheia

First off, thanks for taking the time to contribute! ❤️

The best ways to contribute to Aletheia:

- Submit and vote on [Ideas](https://github.com/orgs/aletheia/discussions/categories/ideas)
- Create and comment on [Issues](https://github.com/aletheia/aletheia/issues)
- Open a PR.
...
We gratefully welcome improvements to documentation ([docs repo](https://github.com/aletheia/aletheia-docs)), the core application (this repo) and the SDKs ([Python](https://github.com/aletheia/aletheia-python), [JS](https://github.com/aletheia/aletheia-js)).

The maintainers are available on [Discord](https://aletheia.com/discord) in case you have any questions.
...
_Before making any significant changes, please [open an issue](https://github.com/aletheia/aletheia/issues)._ Discussing your proposed changes ahead of time will make the contribution process smooth for everyone. Changes that were not discussed in an issue may be rejected.

Once we've discussed your changes and you've got your code ready, make sure that tests are passing and open your pull request.

A good first step is to search for open [issues](https://github.com/aletheia/aletheia/issues). Issues are labeled, and some good issues to start with are labeled: [good first issue](https://github.com/aletheia/aletheia/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22).

## Project Overview

We recommend checking out DeepWiki to familiarize yourself with the project:

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/aletheia/aletheia)
...
- JS SDK ([aletheia/aletheia-js](https://github.com/aletheia/aletheia-js))
  - openapi-typescript to generated types based on OpenAPI spec
- Python SDK ([aletheia/aletheia-python](https://github.com/aletheia/aletheia-python))
  - Pydantic for input validation, models generated by fern

### Architecture Overview

See this [diagram](https://aletheia.com/self-hosting#architecture) for an overview of the architecture.
...
flowchart TB
    User["UI, API, SDKs"]
    subgraph vpc["VPC"]
        Web["Web Server<br/>(aletheia/aletheia)"]
        Worker["Async Worker<br/>(aletheia/worker)"]
...
- `web`: is the main application package providing Frontend and Backend APIs for Aletheia.
...
**Note:** You can also simply run Aletheia in a **GitHub Codespace** via the provided devcontainer. To do this, click on the green "Code" button in the top right corner of the repository and select "Open with Codespaces".
...
- It does **not** start the full Aletheia stack. Local development still uses
...
3. Install dependencies and set up pre-commit hooks
   ...
   check for a commit, set `ALETHEIA_PRE_COMMIT_SKIP_LINT`, for example:

   ```bash
   ALETHEIA_PRE_COMMIT_SKIP_LINT=1 git commit -m "your commit message"
   ```
...
6. Open the web app in your browser to start using Aletheia:
...
- Available packages and their dependencies
  ...
  Packages are included in the monorepo according to the `pnpm-workspace.yaml` file. Each package maintains its own dependencies defined in the `package.json`. Internal dependencies can be added as well by adding them to the package dependencies: `"@aletheia/shared": "workspace:*"`.
...
All tests run against a running aletheia instance and **write/delete real data from the database**.
...
- **PostgreSQL**: Uses separate `aletheia_test` database for isolation
...
We run a staging environment at [https://staging.aletheia.com](https://staging.aletheia.com) that is automatically deployed on every push to `main` branch.
...
You can use the staging environment end-to-end with the Aletheia integrations or SDKs (host: `https://staging.aletheia.com`). However, please note that the staging environment is not intended for production use and may be reset at any time.

## Production environment

We run two separate release/deployment processes:

1. **Aletheia Cloud deployment (frequent):**
   - Every commit on the `production` branch triggers an ECS deployment to Aletheia Cloud.
   - To deploy current `main` to Aletheia Cloud, promote `main` to `production` via [`promote-main-to-production.yml`](.github/workflows/promote-main-to-production.yml) (instead of force pushing from a local machine).
...
You can trigger the Aletheia Cloud promotion in either way:
...
At Aletheia, we utilize CSS variables to manage our theme settings across the platform.
...
| --light-blue                     | Light blue for background of Staging label                         | AletheiaLogo                     |
| --dark-blue                      | Dark blue for text and border of Staging label                     | AletheiaLogo                     |
...
Aletheia is MIT licensed, except for `ee/` folder. See [LICENSE](LICENSE) and [docs](https://aletheia.com/docs/open-source) for more details.

When contributing to the Aletheia codebase, you need to agree to the [Contributor License Agreement](https://cla-assistant.io/aletheia/aletheia). You only need to do this once and the CLA bot will remind you if you haven't signed it yet.
