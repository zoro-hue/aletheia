...
| --- | --- | --- |
| Public REST API | `web/src/pages/api/public/**` | Request span: `service:web resource_name:"GET /api/public/<path>"`; tenant span: `resource_name:api-auth-verify` with `@aletheia.project.id` / `@aletheia.org.id` |
...
`resource_name:api-auth-verify` by `@aletheia.project.id` or
`@aletheia.org.id`, then open representative trace IDs and inspect the request
...
| Logger / instrumentation | `packages/shared/src/server/logger.ts`, `packages/shared/src/server/instrumentation.ts` | log silently dropped because `ALETHEIA_LOG_LEVEL` set wrong, or span missing because handler doesn't call `instrumentAsync` |
