...
description: |
  ...
  Datadog (APM, logs, metrics) with the Aletheia repo to establish a
...
4. **Cluster the errors.** Group by `(projectId, error.message)` or
   ...
   hypothesis — Aletheia incidents commonly have *multiple* coexisting root
   causes, not one.
...
- Log search filter pattern:
  `service:worker env:prod-eu @aletheia.project.id:cm1r6u… status:error`
