...
name: seed-test-data
description: |
  Seed local Aletheia test data with one command: large/branching observation
...
One-shot deterministic test data for local Aletheia. The CLI handles env
...
- Default project is the seeded `7a88fb47-b4e2-43b8-a06c-a5ce950dc53a`
  (login `demo@aletheia.com` / `password`); override with `--project`.
- Open the printed `links` in the browser to verify visually. The v4
  ...
  `ALETHEIA_MIGRATION_V4_WRITE_MODE=events_only` server-side.
