...
Any Aletheia code path that issues an outbound HTTP request to a URL derived
...
internal targets in Aletheia's deployment topology include:
...
3. Define the env allowlist trio for self-hosted users who legitimately point
   ...
   `ALETHEIA_WEBHOOK_WHITELISTED_HOST/IPS/IP_SEGMENTS`,
   `ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST/IPS/IP_SEGMENTS`, and
   `ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_HOST/IPS/IP_SEGMENTS`). Do
...
- Cloud (`NEXT_PUBLIC_ALETHEIA_CLOUD_REGION` set) forces strict mode.
...
- Self-hosted reads the per-surface env trio:
  - `ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST/IPS/IP_SEGMENTS`
  - `ALETHEIA_WEBHOOK_WHITELISTED_HOST/IPS/IP_SEGMENTS`
  - `ALETHEIA_BLOB_STORAGE_ENDPOINT_WHITELISTED_HOST/IPS/IP_SEGMENTS`
