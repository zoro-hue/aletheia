...
Complete guide to middleware patterns in Aletheia's Next.js + tRPC architecture.
...
tRPC middleware in Aletheia is composable and type-safe. Each middleware enriches the context and provides guarantees to subsequent middleware.
...
Propagates OpenTelemetry context with Aletheia-specific baggage:

```typescript
const withOtelInstrumentation = t.middleware(async (opts) => {
  const actualInput = await opts.getRawInput();

  const baggageCtx = contextWithAletheiaProps({
...
Aletheia exports composed procedures with middleware chains:
...
export function withMiddlewares(handlers: Handlers) {
  return async (req: NextApiRequest, res: NextApiResponse) => {
    const ctx = contextWithAletheiaProps({ headers: req.headers });
...
async function verifyAdminApiKeyAuth(req: NextApiRequest) {
  ...
  // 2. x-aletheia-admin-api-key: <ADMIN_API_KEY>
  // 3. x-aletheia-project-id: <project-id>

  if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) {
    throw {
      status: 403,
      message: "Admin API key auth not available on Aletheia Cloud",
    };
  }
  ...
  const projectId = req.headers["x-aletheia-project-id"];
...
All requests (tRPC and public API) propagate OpenTelemetry context with Aletheia-specific baggage.
...
// Create context with Aletheia baggage
const ctx = contextWithAletheiaProps({
  headers: req.headers,
  userId: session?.user?.id,
