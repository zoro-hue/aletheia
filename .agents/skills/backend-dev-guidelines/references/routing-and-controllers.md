...
Complete guide to routing and separation of concerns in Aletheia's Next.js + tRPC architecture.
...
Aletheia uses a **layered architecture** with clear separation of concerns:
...
import { paginationZod, singleFilter, orderBy } from "@aletheia/shared";
import {
  getScoresUiTable,
  getScoresUiCount,
  upsertScore,
...
import { eventTypes, processEventBatch } from "@aletheia/shared/src/server";
...
imported from `@aletheia/shared` originate there.
...
import { _handleGetScoreById } from "@aletheia/shared/src/server";
...
export async function createScoreWithValidation({
  ...
  if (!config) {
    throw new AletheiaNotFoundError("Score config not found");
  }
