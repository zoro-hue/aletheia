...
Complete guide to database access patterns in Aletheia using PostgreSQL (Prisma ORM) and ClickHouse (direct client).
...
Aletheia uses a **dual database architecture**:
...
**Important**: Always import from `@aletheia/shared/src/db`, not `@prisma/client` directly.
...
```typescript
import { queryClickhouse } from "@aletheia/shared/src/server/repositories/clickhouse";
import { clickhouseClient } from "@aletheia/shared/src/server/clickhouse/client";
```
...
```typescript
import { queryClickhouseStream } from "@aletheia/shared/src/server/repositories/clickhouse";
...
```typescript
import { upsertClickhouse } from "@aletheia/shared/src/server/repositories/clickhouse";
...
```typescript
import { commandClickhouse } from "@aletheia/shared/src/server/repositories/clickhouse";
...
```typescript
import { convertDateToClickhouseDateTime } from "@aletheia/shared/src/server/clickhouse/client";
...
- Aletheia is multi-tenant - each project's data must be isolated
...
} from "@aletheia/shared/src/server/repositories/clickhouse";
...
Aletheia uses repositories in `packages/shared/src/server/repositories/` for complex data access patterns.
...
ALETHEIA_CLICKHOUSE_QUERY_MAX_ATTEMPTS: z.coerce.number().positive().default(3);
```
