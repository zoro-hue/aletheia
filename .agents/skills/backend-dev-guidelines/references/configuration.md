...
Complete guide to managing configuration across Aletheia's monorepo packages.
...
const region = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
```
...
const concurrency = env.ALETHEIA_INGESTION_QUEUE_PROCESSING_CONCURRENCY;
const s3Bucket = env.ALETHEIA_S3_EVENT_UPLOAD_BUCKET;
```
...
```typescript
import { env } from "@aletheia/shared/src/env";
...
```typescript
import { env } from "@aletheia/ee/src/env";
...
**Purpose:** Identifies the cloud deployment region for Aletheia Cloud.
...
| **Aletheia Cloud US**    | `"US"`                 | Production US region                           |
| **Aletheia Cloud EU**    | `"EU"`                 | Production EU region                           |
| **Aletheia Cloud HIPAA** | `"HIPAA"`              | HIPAA-compliant region                         |
| **Aletheia Cloud JP**    | `"JP"`                 | Production JP region                           |
...
if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION) {
...
if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION === "HIPAA") {
  // HIPAA compliance features
}

// Development/staging checks
if (env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION === "DEV") {
...
NEXT_PUBLIC_ALETHEIA_CLOUD_REGION=DEV

# Cloud US deployment
NEXT_PUBLIC_ALETHEIA_CLOUD_REGION=US

# Self-hosted OSS deployment
# (variable not set)
```

### ALETHEIA_EE_LICENSE_KEY
...
| ------------------- | ------------------ | ---------------------------------------------------------------- |
| **Aletheia Cloud**  | Not set            | Cloud features controlled by `NEXT_PUBLIC_ALETHEIA_CLOUD_REGION` |
...
When `ALETHEIA_EE_LICENSE_KEY` is set and valid:
...
if (env.ALETHEIA_EE_LICENSE_KEY) {
  // Validate license
  const isValidLicense = await validateEELicense(env.ALETHEIA_EE_LICENSE_KEY);
...
# ALETHEIA_EE_LICENSE_KEY not set

# EE self-hosted
ALETHEIA_EE_LICENSE_KEY=ee_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

# Aletheia Cloud (uses region instead)
NEXT_PUBLIC_ALETHEIA_CLOUD_REGION=US
# ALETHEIA_EE_LICENSE_KEY not used
```
...
ALETHEIA_LOG_PROPAGATED_HEADERS: z.string().optional().transform((s) =>
  s ? s.split(",").map((s) => s.toLowerCase().trim()) : []
),

// Parse project:rate pairs
ALETHEIA_INGESTION_PROCESSING_SAMPLED_PROJECTS: z.string().optional().transform((val) => {
