# Architecture Overview - Aletheia Backend

Complete guide to the layered architecture pattern used in Aletheia's Next.js/tRPC/Express monorepo. Check package manifests such as `web/package.json` for current framework versions before version-sensitive work.
...
Aletheia uses a **three-layer architecture** with two primary entry points (tRPC and Public API) plus async processing via Worker.
...
| ------------------------------------------ | --------------------- | ---------------------------------------------------------------------------------- |
| `@aletheia/shared`                         | ✅ Frontend + Backend | Prisma types, Zod schemas, constants, table definitions, domain models, utilities  |
| `@aletheia/shared/src/db`                  | 🔒 Backend only       | Prisma client instance                                                             |
| `@aletheia/shared/src/server`              | 🔒 Backend only       | Services, repositories, queues, auth, ClickHouse, LLM integration, instrumentation |
| `@aletheia/shared/src/server/auth/apiKeys` | 🔒 Backend only       | API key management (separated to avoid circular deps)                              |
| `@aletheia/shared/encryption`              | 🔒 Backend only       | Database field encryption/decryption                                               |
...
import { encrypt, decrypt } from "@aletheia/shared/encryption";
```
...
import { prisma } from "@aletheia/shared/src/db";
import { instrumentAsync, traceException } from "@aletheia/shared/src/server";
...
Aletheia uses two databases with different purposes:
...
import { getTracesTable } from "@aletheia/shared/src/server";
...
import { redis } from "@aletheia/shared/src/server";
...
Aletheia uses repositories in `packages/shared/src/server/repositories/` for complex data access patterns. Repositories provide:
...
**Aletheia uses OpenTelemetry for backend observability, with traces and logs sent to DataDog.**
