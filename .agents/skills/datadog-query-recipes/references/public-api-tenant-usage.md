...
- `@aletheia.project.id`: project-scoped API key target.
- `@aletheia.org.id`: owning org or org-scoped key target.
- `@aletheia.org.plan`: plan when present.
...
```text
@aletheia.project.id:<id> @http.path_group:/api/public/observations
```
...
```text
env:<env> @aletheia.org.id:<orgId> resource_name:api-auth-verify
env:<env> @aletheia.project.id:<projectId> resource_name:api-auth-verify
```

Group by `service`, `@aletheia.project.id`, and optionally a daily interval.

2. Fetch representative matching auth spans with `search_datadog_spans`.
   Include custom attributes such as `aletheia.*`, then copy representative
   `traceid` values.

3. Open those traces with `get_datadog_trace`. Request service-entry spans and
   include HTTP and Aletheia attributes:

```text
only_service_entry_spans: true
extra_fields: ["http.*", "next.*", "aletheia.*"]
```
