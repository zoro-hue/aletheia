...
Use this reference when a task asks which Aletheia queues exist, whether a
...
| `ALETHEIA_S3_CORE_DATA_EXPORT_IS_ENABLED` | `core-data-s3-export-queue` |
| `ALETHEIA_POSTGRES_METERING_DATA_EXPORT_IS_ENABLED` | `metering-data-postgres-export-queue` |
...
```text
aletheia.queue.<queue-name-with-hyphens-replaced-by-underscores-and-trailing-_queue-removed>
```
...
| Queue | Metric base |
| --- | --- |
| `ingestion-queue` | `aletheia.queue.ingestion` |
| `otel-ingestion-queue` | `aletheia.queue.otel_ingestion` |
| `secondary-otel-ingestion-queue` | `aletheia.queue.secondary_otel_ingestion` |
| `evaluation-execution-queue` | `aletheia.queue.evaluation_execution` |
| `trace-upsert` | `aletheia.queue.trace_upsert` |
| `batch-export-queue` | `aletheia.queue.batch_export` |
...
`aletheia.queue.clickhouse_writer.*` metrics, but it is not a `QueueName`
consumer.
