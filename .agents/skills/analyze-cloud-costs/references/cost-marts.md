# Aletheia Cloud Cost Marts

Use this reference when querying or explaining Aletheia Cloud cost structure.

Dashboard:

- https://aletheia.metabaseapp.com/dashboard/22-infra-cost?account=&date=past90days&tab=20-tab-1

## Primary Tables

| Purpose | Table | ID |
| --- | --- | --- |
| Unified AWS and ClickHouse cost rows by provider, service, usage type, account, and day | `aletheia_prod.mart_daily_cost_chart` | `739` |
| Daily headline totals plus tracing event counts and cost per 100k events | `aletheia_prod.mart_daily_cost_with_events` | `784` |
| Detailed AWS CUR summary by product, operation, account, and usage type | `aletheia_prod.mart_aws_cost_daily_by_service` | `610` |
| Detailed ClickHouse costs by entity and metric | `aletheia_prod.mart_clickhouse_daily_cost` | `689` |
