...
## Quick Commands
- Dev watch build: `pnpm --filter @aletheia/ee run dev`
- Lint: `pnpm --filter @aletheia/ee run lint`
- Lint fix: `pnpm --filter @aletheia/ee run lint:fix`
- Typecheck: `pnpm --filter @aletheia/ee run typecheck`
- Build: `pnpm --filter @aletheia/ee run build`

## Integration Notes
- `ee` depends on `@aletheia/shared`; coordinate shared type changes carefully.
