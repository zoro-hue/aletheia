...
ensure_minio_running() {
  ...
  if ! mc mb --ignore-existing local/aletheia >/dev/null 2>&1; then
    if [ "$minio_already_running" = "true" ]; then
      echo "Failed to reconcile MinIO bucket 'aletheia'; will retry on next run."
      return 0
    fi

    echo "Failed to create MinIO bucket 'aletheia' after fresh startup."
    exit 1
  fi
}
