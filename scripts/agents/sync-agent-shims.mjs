...
const isMatchingSymlink = (path, target) => {
  ...
  if (!stats.isSymbolicLink()) {
    if (!stats.isDirectory()) {
      try {
        return readFileSync(path, "utf8") === readFileSync(target, "utf8");
      } catch {
        return false;
      }
    }
...
for (const output of symlinkOutputs) {
  ...
  try {
    if (process.platform === "win32") {
      if (lstatSync(output.target).isDirectory()) {
        symlinkSync(output.target, output.path, "junction");
        console.log(`Junctioned ${output.path} -> ${output.target}`);
      } else {
        try {
          linkSync(output.target, output.path);
          console.log(`Hardlinked ${output.path} -> ${output.target}`);
        } catch {
          writeFileSync(output.path, readFileSync(output.target));
          console.log(`Copied fallback file ${output.path} -> ${output.target}`);
        }
      }
    } else {
      symlinkSync(
    ...
    const isDir = lstatSync(output.target).isDirectory();
    if (isDir) {
      ...
      copyDir(output.target, output.path);
      console.log(`Copied fallback directory ${output.path}`);
    } else {
      writeFileSync(output.path, readFileSync(output.target));
      console.log(`Copied fallback file ${output.path}`);
    }
  }
}
