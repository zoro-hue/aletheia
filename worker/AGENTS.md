...
- Prefer `@aletheia/shared/src/server` in worker runtime code for queue
...
- Use `@aletheia/shared` for cross-runtime types, schemas, domain contracts,
  model-pricing helpers, and other frontend-safe utilities.
- Use `@aletheia/shared/src/db` only when worker code or tests need direct
  Prisma access.
- Use narrower subpaths such as `@aletheia/shared/src/env` or
  `@aletheia/shared/encryption` when you specifically need those focused
