...
import { decrypt, createSignatureHeader } from "@aletheia/shared/encryption";
...
} from "@aletheia/shared/monitors/server";
  ...
  try {
    ...
    await backOff(
      async () => {
        ...
        }, env.ALETHEIA_WEBHOOK_TIMEOUT_MS);
        ...
        try {
          ...
          const redirectOptions = skipValidation
            ? {
                maxRedirects: env.ALETHEIA_WEBHOOK_MAX_REDIRECTS,
          ...
          if (error instanceof Error && error.name === "AbortError") {
            logger.warn(
              `Webhook timeout after ${env.ALETHEIA_WEBHOOK_TIMEOUT_MS}ms for url ${url} and project ${projectId}`,
            );
            throw new Error(
              `Webhook timeout after ${env.ALETHEIA_WEBHOOK_TIMEOUT_MS}ms for url ${url} and project ${projectId}`,
            );
    ...
    const shouldRetryJob =
      error instanceof AletheiaNotFoundError ||
...
async function executeWebhookAction({
  ...
  try {
    ...
    requestHeaders["x-aletheia-signature"] = signature;
  } catch (error) {
    logger.error(
