...
} from "@aletheia/shared/src/server/ee/ingestionMasking";
import {
  env,
 ...
 *
 * Requirements:
 * - x-aletheia-sdk-name "python" with x-aletheia-sdk-version >= 4.0.0
 * - x-aletheia-sdk-name "javascript" with x-aletheia-sdk-version >= 5.0.0
 * - x-aletheia-ingestion-version === "4" (custom OTel exporter opt-in)
 */
export function checkHeaderBasedDirectWrite(params: {
  ...
  // Check x-aletheia-ingestion-version (>= 4 means direct write eligible).
  ...
  // Check Aletheia SDK name + version
 ...
 *
 * Requirements:
 * - Scope name must contain 'aletheia' (case-insensitive)
...
export function checkSdkVersionRequirements(
  ...
  // Must be a Aletheia SDK
  if (!scopeName || !String(scopeName).toLowerCase().includes("aletheia")) {
    return false;
  }
...
export const otelIngestionQueueProcessorBuilder = (
  ...
  const projectIdsToRedirectToSecondaryQueue =
    env.ALETHEIA_SECONDARY_OTEL_INGESTION_QUEUE_ENABLED_PROJECT_IDS?.split(
      ",",
    ) ?? [];
  ...
  return async (
    ...
    try {
      ...
      recordHistogram(
        "aletheia.ingestion.s3_file_size_bytes",
      ...
      recordIncrement("aletheia.ingestion.event", observations.length, {
        source: "otel",
      });
      // Record more stats specific to the Otel processing
      recordDistribution("aletheia.ingestion.otel.trace_count", traces.length);
      recordDistribution(
        "aletheia.ingestion.otel.observation_count",
        observations.length,
      );
      span?.setAttribute("aletheia.ingestion.otel.trace_count", traces.length);
      span?.setAttribute(
        "aletheia.ingestion.otel.observation_count",
        observations.length,
      );
      ...
      //   - x-aletheia-sdk-name/version: Python >= 4.0.0 or JS >= 5.0.0
      //   - x-aletheia-ingestion-version: "4" (custom OTel exporter opt-in)
      ...
      //   - scope.name contains "aletheia", sdk-experiment environment, Python >= 3.9.0 or JS >= 4.4.0
      ...
      //   ALETHEIA_MIGRATION_V4_NATIVE_OTEL_BEHAVIOUR=direct forces every batch
      ...
      span?.setAttribute("aletheia.ingestion.otel.write_path", writePath);
      recordIncrement("aletheia.ingestion.otel.write_path", 1, {
        path: writePath,
      });
      ...
      if (skipLegacyWrites) {
        span?.setAttribute(
          "aletheia.ingestion.otel.skipped_legacy_writes",
          true,
        );
        recordIncrement("aletheia.ingestion.otel.skipped_legacy_writes", 1);
      } else {
