...
const getS3StorageServiceClient = (bucketName: string): StorageService => {
  if (!s3StorageServiceClient) {
    s3StorageServiceClient = StorageServiceFactory.getInstance({
      bucketName,
      accessKeyId: env.ALETHEIA_S3_CORE_DATA_UPLOAD_ACCESS_KEY_ID,
      secretAccessKey: env.ALETHEIA_S3_CORE_DATA_UPLOAD_SECRET_ACCESS_KEY,
      endpoint: env.ALETHEIA_S3_CORE_DATA_UPLOAD_ENDPOINT,
      region: env.ALETHEIA_S3_CORE_DATA_UPLOAD_REGION,
      forcePathStyle:
        env.ALETHEIA_S3_CORE_DATA_UPLOAD_FORCE_PATH_STYLE === "true",
      awsSse: env.ALETHEIA_S3_CORE_DATA_UPLOAD_SSE,
      awsSseKmsKeyId: env.ALETHEIA_S3_CORE_DATA_UPLOAD_SSE_KMS_KEY_ID,
    });
  }
...
export const coreDataS3ExportProcessor: Processor = async (): Promise<void> => {
  if (!env.ALETHEIA_S3_CORE_DATA_UPLOAD_BUCKET) {
    logger.error("No bucket name provided for core data S3 export");
    throw new Error(
      "Must provide ALETHEIA_S3_CORE_DATA_UPLOAD_BUCKET to use core data S3 exports",
    );
  }
  ...
  const s3Client = getS3StorageServiceClient(
    env.ALETHEIA_S3_CORE_DATA_UPLOAD_BUCKET,
  );
  ...
  await Promise.all(
    Object.entries({
      ...
      s3Client.uploadFile({
        fileName: `${env.ALETHEIA_S3_CORE_DATA_UPLOAD_PREFIX}${key}.jsonl`,
        fileType: "application/x-ndjson",
  ...
  await uploadPromptsCoreDataJsonl({
    s3Client,
    uploadPrefix: env.ALETHEIA_S3_CORE_DATA_UPLOAD_PREFIX,
  });
