...
export const ingestionQueueProcessorBuilder = (
  ...
  const projectIdsToRedirectToSecondaryQueue =
    env.ALETHEIA_SECONDARY_INGESTION_QUEUE_ENABLED_PROJECT_IDS?.split(",") ??
    [];
  ...
  return async (job: Job<TQueueJobTypes[QueueName.IngestionQueue]>) => {
    try {
      ...
      // never disagree, even if `ALETHEIA_S3_EVENT_KEY_MAX_SEGMENT_BYTES`
      ...
      if (
        env.ALETHEIA_ENABLE_REDIS_SEEN_EVENT_CACHE === "true" &&
        redis &&
        job.data.payload.data.fileKey
      ) {
        const key = `aletheia:ingestion:recently-processed:${job.data.payload.authCheck.scope.projectId}:${job.data.payload.data.type}:${job.data.payload.data.eventBodyId}:${job.data.payload.data.fileKey}`;
        const exists = await redis.exists(key);
        if (exists) {
          recordIncrement("aletheia.ingestion.recently_processed_cache", 1, {
      ...
      if (shouldSkipS3List) {
        ...
        recordHistogram("aletheia.ingestion.s3_file_size_bytes", fileSize, {
          skippedS3List: "true",
        });
        ...
        const S3_CONCURRENT_READS = env.ALETHEIA_S3_CONCURRENT_READS;
      ...
      recordDistribution(
        "aletheia.ingestion.count_files_distribution",
        eventFiles.length,
        {
      ...
      span?.setAttribute(
        "aletheia.ingestion.event.count_files",
        eventFiles.length,
      );
      span?.setAttribute("aletheia.ingestion.event.kind", clickhouseEntityType);
      span?.setAttribute(
        "aletheia.ingestion.s3_all_files_size_bytes",
      ...
      if (env.ALETHEIA_ENABLE_REDIS_SEEN_EVENT_CACHE === "true" && redis) {
        try {
          await Promise.all(
            eventFiles
              ...
              .map((key) =>
                redis!.set(
                  `aletheia:ingestion:recently-processed:${job.data.payload.authCheck.scope.projectId}:${job.data.payload.data.type}:${job.data.payload.data.eventBodyId}:${key.replace(".json", "")}`,
                  "1",
                  "EX",
                  60 * 5, // 5 minutes
                ),
