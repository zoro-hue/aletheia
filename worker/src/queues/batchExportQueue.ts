...
import { traceException, logger } from "@aletheia/shared/src/server";
import { QueueName, TQueueJobTypes } from "@aletheia/shared/src/server";
...
export const batchExportQueueProcessor = async (
  ...
  try {
    ...
    if (e instanceof AletheiaNotFoundError) {
      logger.warn(
