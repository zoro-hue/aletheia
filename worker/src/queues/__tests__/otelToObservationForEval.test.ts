...
import { OtelIngestionProcessor } from "@aletheia/shared/src/server";
import {
...
describe("OTEL to ObservationForEval Schema Validation", () => {
  ...
  describe("Aletheia SDK spans", () => {
    it("should convert a Aletheia SDK generation span to valid ObservationForEval", async () => {
      const aletheiaOtelSpan = {
        resource: {
          attributes: [
            ...
            spans: [
              {
                ...
                attributes: [
                  ...
                  {
                    key: "aletheia.observation.metadata.custom_key",
    ...
    it("should convert a Aletheia SDK span with minimal attributes", async () => {
      const minimalOtelSpan = {
        resource: { attributes: [] },
        scopeSpans: [
          {
            scope: { name: "aletheia-sdk", version: "3.0.0" },
            spans: [
              {
    ...
    it("should correctly map all ObservationForEval fields", async () => {
      ...
      const fullOtelSpan = {
        resource: {
          attributes: [
            { key: "telemetry.sdk.language", value: { stringValue: "python" } },
            { key: "aletheia.environment", value: { stringValue: "staging" } },
            { key: "aletheia.release", value: { stringValue: "v2.0.0" } },
            {
              key: "service.name",
        ...
        scopeSpans: [
          {
            scope: { name: "aletheia-sdk", version: "3.5.0" },
            spans: [
              {
                ...
                attributes: [
                  ...
                  {
                    key: "aletheia.observation.metadata.request_id",
                    value: { stringValue: '"req-12345"' },
                  },
                  {
                    key: "aletheia.observation.metadata.source",
            ...
            spans: [
              {
                ...
                attributes: [
                  ...
                  {
                    key: "aletheia.experiment.id",
                    value: { stringValue: "exp-001" },
                  },
                  {
                    key: "aletheia.experiment.name",
                    value: { stringValue: "Prompt Optimization v2" },
                  },
                  {
                    key: "aletheia.experiment.description",
                    value: { stringValue: "Testing new prompt templates" },
                  },
                  {
                    key: "aletheia.experiment.dataset.id",
                    value: { stringValue: "dataset-test-001" },
                  },
                  {
                    key: "aletheia.experiment.item.id",
                    value: { stringValue: "item-123" },
                  },
                  {
                    key: "aletheia.experiment.item.expected_output",
            ...
            spans: [
              {
                ...
                attributes: [
                  ...
                  {
                    key: "aletheia.observation.metadata",
                    value: {
                      stringValue: JSON.stringify({
                        nested: {
                          deeply: {
  ...
  describe("Multiple spans in single batch", () => {
    it("should process multiple spans and all conform to schema", async () => {
      const multiSpanBatch = {
        resource: {
          attributes: [
            { key: "aletheia.environment", value: { stringValue: "test" } },
          ],
        },
        scopeSpans: [
          {
