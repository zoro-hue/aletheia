...
describe("StorageService", () => {
  ...
  let s3Prefix: string;
  const baseUrl = `${env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT}/${env.ALETHEIA_S3_EVENT_UPLOAD_BUCKET}`;
  ...
  test("uploadWithSignedUrl should upload a file and return a signed URL", async () => {
    ...
    // Then
    if (env.ALETHEIA_USE_OCI_NATIVE_OBJECT_STORAGE !== "true") {
  ...
  test("uploadFile should successfully process a Readable entity", async () => {
    ...
    // Then
    if (env.ALETHEIA_USE_OCI_NATIVE_OBJECT_STORAGE !== "true") {
      expect(signedUrl).toContain(env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT);
    } else {
  ...
  test("getSignedUrl should return URL with external endpoint when configured", async () => {
    ...
    expect(signedUrl).not.toContain(env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT);
  });
  ...
  test("uploadWithSignedUrl should return signed URL with external endpoint when configured", async () => {
    ...
    expect(result.signedUrl).not.toContain(
      env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT,
    );
  ...
  describeIfConnectionValidationSupported("connection validation", () => {
    test("uploadJson succeeds when the storage endpoint host is whitelisted", async () => {
      expect(env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT).toBeDefined();
      if (!env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT) return;

      const fileName = `${s3Prefix}${randomUUID()}.json`;
      const data = [{ hello: "validated world" }];
      const endpointHost = new URL(env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT)
        .hostname;
