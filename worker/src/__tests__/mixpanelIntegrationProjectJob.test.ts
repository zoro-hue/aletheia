 ...
 *   3. a configurable inter-flush delay (ALETHEIA_MIXPANEL_FLUSH_DELAY_MS).
 *
...
const h = vi.hoisted(() => {
  ...
  function fakeStream(label: string) {
    return (async function* () {
      ...
      yield { aletheia_id: `${label}-1` };
      await Promise.resolve();
      yield { aletheia_id: `${label}-2` };
...
vi.mock("@aletheia/shared/encryption", () => ({
...
vi.mock("../env", () => ({
  env: { ALETHEIA_MIXPANEL_FLUSH_DELAY_MS: 0 },
}));
