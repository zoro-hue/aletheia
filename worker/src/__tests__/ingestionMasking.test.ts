...
import type { SharedEnv } from "@aletheia/shared/src/env";
import {
...
const VALID_EE_LICENSE_KEY = "aletheia_ee_test-license-key";

// Default test env with masking disabled
const defaultTestEnv: SharedEnv = {
  ALETHEIA_INGESTION_MASKING_CALLBACK_URL: undefined,
  ALETHEIA_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: 500,
  ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
  ALETHEIA_INGESTION_MASKING_MAX_RETRIES: 1,
  ALETHEIA_INGESTION_MASKING_PROPAGATED_HEADERS: [],
  NEXT_PUBLIC_ALETHEIA_CLOUD_REGION: undefined,
  ALETHEIA_EE_LICENSE_KEY: undefined,
} as SharedEnv;
...
describe("Ingestion Masking", () => {
  ...
  describe("isIngestionMaskingEnabled", () => {
    it("should return false when callback URL is not configured", () => {
      const testEnv = createTestEnv({
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
      });
    ...
    it("should return false when EE license is not available", () => {
      const testEnv = createTestEnv({
        ALETHEIA_INGESTION_MASKING_CALLBACK_URL:
  ...
  describe("applyIngestionMasking", () => {
    ...
    it("should include X-Aletheia-Org-Id and X-Aletheia-Project-Id headers", async () => {
      const testEnv = createTestEnv({
        ALETHEIA_INGESTION_MASKING_CALLBACK_URL:
          "https://masking.example.com/success",
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
      ...
      expect(request?.headers["x-aletheia-org-id"]).toBe("test-org-456");
      expect(request?.headers["x-aletheia-project-id"]).toBe(
        "test-project-123",
      );
    });
    ...
    it("should return original data on HTTP 500 with fail-open (default)", async () => {
      const testEnv = createTestEnv({
        ...
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        ALETHEIA_INGESTION_MASKING_MAX_RETRIES: 0,
      });
    ...
    it("should return failure on HTTP 500 with fail-closed", async () => {
      const testEnv = createTestEnv({
        ALETHEIA_INGESTION_MASKING_CALLBACK_URL:
          "https://masking.example.com/error",
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "true",
        ALETHEIA_INGESTION_MASKING_MAX_RETRIES: 0,
      });
    ...
    it("should retry on failure", async () => {
      const testEnv = createTestEnv({
        ...
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        ALETHEIA_INGESTION_MASKING_MAX_RETRIES: 2,
      });
    ...
    it("should handle timeout with fail-open", async () => {
      const testEnv = createTestEnv({
        ...
        ALETHEIA_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        ALETHEIA_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: 100, // Short timeout
        ALETHEIA_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        ALETHEIA_INGESTION_MASKING_MAX_RETRIES: 0,
      });
