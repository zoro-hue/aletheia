...
} from "@aletheia/shared/encryption";
import { generateWebhookSecret } from "@aletheia/shared/encryption";
...
describe("Webhook Integration Tests", () => {
  ...
  describe("executeWebhook function", () => {
    it("should execute webhook successfully with signature", async () => {
      ...
      expect(request.headers["x-aletheia-signature"]).toMatch(
        /^t=\d+,v1=[a-f0-9]+$/,
      );

      // check signature
      const signature = request.headers["x-aletheia-signature"];
        ...
        const webhookInput: WebhookInput = {
          ...
          payload: {
            ...
            payload: {
              ...
              projectId,
              severity: "ALERT",
              permalink: `https://cloud.aletheia.com/project/${projectId}/monitors/mon`,
              timestamp: new Date(),
    ...
    it("should reset failure count correctly with lastFailingExecutionId", async () => {
      ...
      const { getConsecutiveAutomationFailures } =
        await import("@aletheia/shared/src/server");
