...
const requiredSharedEnv = {
  ...
  ALETHEIA_S3_EVENT_UPLOAD_BUCKET: "test-bucket",
};
...
describe("BullMQ Redis version check options", () => {
  ...
  test("does not skip Redis version checks by default", async () => {
    delete process.env.ALETHEIA_BULLMQ_SKIP_REDIS_VERSION_CHECK;
  ...
  test("skips Redis version checks when configured", async () => {
    process.env.ALETHEIA_BULLMQ_SKIP_REDIS_VERSION_CHECK = "true";
  ...
  test("passes centralized options to BullMQ workers", async () => {
    ...
    vi.doMock("@aletheia/shared/src/server", () => ({
      QueueName: {
    ...
    vi.doMock("../env", () => ({
      env: {
        ALETHEIA_QUEUE_METRICS_SAMPLE_RATE: 0,
      },
    }));
