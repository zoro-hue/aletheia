...
const originalLocalCacheSetting =
  process.env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED;

describe("modelMatch", () => {
  beforeAll(async () => {
    process.env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED = "true";
    vi.resetModules();
  ...
  afterAll(() => {
    if (originalLocalCacheSetting === undefined) {
      delete process.env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED;
    } else {
      process.env.ALETHEIA_LOCAL_CACHE_MODEL_MATCH_ENABLED =
  ...
  describe("findModel", () => {
    ...
    it("should cache not found models in Redis", async () => {
      ...
      const cachedValue = await redis?.get(redisKey);
      expect(cachedValue).toBe("ALETHEIA_MODEL_MATCH_NOT_FOUND");
    });
