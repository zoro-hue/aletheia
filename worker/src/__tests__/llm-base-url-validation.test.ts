...
const originalCloudRegion = env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
const originalAllowedHosts = env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST;
const originalAllowedIps = env.ALETHEIA_LLM_CONNECTION_WHITELISTED_IPS;
const originalAllowedIpSegments =
  env.ALETHEIA_LLM_CONNECTION_WHITELISTED_IP_SEGMENTS;
...
describe("LLM base URL validation", () => {
  ...
  afterEach(() => {
    (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalCloudRegion;
    (env as any).ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST =
      originalAllowedHosts;
    (env as any).ALETHEIA_LLM_CONNECTION_WHITELISTED_IPS = originalAllowedIps;
    (env as any).ALETHEIA_LLM_CONNECTION_WHITELISTED_IP_SEGMENTS =
  ...
  it("should ignore self-host allowlists on Aletheia Cloud", async () => {
  ...
  it("should allow public HTTPS URLs on Aletheia Cloud", async () => {
    (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "US";
  ...
  it("should reject unresolved hostnames on Aletheia Cloud regardless of whitelist", async () => {
    (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "US";
  ...
  it("should reject non-HTTPS URLs on Aletheia Cloud", async () => {
    (env as any).NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = "US";

    await expect(
      validateLlmConnectionBaseURL("http://1.1.1.1/v1"),
    ).rejects.toThrow("Only HTTPS base URLs are allowed on Aletheia Cloud");
  });
});
