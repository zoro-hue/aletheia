...
import type { WebhookInput } from "@aletheia/shared/src/server";
...
describe("Slack Processor", () => {
  ...
  beforeAll(async () => {
    ...
    const server = await import("@aletheia/shared/src/server");
    const { SlackService } = server;

    // getActionById delegates to the real impl so existing tests read real rows
    const actual = await vi.importActual<any>("@aletheia/shared/src/server");
  ...
  describe("executeSlack function", () => {
    it("should execute slack action successfully", async () => {
      const { SlackService } = await import("@aletheia/shared/src/server");
    ...
    it("monitor-alert with a JSON-round-tripped (string) timestamp builds the full message, not the fallback", async () => {
      ...
      const monitorInput: WebhookInput = {
        ...
        payload: {
          ...
          payload: {
            ...
            severity: "ALERT",
            permalink:
              "https://cloud.aletheia.com/project/proj_01/monitors/mon_01",
      ...
      const isFallback =
        ...
        sent.blocks[0].text?.text?.startsWith("*Aletheia Notification*");
