...
describe("secure LLM fetch", () => {
  const originalWhitelistedHosts = env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST;
  ...
  beforeEach(() => {
    ...
    env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST = ["127.0.0.1"];
  });

  afterEach(async () => {
    vi.restoreAllMocks();
    await Promise.all(servers.splice(0).map((s) => s.close()));
    env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST = originalWhitelistedHosts;
    env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalCloudRegion;
  });
  ...
  describe("SSRF protection", () => {
    ...
    test("rejects link-local metadata hostnames before opening a socket", async () => {
      env.ALETHEIA_LLM_CONNECTION_WHITELISTED_HOST = [];
    ...
    test("rejects HTTP base URLs when running on Aletheia Cloud", async () => {
