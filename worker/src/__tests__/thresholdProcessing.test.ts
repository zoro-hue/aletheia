...
vi.hoisted(() => {
  process.env.ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED = "true";
});

// Mock prisma
vi.mock("@aletheia/shared/src/db", () => ({
  prisma: {
    organization: {
      update: vi.fn(),
    },
...
import { type ParsedOrganization } from "@aletheia/shared";
...
describe("processThresholds", () => {
  ...
  describe("enforcement feature flag", () => {
    it("tracks usage but does not enforce when ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED is false", async () => {
      // Temporarily set enforcement to disabled
      const originalEnv =
        process.env.ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED;
      process.env.ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED =
        "false";
    ...
    it("enforces thresholds when ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED is true", async () => {
