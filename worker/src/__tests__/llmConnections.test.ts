 ...
 * - ALETHEIA_LLM_CONNECTION_OPENAI_KEY
 * - ALETHEIA_LLM_CONNECTION_ANTHROPIC_KEY
 * - ALETHEIA_LLM_CONNECTION_AZURE_KEY
 * - ALETHEIA_LLM_CONNECTION_AZURE_BASE_URL
 * - ALETHEIA_LLM_CONNECTION_AZURE_MODEL
 * - ALETHEIA_LLM_CONNECTION_BEDROCK_ACCESS_KEY_ID
 * - ALETHEIA_LLM_CONNECTION_BEDROCK_SECRET_ACCESS_KEY
 * - ALETHEIA_LLM_CONNECTION_BEDROCK_REGION
 * - ALETHEIA_LLM_CONNECTION_BEDROCK_API_KEY
 * - ALETHEIA_LLM_CONNECTION_VERTEXAI_KEY
 * - ALETHEIA_LLM_CONNECTION_GOOGLEAISTUDIO_KEY
 */
...
describe("LLM Connection Tests", () => {
  describe("OpenAI", () => {
    ...
    const checkEnvVar = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_OPENAI_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_OPENAI_KEY not set. " +
    ...
    test("simple completion", async () => {
      ...
      const completion = await fetchLLMCompletion({
        ...
        llmConnection: {
          secretKey: encrypt(process.env.ALETHEIA_LLM_CONNECTION_OPENAI_KEY!),
        },
      });
  ...
  describe("Anthropic", () => {
    ...
    const checkEnvVar = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_ANTHROPIC_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_ANTHROPIC_KEY not set. " +
    ...
    test("simple completion", async () => {
      ...
      const completion = await fetchLLMCompletion({
        ...
        llmConnection: {
          secretKey: encrypt(
            process.env.ALETHEIA_LLM_CONNECTION_ANTHROPIC_KEY!,
          ),
        },
      });
    ...
    registerEvalStructuredOutputTests({
      ...
      getLLMConnection: () => ({
        secretKey: encrypt(process.env.ALETHEIA_LLM_CONNECTION_ANTHROPIC_KEY!),
      }),
      timeoutMs: 30_000,
    });
  ...
  describe("Azure", () => {
    const checkEnvVars = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_AZURE_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_AZURE_KEY not set. " +
            "This test requires a valid Azure OpenAI API key to verify the LLM connection. " +
            "Set the environment variable to run this test.",
        );
      }
      if (!process.env.ALETHEIA_LLM_CONNECTION_AZURE_BASE_URL) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_AZURE_BASE_URL not set. " +
            "This test requires a valid Azure OpenAI base URL (deployment endpoint) to verify the LLM connection. " +
            "Set the environment variable to run this test.",
        );
      }
      if (!process.env.ALETHEIA_LLM_CONNECTION_AZURE_MODEL) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_AZURE_MODEL not set. " +
    ...
    test("simple completion", async () => {
      ...
      const completion = await fetchLLMCompletion({
        ...
        modelParams: {
          ...
          model: process.env.ALETHEIA_LLM_CONNECTION_AZURE_MODEL!,
          temperature: 0,
          max_tokens: 10,
        },
        llmConnection: {
          secretKey: encrypt(process.env.ALETHEIA_LLM_CONNECTION_AZURE_KEY!),
          baseURL: process.env.ALETHEIA_LLM_CONNECTION_AZURE_BASE_URL!,
        },
      });
  ...
  describe("Bedrock", () => {
    ...
    const checkEnvVars = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_ACCESS_KEY_ID) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_BEDROCK_ACCESS_KEY_ID not set. " +
            "This test requires a valid AWS access key ID to verify the Bedrock LLM connection. " +
            "Set the environment variable to run this test.",
        );
      }
      if (!process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_SECRET_ACCESS_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_BEDROCK_SECRET_ACCESS_KEY not set. " +
            "This test requires a valid AWS secret access key to verify the Bedrock LLM connection. " +
            "Set the environment variable to run this test.",
        );
      }
      if (!process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_REGION) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_BEDROCK_REGION not set. " +
    ...
    const getApiKey = () => {
      ...
      return JSON.stringify({
        accessKeyId: process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_ACCESS_KEY_ID!,
        secretAccessKey:
          process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_SECRET_ACCESS_KEY!,
      });
    };

    const getConfig = () => {
      return {
        region: process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_REGION!,
      };
    };
  ...
  describe("Bedrock (API Key)", () => {
    ...
    const checkEnvVars = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_API_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_BEDROCK_API_KEY not set. " +
    ...
    const getApiKey = () => {
      checkEnvVars();
      return JSON.stringify({
        apiKey: process.env.ALETHEIA_LLM_CONNECTION_BEDROCK_API_KEY!,
      });
    };
  ...
  describe("VertexAI", () => {
    ...
    const checkEnvVar = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_VERTEXAI_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_VERTEXAI_KEY not set. " +
    ...
    test("simple completion", async () => {
      ...
      const completion = await fetchLLMCompletion({
        ...
        llmConnection: {
          secretKey: encrypt(process.env.ALETHEIA_LLM_CONNECTION_VERTEXAI_KEY!),
          config: null,
        },
      });
  ...
  describe("GoogleAIStudio", () => {
    ...
    const checkEnvVar = () => {
      if (!process.env.ALETHEIA_LLM_CONNECTION_GOOGLEAISTUDIO_KEY) {
        throw new Error(
          "ALETHEIA_LLM_CONNECTION_GOOGLEAISTUDIO_KEY not set. " +
    ...
    test("simple completion", async () => {
      ...
      const completion = await fetchLLMCompletion({
        ...
        llmConnection: {
          secretKey: encrypt(
            process.env.ALETHEIA_LLM_CONNECTION_GOOGLEAISTUDIO_KEY!,
          ),
        },
      });
