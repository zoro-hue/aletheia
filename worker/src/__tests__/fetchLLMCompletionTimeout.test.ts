...
process.env.ALETHEIA_S3_EVENT_UPLOAD_BUCKET ??= "test-bucket";
...
describe("fetchLLMCompletion runtime timeouts", () => {
  ...
  beforeEach(async () => {
    ...
    vi.resetModules();
    originalCloudRegion = process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
    ...
    originalTimeout = env.ALETHEIA_FETCH_LLM_COMPLETION_TIMEOUT_MS;
  });

  afterEach(() => {
    env.ALETHEIA_FETCH_LLM_COMPLETION_TIMEOUT_MS = originalTimeout;
  ...
  it("wraps non-streaming VertexAI timeouts as non-retryable LLMCompletionError", async () => {
    env.ALETHEIA_FETCH_LLM_COMPLETION_TIMEOUT_MS = 25;
...
describe("fetchLLMCompletion end of model lifetime", () => {
  ...
  beforeEach(async () => {
    ...
    originalCloudRegion = process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
    delete process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
    // @langchain/aws is a dependency of @aletheia/shared, not worker — a bare
