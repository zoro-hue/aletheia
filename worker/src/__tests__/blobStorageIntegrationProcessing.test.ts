...
const originalCloudRegion = vi.hoisted(() => {
  const cloudRegion = process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
  delete process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
  return cloudRegion;
});
...
const maybeIt = env.ALETHEIA_USE_AZURE_BLOB === "true" ? it.skip : it;
const maybeDescribe =
  process.env.ALETHEIA_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN === "true"
    ? describe
    : describe.skip;
...
describe("BlobStorageIntegrationProcessingJob", () => {
  ...
  const bucketName = env.ALETHEIA_S3_EVENT_UPLOAD_BUCKET || "";
  const accessKeyId = env.ALETHEIA_S3_EVENT_UPLOAD_ACCESS_KEY_ID || "";
  const secretAccessKey = env.ALETHEIA_S3_EVENT_UPLOAD_SECRET_ACCESS_KEY || "";
  const endpoint = env.ALETHEIA_S3_EVENT_UPLOAD_ENDPOINT || undefined;
  const region = env.ALETHEIA_S3_EVENT_UPLOAD_REGION || undefined;
  ...
  afterAll(() => {
    if (originalCloudRegion) {
      process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION = originalCloudRegion;
    } else {
      delete process.env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION;
    }
  });
    ...
    it("fails the job and persists lastError when the enriched export path is unavailable", async () => {
      ...
      await prisma.blobStorageIntegration.create({
        data: {
          ...
          forcePathStyle:
            env.ALETHEIA_S3_EVENT_UPLOAD_FORCE_PATH_STYLE === "true",
          enabled: true,
