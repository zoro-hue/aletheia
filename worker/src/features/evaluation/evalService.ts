...
import {
  ...
  AletheiaInternalTraceEnvironment,
...
import { type ExtractedVariable } from "@aletheia/shared/src/server";
...
export const createEvalJobs = async ({
  ...
  // Early exit: Skip eval job creation for internal Aletheia traces from trace-upsert queue
  //
  // CONTEXT: Prevent infinite eval loops
  // Without this safeguard: user trace → eval → eval trace → another eval → infinite loop
  //
  // IMPLEMENTATION:
  // - Block ALL traces with environment starting with "aletheia-" when coming from trace-upsert queue
  // - This excludes traces from prompt experiments that come via dataset-run-item-upsert queue
  // - Internal traces (e.g., eval executions) use AletheiaInternalTraceEnvironment enum values
  //
  // DUAL SAFEGUARD:
  // - This check prevents eval job CREATION for internal traces
  // - fetchLLMCompletion.ts enforces that internal traces MUST use "aletheia-" prefix
  //
  // See: packages/shared/src/server/llm/fetchLLMCompletion.ts (enforcement)
  // See: packages/shared/src/server/llm/types.ts (AletheiaInternalTraceEnvironment enum)
  if (
    sourceEventType === "trace-upsert" &&
    event.traceEnvironment?.startsWith("aletheia")
  ) {
    logger.debug("Skipping eval job creation for internal Aletheia trace", {
  ...
  recordIncrement("aletheia.evaluation-execution.config_count", configs.length);
  if (configs.length > 1) {
    try {
      ...
      recordIncrement("aletheia.evaluation-execution.trace_cache_fetch", 1, {
  ...
  if (datasetConfigs.length > 1) {
    try {
      ...
      recordIncrement(
        "aletheia.evaluation-execution.dataset_item_cache_fetch",
        1,
        {
  ...
  for (const config of configs) {
    ...
    if (cachedTrace && !requiresDatabaseLookup(traceFilter)) {
      ...
      recordIncrement("aletheia.evaluation-execution.trace_cache_check", 1, {
      ...
      recordIncrement("aletheia.evaluation-execution.trace_db_lookup", 1, {
    ...
    recordIncrement("aletheia.evaluation-execution.trace_exists_check", 1, {
...
export async function runLLMAsJudgeEvaluation({
  ...
  return instrumentAsync(
    ...
    async (span) => {
      span.setAttribute("aletheia.project.id", projectId);
          ...
          try {
            return await deps.callLLM({
              ...
              traceSinkParams: {
                ...
                environment: AletheiaInternalTraceEnvironment.LLMJudge,
                metadata: {
                  ...executionMetadata,
                },
              },
...
export async function extractVariablesFromTracingData({
  ...
  for (const variable of variables) {
    ...
    if (mapping.aletheiaObject === "dataset_item") {
      if (!datasetItemId) {
        logger.warn(
    ...
    if (mapping.aletheiaObject === "trace") {
      // find the internal definitions of the column
    ...
    if (
      mapping.aletheiaObject &&
      observationTypes.includes(mapping.aletheiaObject)
    ) {
      const safeInternalColumn = availableTraceEvalVariables
        .find((o) => o.id === mapping.aletheiaObject)
        ?.availableColumns.find((col) => col.id === mapping.selectedColumnId);

      if (!mapping.objectName) {
        logger.info(
          `No object name found for variable ${variable} and object ${mapping.aletheiaObject}`,
        );
        results.push({ var: variable, value: "" });
        continue;
      }
    ...
    throw new Error(`Unknown object type ${mapping.aletheiaObject}`);
  }
