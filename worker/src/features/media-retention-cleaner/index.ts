...
const METRIC_PREFIX = "aletheia.media_retention_cleaner";

export const MEDIA_RETENTION_CLEANER_LOCK_KEY =
  "aletheia:media-retention-cleaner";
...
export class MediaRetentionCleaner extends PeriodicExclusiveRunner {
  protected get defaultIntervalMs(): number {
    return env.ALETHEIA_MEDIA_RETENTION_CLEANER_INTERVAL_MS;
  }

  constructor() {
    // TTL = interval + 5 minutes buffer (media deletion can be slow)
    const lockTtlSeconds =
      Math.ceil(env.ALETHEIA_MEDIA_RETENTION_CLEANER_INTERVAL_MS / 1000) + 300;
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      intervalMs: env.ALETHEIA_MEDIA_RETENTION_CLEANER_INTERVAL_MS,
      itemLimit: env.ALETHEIA_MEDIA_RETENTION_CLEANER_ITEM_LIMIT,
    });
    super.start();
  }
