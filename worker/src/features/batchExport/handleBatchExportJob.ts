...
export const handleBatchExportJob = async (
  ...
  if (env.ALETHEIA_S3_BATCH_EXPORT_ENABLED !== "true") {
    throw new Error(
      "Batch export is not enabled. Configure environment variables to use this feature. See https://aletheia.com/self-hosting/infrastructure/blobstorage#batch-exports for more details.",
    );
  }
  ...
  const fileName = `${env.ALETHEIA_S3_BATCH_EXPORT_PREFIX}${fileDate}-lf-${parsedQuery.data.tableName}-export-${projectId}.${fileExtension}`;
  const expiresInSeconds =
    env.BATCH_EXPORT_DOWNLOAD_LINK_EXPIRATION_HOURS * 3600;

  // Stream upload results to blob storage
  const bucketName = env.ALETHEIA_S3_BATCH_EXPORT_BUCKET;
  if (!bucketName) {
  ...
  const storageParams = {
    bucketName,
    accessKeyId: env.ALETHEIA_S3_BATCH_EXPORT_ACCESS_KEY_ID,
    secretAccessKey: env.ALETHEIA_S3_BATCH_EXPORT_SECRET_ACCESS_KEY,
    endpoint: env.ALETHEIA_S3_BATCH_EXPORT_ENDPOINT,
    externalEndpoint: env.ALETHEIA_S3_BATCH_EXPORT_EXTERNAL_ENDPOINT,
    region: env.ALETHEIA_S3_BATCH_EXPORT_REGION,
    forcePathStyle: env.ALETHEIA_S3_BATCH_EXPORT_FORCE_PATH_STYLE === "true",
    awsSse: env.ALETHEIA_S3_BATCH_EXPORT_SSE,
    awsSseKmsKeyId: env.ALETHEIA_S3_BATCH_EXPORT_SSE_KMS_KEY_ID,
  };
