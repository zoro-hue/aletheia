...
export const BATCH_PROJECT_CLEANER_LOCK_PREFIX =
  "aletheia:batch-project-cleaner";
...
export class BatchProjectCleaner extends PeriodicExclusiveRunner {
  ...
  protected get defaultIntervalMs(): number {
    return env.ALETHEIA_BATCH_PROJECT_CLEANER_SLEEP_ON_EMPTY_MS;
  }

  constructor(tableName: BatchDeletionTable) {
    // TTL = DELETE timeout + 5 minutes buffer
    const lockTtlSeconds =
      Math.ceil(env.ALETHEIA_BATCH_PROJECT_CLEANER_DELETE_TIMEOUT_MS / 1000) +
      300;
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      checkIntervalMs: env.ALETHEIA_BATCH_PROJECT_CLEANER_CHECK_INTERVAL_MS,
      sleepOnEmptyMs: env.ALETHEIA_BATCH_PROJECT_CLEANER_SLEEP_ON_EMPTY_MS,
      projectLimit: env.ALETHEIA_BATCH_PROJECT_CLEANER_PROJECT_LIMIT,
      deleteTimeoutMs: env.ALETHEIA_BATCH_PROJECT_CLEANER_DELETE_TIMEOUT_MS,
    });
    super.start();
  }
  ...
  protected async execute(): Promise<number> {
    ...
    try {
      deletedProjects = await getDeletedProjects(
        env.ALETHEIA_BATCH_PROJECT_CLEANER_PROJECT_LIMIT,
      );
    } catch (error) {
    ...
    return (
      (await this.withLock(
        async () => {
          ...
          return env.ALETHEIA_BATCH_PROJECT_CLEANER_CHECK_INTERVAL_MS;
        },
        async (error) => {
          // Step 5: On failure, re-run count query to determine partial success
          recordIncrement("aletheia.batch_project_cleaner.delete_failures", 1, {
            table: this.tableName,
          });
          ...
          if (incompleteProjects.length > 0) {
            recordIncrement(
              "aletheia.batch_project_cleaner.incomplete_cleanups",
      ...
      )) ?? env.ALETHEIA_BATCH_PROJECT_CLEANER_SLEEP_ON_EMPTY_MS
    );
  }
  ...
  private async executeDelete(projectIds: string[]): Promise<void> {
    ...
    await commandClickhouse({
      ...
      params: { projectIds },
      clickhouseConfigs: {
        request_timeout: env.ALETHEIA_BATCH_PROJECT_CLEANER_DELETE_TIMEOUT_MS,
      },
      tags: {
        feature: "batch-project-cleaner",
        table: this.tableName,
