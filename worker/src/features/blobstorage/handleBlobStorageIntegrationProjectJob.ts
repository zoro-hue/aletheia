  ...
  await instrumentAsync(
    ...
    async (span) => {
      ...
      try {
        ...
        recordGauge(
          "aletheia.blobstorage.event_loop_delay.max",
          maxMs,
          delayTags,
        );
        recordGauge(
          "aletheia.blobstorage.event_loop_delay.p99",
          p99Ms,
          delayTags,
        );
...
export const handleBlobStorageIntegrationProjectJob = async (
  ...
  try {
    ...
    if (
      ...
      !isEnrichedBlobExportAvailable(
        Boolean(env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION),
    ...
    const isTraceOnlyProject =
      env.ALETHEIA_BLOB_STORAGE_EXPORT_TRACE_ONLY_PROJECT_IDS.includes(
        projectId,
      );
...
function notifyBlobStorageExportFailedInBackground(projectId: string): void {
  (async () => {
    try {
      const cooldownMs =
        env.ALETHEIA_BLOB_STORAGE_FAILURE_NOTIFICATION_COOLDOWN_HOURS *
        60 *
        60 *
        1000;
