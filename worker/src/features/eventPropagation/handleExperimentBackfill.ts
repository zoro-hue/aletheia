...
const EXPERIMENT_BACKFILL_TIMESTAMP_KEY =
  "aletheia:event-propagation:experiment-backfill:last-run";
const EXPERIMENT_BACKFILL_LOCK_KEY = "aletheia:experiment-backfill:lock";
...
export async function getRelevantObservations(
  ...
  const query = `
    ...
    WHERE o.project_id IN {projectIds: Array(String)}
      ...
      AND coalesce(o.environment, '') != 'aletheia-prompt-experiment'
...
export async function getRelevantTraces(
  ...
  const query = `
    ...
    WHERE t.project_id IN {projectIds: Array(String)}
      ...
      AND coalesce(t.environment, '') != 'aletheia-prompt-experiment'
...
export async function shouldRunBackfill(lastRun: Date): Promise<boolean> {
  ...
  if (timeSinceLastRun < env.ALETHEIA_EXPERIMENT_BACKFILL_THROTTLE_MS) {
    logger.debug(
...
export async function runExperimentBackfill(): Promise<void> {
  ...
  try {
    ...
    recordGauge(
      "aletheia.experiment_backfill.last_run_delay_seconds",
      lastRunDelaySeconds,
    );
    ...
    recordGauge(
      "aletheia.experiment_backfill.remaining_delay_seconds",
...
async function processExperimentBackfill(
  ...
  const excludeProjectIds =
    env.ALETHEIA_EXPERIMENT_BACKFILL_EXCLUDE_PROJECT_IDS;
  ...
  const chunkSize = env.ALETHEIA_DATASET_RUN_BACKFILL_CHUNK_SIZE;
