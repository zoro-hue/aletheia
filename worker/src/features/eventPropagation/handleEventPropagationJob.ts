...
const LAST_PROCESSED_PARTITION_KEY =
  "aletheia:event-propagation:last-processed-partition";
  ...
  try {
    ...
    if (lastProcessedPartition) {
      ...
      if (!isNaN(lastPartitionTime)) {
        ...
        recordGauge(
          "aletheia.event_propagation.last_processed_partition_delay_seconds",
          delaySeconds,
        );
      }
    }
    ...
    // Filter for partitions older than ALETHEIA_EXPERIMENT_EVENT_PROPAGATION_PARTITION_DELAY_MINUTES minutes and order by partition ASC to get the oldest first
    const partitions = await queryClickhouse<{ partition: string }>({
      query: `
        ...
        WHERE table = 'observations_batch_staging'
          ...
          AND active = 1
          AND toDateTime(partition) < now() - INTERVAL ${env.ALETHEIA_EXPERIMENT_EVENT_PROPAGATION_PARTITION_DELAY_MINUTES} MINUTE
    ...
    recordGauge(
      "aletheia.event_propagation.partition_backlog",
      partitions.length,
    );
    ...
    const excludeProjectIds =
      env.ALETHEIA_EVENT_PROPAGATION_EXCLUDE_PROJECT_IDS;
    ...
    if (!isNaN(processedPartitionTime)) {
      ...
      recordGauge(
        "aletheia.event_propagation.processed_partition_delay_seconds",
        delaySeconds,
      );
    }
