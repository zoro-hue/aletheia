...
const METRIC_PREFIX = "aletheia.batch_project_media_cleaner";
...
export class BatchProjectMediaCleaner extends PeriodicExclusiveRunner {
  ...
  constructor() {
    ...
    super({
      ...
      lockKey: "aletheia:batch-project-media-cleaner",
      lockTtlSeconds,
    });
  }
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      ...
      batchSize: env.ALETHEIA_BATCH_PROJECT_MEDIA_CLEANER_BATCH_SIZE,
    });
    super.start();
  }
  ...
  private async deleteMediaChunk(projectId: string): Promise<number> {
    const batchSize = env.ALETHEIA_BATCH_PROJECT_MEDIA_CLEANER_BATCH_SIZE;
