...
const METRIC_PREFIX = "aletheia.batch_trace_deletion_cleaner";

export const BATCH_TRACE_DELETION_CLEANER_LOCK_KEY =
  "aletheia:batch-trace-deletion-cleaner";
...
export class BatchTraceDeletionCleaner extends PeriodicExclusiveRunner {
  protected get defaultIntervalMs(): number {
    return env.ALETHEIA_BATCH_TRACE_DELETION_CLEANER_INTERVAL_MS;
  }
  ...
  constructor() {
    super({
      ...
      lockTtlSeconds:
        env.ALETHEIA_BATCH_TRACE_DELETION_CLEANER_LOCK_TTL_SECONDS,
      onUnavailable: "fail",
    });
  }
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      intervalMs: env.ALETHEIA_BATCH_TRACE_DELETION_CLEANER_INTERVAL_MS,
      batchSize: env.ALETHEIA_DELETE_BATCH_SIZE,
      lockTtlSeconds:
        env.ALETHEIA_BATCH_TRACE_DELETION_CLEANER_LOCK_TTL_SECONDS,
    });
    super.start();
  ...
  private async processProject(projectId: string): Promise<void> {
    ...
    const pendingDeletions = await prisma.pendingDeletion.findMany({
      ...
      take: env.ALETHEIA_DELETE_BATCH_SIZE,
    });
