...
import { percentile } from "@aletheia/shared";
...
const METRIC_PREFIX = "aletheia.batch_data_retention_cleaner";

export const BATCH_DATA_RETENTION_CLEANER_LOCK_PREFIX =
  "aletheia:batch-data-retention-cleaner";
...
export class BatchDataRetentionCleaner extends PeriodicExclusiveRunner {
  ...
  protected get defaultIntervalMs(): number {
    return env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_INTERVAL_MS;
  }
  ...
  constructor(tableName: BatchDataRetentionTable) {
    ...
    const lockTtlSeconds =
      Math.ceil(
        env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_DELETE_TIMEOUT_MS / 1000,
      ) + 300;
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      intervalMs: env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_INTERVAL_MS,
      projectLimit: env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_PROJECT_LIMIT,
      deleteTimeoutMs:
        env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_DELETE_TIMEOUT_MS,
    });
    super.start();
  }
  ...
  private async getProjectWorkloads(): Promise<ProjectWorkload[]> {
    ...
    const chunkSize = env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_CHUNK_SIZE;
    ...
    return withExpiredRows.slice(
      0,
      env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_PROJECT_LIMIT,
    );
  }
