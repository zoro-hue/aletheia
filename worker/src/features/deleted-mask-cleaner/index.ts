...
export const DELETED_MASK_CLEANER_LOCK_KEY =
  "aletheia:clickhouse-deleted-mask-cleaner";

const METRIC_PREFIX = "aletheia.clickhouse_deleted_mask_cleaner";
...
export class DeletedMaskCleaner extends PeriodicExclusiveRunner {
  protected get defaultIntervalMs(): number {
    return env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_INTERVAL_MS;
  }

  constructor() {
    const lockTtlSeconds =
      Math.ceil(
        env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_SUBMIT_TIMEOUT_MS / 1000,
      ) + 300;
  ...
  public override start(): void {
    logger.info(`Starting ${this.instanceName}`, {
      intervalMs: env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_INTERVAL_MS,
      submitTimeoutMs:
        env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_SUBMIT_TIMEOUT_MS,
  ...
  private useCleanerClusterMode(): boolean {
    return shouldUseDeletedMaskCleanerClusterMode({
      ...
      cleanerClusterModeEnabled:
        env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_CLUSTER_MODE_ENABLED ===
        "true",
    });
  }
