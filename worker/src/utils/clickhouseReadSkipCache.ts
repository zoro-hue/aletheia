import { PrismaClient, AletheiaNotFoundError } from "@aletheia/shared";
...
export class ClickhouseReadSkipCache {
  ...
  private async performInitialization(): Promise<void> {
    if (!env.ALETHEIA_SKIP_INGESTION_CLICKHOUSE_READ_MIN_PROJECT_CREATE_DATE) {
      logger.info(
    ...
    const cutoffDate = new Date(
      env.ALETHEIA_SKIP_INGESTION_CLICKHOUSE_READ_MIN_PROJECT_CREATE_DATE,
    );
  ...
  public async shouldSkipClickHouseRead(
    ...
    if (
      env.ALETHEIA_SKIP_INGESTION_CLICKHOUSE_READ_PROJECT_IDS &&
      env.ALETHEIA_SKIP_INGESTION_CLICKHOUSE_READ_PROJECT_IDS.split(
        ",",
      ).includes(projectId)
    ) {
      return true;
    ...
    try {
      ...
      if (!project) {
        throw new AletheiaNotFoundError(`Project ${projectId} not found`);
      }
