...
import { monitorProcessorTtl } from "@aletheia/shared/monitors/server";
...
app.get<{}, MessageResponse>("/", (req, res) => {
  res.json({
    message: "Aletheia Worker API ⚡ — AI Reliability Platform",
  });
});
...
if (env.ALETHEIA_ENABLE_BACKGROUND_MIGRATIONS === "true") {
...
if (env.QUEUE_CONSUMER_TRACE_UPSERT_QUEUE_IS_ENABLED === "true") {
  ...
  traceUpsertShardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      evalJobTraceCreatorQueueProcessor,
      {
        concurrency: env.ALETHEIA_TRACE_UPSERT_WORKER_CONCURRENCY,
      },
    );
  });
}
...
if (env.QUEUE_CONSUMER_CREATE_EVAL_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(
    ...
    evalJobCreatorQueueProcessor,
    {
      concurrency: env.ALETHEIA_EVAL_CREATOR_WORKER_CONCURRENCY,
      limiter: {
        // Process at most `max` jobs per `duration` milliseconds globally
        max: env.ALETHEIA_EVAL_CREATOR_WORKER_CONCURRENCY,
        duration: env.ALETHEIA_EVAL_CREATOR_LIMITER_DURATION,
      },
    },
  );
}

if (env.ALETHEIA_S3_CORE_DATA_EXPORT_IS_ENABLED === "true") {
...
if (env.ALETHEIA_POSTGRES_METERING_DATA_EXPORT_IS_ENABLED === "true") {
  // Instantiate the queue to trigger scheduled jobs
...
if (env.QUEUE_CONSUMER_TRACE_DELETE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.TraceDelete, traceDeleteProcessor, {
    concurrency: env.ALETHEIA_TRACE_DELETE_CONCURRENCY,
    ...
    limiter: {
      ...
      max: env.ALETHEIA_TRACE_DELETE_CONCURRENCY,
      duration: env.ALETHEIA_CLICKHOUSE_TRACE_DELETION_CONCURRENCY_DURATION_MS,
    },
  });
}
...
if (env.QUEUE_CONSUMER_SCORE_DELETE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.ScoreDelete, scoreDeleteProcessor, {
    concurrency: env.ALETHEIA_SCORE_DELETE_CONCURRENCY,
    limiter: {
      // Process at most `max` delete jobs per 15 seconds
      max: env.ALETHEIA_SCORE_DELETE_CONCURRENCY,
      duration: env.ALETHEIA_CLICKHOUSE_TRACE_DELETION_CONCURRENCY_DURATION_MS,
    },
  });
}
...
if (env.QUEUE_CONSUMER_DATASET_DELETE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.DatasetDelete, datasetDeleteProcessor, {
    concurrency: env.ALETHEIA_DATASET_DELETE_CONCURRENCY,
    limiter: {
      max: env.ALETHEIA_DATASET_DELETE_CONCURRENCY,
      duration:
        env.ALETHEIA_CLICKHOUSE_DATASET_DELETION_CONCURRENCY_DURATION_MS,
    },
  });
}
...
if (env.QUEUE_CONSUMER_PROJECT_DELETE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.ProjectDelete, projectDeleteProcessor, {
    concurrency: env.ALETHEIA_PROJECT_DELETE_CONCURRENCY,
    limiter: {
      // Process at most `max` delete jobs per ALETHEIA_CLICKHOUSE_PROJECT_DELETION_CONCURRENCY_DURATION_MS (default 10 min)
      max: env.ALETHEIA_PROJECT_DELETE_CONCURRENCY,
      duration:
        env.ALETHEIA_CLICKHOUSE_PROJECT_DELETION_CONCURRENCY_DURATION_MS,
    },
  });
}
...
if (env.QUEUE_CONSUMER_EVAL_EXECUTION_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency: env.ALETHEIA_EVAL_EXECUTION_WORKER_CONCURRENCY,
  ...
  llmAsJudgeShardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency: env.ALETHEIA_LLM_AS_JUDGE_EXECUTION_WORKER_CONCURRENCY,
        lockDuration: 60000,
...
if (env.QUEUE_CONSUMER_CODE_EVAL_EXECUTION_QUEUE_IS_ENABLED === "true") {
  ...
  codeEvalShardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency: env.ALETHEIA_CODE_EVAL_EXECUTION_WORKER_CONCURRENCY,
        lockDuration: 60000,
        stalledInterval: 120000,
        maxStalledCount: 3,
      },
...
if (env.QUEUE_CONSUMER_EVAL_EXECUTION_SECONDARY_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency:
          env.ALETHEIA_EVAL_EXECUTION_SECONDARY_QUEUE_PROCESSING_CONCURRENCY,
        lockDuration: 60000, // 60 seconds
        stalledInterval: 120000, // 120 seconds
        maxStalledCount: 3,
      },
...
if (env.QUEUE_CONSUMER_OTEL_INGESTION_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency: env.ALETHEIA_OTEL_INGESTION_QUEUE_PROCESSING_CONCURRENCY,
      },
    );
  });
}
...
if (env.QUEUE_CONSUMER_OTEL_INGESTION_SECONDARY_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency:
          env.ALETHEIA_OTEL_INGESTION_SECONDARY_QUEUE_PROCESSING_CONCURRENCY,
      },
    );
  });
}
...
if (env.QUEUE_CONSUMER_INGESTION_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency: env.ALETHEIA_INGESTION_QUEUE_PROCESSING_CONCURRENCY,
      },
    );
  });
}
...
if (env.QUEUE_CONSUMER_INGESTION_SECONDARY_QUEUE_IS_ENABLED === "true") {
  ...
  shardNames.forEach((shardName) => {
    WorkerManager.register(
      ...
      {
        concurrency:
          env.ALETHEIA_INGESTION_SECONDARY_QUEUE_PROCESSING_CONCURRENCY,
      },
    );
  });
}
...
if (env.QUEUE_CONSUMER_MONITOR_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.MonitorQueue, monitorQueueProcessor, {
    concurrency: env.ALETHEIA_MONITOR_QUEUE_PROCESSING_CONCURRENCY,
...
if (
  ...
  env.NEXT_PUBLIC_ALETHEIA_CLOUD_REGION && // Only in cloud deployments
  env.STRIPE_SECRET_KEY
) {
  // Instantiate the queue to trigger scheduled jobs
...
if (env.QUEUE_CONSUMER_EXPERIMENT_CREATE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(
    ...
    {
      concurrency: env.ALETHEIA_EXPERIMENT_CREATOR_WORKER_CONCURRENCY,
    },
  );
}
...
if (env.QUEUE_CONSUMER_WEBHOOK_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(QueueName.WebhookQueue, webhookProcessor, {
    concurrency: env.ALETHEIA_WEBHOOK_QUEUE_PROCESSING_CONCURRENCY,
  });
}
...
if (env.QUEUE_CONSUMER_ENTITY_CHANGE_QUEUE_IS_ENABLED === "true") {
  WorkerManager.register(
    ...
    {
      concurrency: env.ALETHEIA_ENTITY_CHANGE_QUEUE_PROCESSING_CONCURRENCY,
    },
  );
}
...
if (env.ALETHEIA_BATCH_PROJECT_CLEANER_ENABLED === "true") {
...
if (env.ALETHEIA_BATCH_DATA_RETENTION_CLEANER_ENABLED === "true") {
...
if (
  env.ALETHEIA_BATCH_PROJECT_CLEANER_ENABLED === "true" &&
  env.ALETHEIA_S3_MEDIA_UPLOAD_BUCKET
) {
...
if (
  env.ALETHEIA_BATCH_PROJECT_CLEANER_ENABLED === "true" &&
  env.ALETHEIA_ENABLE_BLOB_STORAGE_FILE_LOG === "true"
) {
...
if (env.ALETHEIA_BATCH_TRACE_DELETION_CLEANER_ENABLED === "true") {
...
if (env.ALETHEIA_CLICKHOUSE_DELETED_MASK_CLEANER_ENABLED === "true") {
...
if (env.ALETHEIA_QUEUE_METRICS_ENABLED === "true") {
...
if (env.ALETHEIA_MONITOR_SCHEDULER_ENABLED === "true") {
  for (let i = 0; i < env.ALETHEIA_MONITOR_SCHEDULERS; i++) {
    const runner = new MonitorRunner(i, env.ALETHEIA_MONITOR_SCHEDULERS);
