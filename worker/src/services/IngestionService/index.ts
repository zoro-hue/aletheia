...
export class IngestionService {
  ...
  private async processScoreEventList(params: {
    ...
    if (clickhouseScoreRecord) {
      recordIncrement("aletheia.ingestion.lookup.hit", 1, {
        store: "clickhouse",
        object: "score",
      });
    }
      ...
      try {
        ...
        await instrumentAsync(
          ...
          async (span) => {
            ...
            newInputCount
              ? span.setAttribute(
                  "aletheia.tokenization.input-count",
                  newInputCount,
                )
              : undefined;
            newOutputCount
              ? span.setAttribute(
                  "aletheia.tokenization.output-count",
                  newOutputCount,
                )
              : undefined;
            newInputCount || newOutputCount
              ? span.setAttribute(
                  "aletheia.tokenization.tokenizer",
                  model.tokenizerId || "unknown",
                )
              : undefined;
            newInputCount
              ? recordIncrement("aletheia.tokenisedTokens", newInputCount)
              : undefined;
            newOutputCount
              ? recordIncrement("aletheia.tokenisedTokens", newOutputCount)
              : undefined;
          },
        );
  ...
  private async getClickhouseRecord(params: {
    ...
    if (
      ...
      recordIncrement("aletheia.ingestion.clickhouse_read_for_update", 1, {
        skipped: "true",
        table: params.table,
      });
      return null;
