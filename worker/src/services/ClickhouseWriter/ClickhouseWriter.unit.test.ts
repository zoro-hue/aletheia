...
import * as serverExports from "@aletheia/shared/src/server";
...
vi.mock("@aletheia/shared/src/server", async (importOriginal) => {
...
vi.mock("../../env", async (importOriginal) => {
  ...
  return {
    ...original,
    env: {
      ALETHEIA_INGESTION_CLICKHOUSE_WRITE_BATCH_SIZE: 100,
      ALETHEIA_INGESTION_CLICKHOUSE_WRITE_INTERVAL_MS: 5000,
      ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS: 3,
    },
  };
});
...
describe("ClickhouseWriter", () => {
  ...
  it("should initialize with correct values", () => {
    expect(writer.batchSize).toBe(
      env.ALETHEIA_INGESTION_CLICKHOUSE_WRITE_BATCH_SIZE,
    );
    expect(writer.writeInterval).toBe(
      env.ALETHEIA_INGESTION_CLICKHOUSE_WRITE_INTERVAL_MS,
    );
    expect(writer.maxAttempts).toBe(
      env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS,
    );
  });
  ...
  it("should report wait time and processing time metrics correctly", async () => {
    ...
    expect(metricsDistributionSpy).toHaveBeenCalledWith(
      "aletheia.queue.clickhouse_writer.wait_time",
      expect.any(Number),
      { unit: "milliseconds" },
    );
