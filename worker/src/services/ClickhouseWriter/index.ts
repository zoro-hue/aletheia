...
export class ClickhouseWriter {
  ...
  private constructor() {
    this.batchSize = env.ALETHEIA_INGESTION_CLICKHOUSE_WRITE_BATCH_SIZE;
    this.writeInterval = env.ALETHEIA_INGESTION_CLICKHOUSE_WRITE_INTERVAL_MS;
    this.maxAttempts = env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS;
  ...
  public async flushAll(fullQueue = false) {
    return instrumentAsync(
      ...
      async () => {
        recordIncrement("aletheia.queue.clickhouse_writer.request");
        await Promise.all([
  ...
  private clampDecimal64Map(
    ...
    recordIncrement("aletheia.clickhouse_writer.decimal64_clamped");
    return result;
  }
  ...
  private async flush<T extends TableName>(tableName: T, fullQueue = false) {
    ...
    queueItems.forEach((item) => {
      ...
      recordHistogram("aletheia.queue.clickhouse_writer.wait_time", waitTime, {
        unit: "milliseconds",
      });
    });
    ...
    try {
      ...
      await backOff(
        ...
        {
          numOfAttempts: env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS,
          retry: (error: Error, attemptNumber: number) => {
            ...
            if (isRetryable) {
              logger.warn(
                `ClickHouse Writer failed with retryable error for ${tableName} (attempt ${attemptNumber}/${env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS}): ${error.message}`,
                {
                  error: error.message,
                  attemptNumber,
                },
              ...
              logger.warn(
                `ClickHouse Writer failed with string length error for ${tableName} (attempt ${attemptNumber}/${env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS}): Splitting batch and retrying`,
                {
                  error: error.message,
                  attemptNumber,
              ...
              logger.warn(
                `ClickHouse Writer failed with size error for ${tableName} (attempt ${attemptNumber}/${env.ALETHEIA_INGESTION_CLICKHOUSE_MAX_ATTEMPTS}): Truncating oversized records and retrying`,
                {
                  error: error.message,
                  attemptNumber,
                },
      ...
      // Log processing time
      recordHistogram(
        "aletheia.queue.clickhouse_writer.processing_time",
      ...
      queueItems.forEach((item) => {
        if (item.attempts < this.maxAttempts) {
          ...
          recordIncrement("aletheia.queue.clickhouse_writer.error");
          droppedCount++;
        }
      });
      ...
      if (droppedCount > 0) {
        recordIncrement(
          "aletheia.queue.clickhouse_writer.rows_dropped",
          droppedCount,
