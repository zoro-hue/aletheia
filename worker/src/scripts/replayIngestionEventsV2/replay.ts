...
/**
 * Standalone replay script for Aletheia ingestion events.
...
const ALETHEIA_HOST = process.env.ALETHEIA_HOST;
const ADMIN_API_KEY = process.env.ADMIN_API_KEY;

if (!ALETHEIA_HOST) {
  console.error("Error: ALETHEIA_HOST environment variable is required");
  process.exit(1);
}
if (!ADMIN_API_KEY) {
...
const API_URL = `${ALETHEIA_HOST.replace(/\/+$/, "")}/api/admin/ingestion-replay`;
