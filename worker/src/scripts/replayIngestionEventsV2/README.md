...
Replays failed ingestion events by reading S3 keys from a CSV and submitting them to Aletheia via an admin API endpoint. This replaces the [v1 script](../replayIngestionEvents/README.md), which required direct Redis, ClickHouse, and PostgreSQL access plus a full repo clone.

v2 only requires the Aletheia host URL, an admin API key, and a CSV file exported from Athena.
...
- **S3 server access logging** enabled on the Aletheia events bucket (see [Initial setup](#1-initial-setup-one-time))
...
- **`ALETHEIA_HOST`** URL of the target Aletheia instance (e.g. `https://cloud.aletheia.com`)
- **`ADMIN_API_KEY`** for authenticating against the admin API
- Network access from your machine to the Aletheia host
...
Create a dedicated S3 bucket to store access logs for your Aletheia events bucket.
...
```bash
ALETHEIA_HOST=https://cloud.aletheia.com \
...
| `ALETHEIA_HOST` | - | Target Aletheia instance URL (required) |
...
| | v1 | v2 |
|-|----|----|
| Infrastructure access | Redis, ClickHouse, PostgreSQL, S3 | Aletheia host URL only |
