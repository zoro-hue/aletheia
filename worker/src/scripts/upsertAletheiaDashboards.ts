...
import aletheiaDashboards from "../constants/aletheia-dashboards.json";
import {
  logger,
  WidgetDomainSchema,
...
export const upsertAletheiaDashboards = async (force = false) => {
  const startTime = Date.now();
  try {
    logger.debug(`Starting upsert of Aletheia dashboards (force = ${force})`);

    const parsed = FileSchema.parse(aletheiaDashboards);

    await upsertWidgets(parsed.widgets, force);
    await upsertDashboards(parsed.dashboards, force);

    logger.info(
      `Finished upserting Aletheia dashboards and widgets in ${Date.now() - startTime}ms`,
    );
  } catch (error) {
    logger.error(
      `Error upserting Aletheia dashboards after ${Date.now() - startTime}ms: ${
