...
In case the Aletheia or ClickHouse processing fails in any way, we can replay messages from S3 using the access logs or similar.
...
Make sure to place the csv file as `./worker/events.csv` in the aletheia repo.
...
ALETHEIA_S3_EVENT_UPLOAD_BUCKET=<bucket-name>
