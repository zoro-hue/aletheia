...
import { type ParsedOrganization, Role } from "@aletheia/shared";
import {
...
async function sendThresholdNotificationEmail(
  ...
  try {
    ...
    if (emailSent) {
      recordIncrement(
        "aletheia.queue.usage_threshold_queue.warning_emails_sent",
        1,
        {
          unit: "emails",
        },
    ...
    if (emailFailed) {
      recordIncrement(
        "aletheia.queue.usage_threshold_queue.email_failures",
        1,
        {
          unit: "emails",
        },
    ...
    emailFailed = true;
    recordIncrement("aletheia.queue.usage_threshold_queue.email_failures", 1, {
      unit: "emails",
    });
  }
...
async function sendBlockingNotificationEmail(
  ...
  try {
    ...
    if (emailSent) {
      recordIncrement(
        "aletheia.queue.usage_threshold_queue.blocking_emails_sent",
        1,
        {
          unit: "emails",
        },
...
export async function processThresholds(
  ...
  if (env.ALETHEIA_FREE_TIER_USAGE_THRESHOLD_ENFORCEMENT_ENABLED !== "true") {
  ...
  if (stateTransitioned && currentState === "BLOCKED") {
    ...
    recordIncrement(
      "aletheia.queue.usage_threshold_queue.blocked_orgs_total",
      1,
      {
        unit: "organizations",
      },
    ...
    recordIncrement(
      "aletheia.queue.usage_threshold_queue.warning_orgs_total",
      1,
      {
        unit: "organizations",
      },
