...
import { parseDbOrg, type ParsedOrganization } from "@aletheia/shared";
...
export async function processUsageAggregationForAllOrgs(
  ...
  for (let daysAgo = 0; daysAgo <= daysToLookBack; daysAgo++) {
    ...
    await instrumentAsync(
      ...
      async (span) => {
        span.setAttribute(
          "aletheia.free_tier.dayStart",
          dayStart.toISOString(),
        );
        span.setAttribute("aletheia.free_tier.dayEnd", dayEnd.toISOString());
        span.setAttribute("aletheia.free_tier.dayDate", dayDateString);
        span.setAttribute("aletheia.free_tier.daysAgo", daysAgo);
        ...
        for (const org of orgsToProcess) {
          ...
          if (state) {
            ...
            recordIncrement(
              "aletheia.queue.usage_threshold_queue.total_orgs",
              1,
              {
                unit: "organizations",
              },
            ...
            if (result.actionTaken === "PAID_PLAN") {
              recordIncrement(
                "aletheia.queue.usage_threshold_queue.paid_plan_orgs",
                1,
                {
                  unit: "organizations",
                },
              ...
              recordIncrement(
                "aletheia.queue.usage_threshold_queue.free_tier_orgs",
                1,
                {
                  unit: "organizations",
                },
        ...
        if (updatesToProcess.length > 0) {
          ...
          if (bulkResult.failedCount > 0) {
            recordIncrement(
              "aletheia.queue.usage_threshold_queue.bulk_update_failures",
