import { parseDbOrg, Prisma } from "@aletheia/shared";
...
export const handleCloudUsageMeteringJob = async (job: Job) => {
  ...
  for (const org of organizations) {
    ...
    if (!stripeCustomerId) {
      ...
      recordIncrement(
        "aletheia.queue.cloud_usage_metering_queue.skipped_orgs",
        1,
        {
          unit: "organizations",
        },
      ...
      recordIncrement(
        "aletheia.queue.cloud_usage_metering_queue.skipped_orgs_with_errors",
        1,
        {
          unit: "organizations",
        },
    ...
    recordIncrement(
      "aletheia.queue.cloud_usage_metering_queue.processed_orgs",
      1,
      {
        unit: "organizations",
      },
    ...
    recordIncrement(
      "aletheia.queue.cloud_usage_metering_queue.processed_observations",
      countObservations,
      {
        unit: "observations",
      },
    ...
    recordIncrement(
      "aletheia.queue.cloud_usage_metering_queue.processed_events",
      countEvents,
      {
        unit: "events",
      },
  ...
  if (meterIntervalEnd.getTime() + delayFromStartOfInterval < Date.now()) {
    ...
    recordIncrement(
      "aletheia.queue.cloud_usage_metering_queue.scheduled_catchup_jobs",
      1,
      {
        unit: "jobs",
      },
