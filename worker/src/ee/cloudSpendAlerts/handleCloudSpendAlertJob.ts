import { parseDbOrg, Role } from "@aletheia/shared";
import { prisma } from "@aletheia/shared/src/db";
import Stripe from "stripe";
import { env } from "../../env";
import { logger } from "@aletheia/shared/src/server";
import { recordIncrement, traceException } from "@aletheia/shared/src/server";
import { Job } from "bullmq";
import { backOff } from "exponential-backoff";
import { sendCloudSpendAlertEmail } from "@aletheia/shared/src/server";

const recordMissingBillingConfigSkip = (
  reason: "missing_stripe_customer_id" | "missing_stripe_subscription_id",
) => {
  recordIncrement(
    "aletheia.queue.cloud_spend_alert_queue.skipped_orgs_missing_billing_config",
    1,
    {
      unit: "organizations",
      reason,
...
export const handleCloudSpendAlertJob = async (job: Job<{ orgId: string }>) => {
  ...
  try {
        ...
        if (!alreadyTriggered) {
          ...
          if (adminEmails.length > 0) {
            try {
              ...
              recordIncrement(
                "aletheia.queue.cloud_spend_alert_queue.emails_sent",
                1,
                { unit: "emails" },
              );
              ...
              recordIncrement(
                "aletheia.queue.cloud_spend_alert_queue.email_failures",
                1,
                { unit: "emails" },
              );
              throw e;
          ...
          recordIncrement(
            "aletheia.queue.cloud_spend_alert_queue.triggered_alerts",
            1,
            {
              unit: "alerts",
            },
    ...
    recordIncrement(
      "aletheia.queue.cloud_spend_alert_queue.processed_orgs",
      1,
      {
        unit: "organizations",
      },
    ...
    recordIncrement(
      "aletheia.queue.cloud_spend_alert_queue.skipped_orgs_with_errors",
      1,
      {
        unit: "organizations",
      },
