# Agent Guidelines for Aletheia

Aletheia is an open source LLM engineering platform for developing, monitoring,
...
- Dependency direction:
  - `web` -> `@aletheia/shared`, `@aletheia/ee`
  - `worker` -> `@aletheia/shared`
  - `@aletheia/ee` -> `@aletheia/shared`
  - `@aletheia/shared` -> no imports from `web`, `worker`, or `ee`
